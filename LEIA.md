## v7.4.3 — achado o motivo de "parecer duas IAs": digitar no RP abortava o Espaço no meio

Usuário deu o diagnóstico mais preciso até agora: pedir pra ela mandar algo
no RP funciona, mas só se ele NÃO estiver tocando na caixa de digitar do
RP nativo enquanto isso processa — durante, morre sem aviso; depois de
parar de digitar, vai. Isso não é "duas IAs diferentes", é UMA sendo
abortada no meio por um sinal que não devia abortar nada.

**Causa**: `wireRpPresence` chamava `comecarAtividade('rp')` em TODA tecla
digitada na caixa nativa do RP (`send_textarea`) — mesmo sem enviar nada.
`comecarAtividade` ABORTA a atividade atual sempre que o "onde" muda
(`atividadeAtual.controller.abort()`). Se o Espaço estava no meio de
`passadaComFerramentas` processando o pedido do usuário (ex: "manda uma
mensagem"), só ele focar/digitar na caixa do RP — sem enviar — matava esse
processamento no meio (o loop para no próximo `if (controller.signal.
aborted) break`). Duas fontes de intenção competindo pelo mesmo slot de
atividade por um sinal que deveria ser só informativo, não uma tomada de
posse.

**Fix**: separei os dois papéis. `usuarioDigitandoRp` é agora um aviso LEVE
e isolado — só serve pra `activityBusy()` dizer ao Pensamento "não comece
algo novo agora", exatamente como já fazia antes de existir esse bug. Ele
NUNCA aborta nada que já esteja rodando (Espaço, RP, Pensamento) — só
`comecarAtividade` faz isso, e ela só é chamada de verdade quando o
usuário ENVIA algo (RP) ou manda mensagem no Espaço, não a cada tecla.
Some sozinho depois de ~4s sem tecla nova ou quando o campo esvazia, então
também não fica pendurado se o usuário digitar e desistir sem enviar.

**Teste**: `node --check` limpo. Esse é o mais fácil de confirmar sozinho:
pede algo no Espaço que dependa de ação no RP, e ENQUANTO espera, encosta
e digita alguma coisa na caixa nativa do RP sem enviar — antes disso
quebrava na hora, agora não deveria mais ter esse efeito.

---

## v7.4.2 — a v7.4.1 não bastou: o vínculo Espaço↔RP também falhava na hora de CRIAR

Usuário testou de novo (231 + slowburn) depois da v7.4.1 — nada mudou. O
fix da v7.4.1 (item 1) resolvia a resolução ficar presa numa conversa
errada, mas não cobria a causa mais comum na prática: **a resolução nunca
rodava pra começo de conversa**.

`resolverEspacoChatAtivo` (o que decide/corrige o vínculo) só era chamado
em `CHAT_CHANGED` (trocar de chat) ou ao abrir o painel pelo botão flutuante
— nunca em `APP_READY` (carregar a extensão). Recarregar a extensão pra
testar um fix, SEM trocar de chat antes — o caso mais comum de todos, é
literalmente o que se faz pra testar — deixava `espacoChatStPendente`
travado no valor default (`null`). Escrever no Espaço nesse estado criava
um Artefato… digo, uma conversa de Espaço nova com `stChatId: null` — nunca
batendo com o chat real na hora de montar o prompt do RP.

Dois fixes, não um — pra não depender só de UM ponto disparar certo:

1. `APP_READY` agora também chama `abrirEspacoNoContextoAtual()` — resolve
   o vínculo assim que a extensão carrega, não só quando troca de chat.
2. `garantirEspacoChatEscreve` (quem de fato CRIA/reusa a conversa de
   Espaço) não confia mais cegamente em `espacoChatStPendente` nem no
   `espacoChatIdAtual` já ativo — valida contra `stChatIdAtual()` na hora
   exata de escrever, e se não bater, resolve na hora em vez de propagar o
   vínculo errado. Antes, só existia essa checagem em QUEM decide o
   vínculo (resolverEspacoChatAtivo) — não em quem escreve de fato; se por
   qualquer motivo os dois ficassem fora de sincronia, escrever vencia.

**Teste**: `node --check` limpo. Continua sem rodar num SillyTavern de
verdade — dessa vez, importa mais que nunca: recarrega a extensão, e
antes de testar o ajuste de novo, confirma duas coisas de fora do código
(coisa que eu não consigo verificar daqui): (1) o arquivo `index.js` no
seu SillyTavern é ESSE aqui, substituído de verdade, não uma cópia velha;
(2) deu F5/reload completo na aba depois de substituir — mudança de JS só
vale depois disso, a própria extensão já avisa isso em outro contexto
(edição de código pela Construtora).

---

## v7.4.1 — 3 bugs de verdade, não força de prompt (auditoria + fix)

Usuário reportou 3 sintomas na v7.4.0: (1) pedir ajuste no Espaço ("231 no
topo", falas dinâmicas) não aparecia no RP; (2) editar/apagar a última fala
falhava depois de regenerate/swipe; (3) "trava" em segundo plano até mandar
qualquer mensagem nova. Pedido explícito: nada de sistema novo, nada de
prompt a mais — se tem bug, é bug de código, não precisa de mais instrução
pro modelo.

**Nota sobre o que já existia**: as entradas v7.2.0–v7.3.1 abaixo descrevem
um mecanismo `sempreIncluir`/`compiledAjustesFixosBlock` que **não existe
neste arquivo** — nem no schema de `artefato_escrever`, nem em
`escreverArtefato`, nem em `buildSystemPrompt` (só o `spade-fs` do lado do
servidor ainda tem o campo no frontmatter, órfão). Não investiguei a fundo
o motivo (não é o que foi pedido desta vez), só registro que o que RODA de
fato hoje é o mecanismo mais simples: `compiledEspacoRecenteBlock` inclui
as últimas mensagens do Espaço inteiras, sem flag, sem categoria especial
— o que aliás bate mais com "é uma IA só" do que a versão com flag. O bug
real não era esse mecanismo estar errado, era ele nunca RECEBER o que foi
escrito no Espaço. Achei 4 causas, todas em código, nenhuma em prompt:

**1. Ajuste do Espaço nunca chegava no RP.** `resolverEspacoChatAtivo`
caía pro "Espaço mexido mais recentemente" sempre que o chat do ST atual
não tinha Espaço linkado ainda — mesmo sabendo exatamente qual era esse
chat (`stChatIdAtual()` não nulo). A mensagem nova virava rascunho numa
conversa de Espaço com `stChatId` de OUTRA história. Na hora de gerar,
`compiledEspacoRecenteBlock` procura por `stChatId` EXATO e não acha nada
— bloco inteiro sai como `''`, silencioso. Fix: só cai pro "mais recente"
quando nem sabemos o `stChatId` atual (null de verdade); sabendo, sempre
começa um rascunho novo linkado a ESSA conversa.

**2. Erro de verdade durante a fala trava tudo.** `Spade_interceptGeneration`
tinha o `terminarAtividade` fora do `catch` — uma exceção real no meio do
streaming (rede caiu, por exemplo) ia pro catch e nunca desmarcava
`atividadeAtual`. Ficava "ocupado" pra sempre até uma mensagem nova em
qualquer lugar sobrescrever por acidente. Fix: tudo em `try/finally`, ela
desmarca sempre, sucesso ou erro.

**3. Regenerate/swipe também travava, sem erro nenhum.** O listener global
de `GENERATION_STARTED` marca ocupado pra QUALQUER tipo de geração, mas
`Spade_interceptGeneration` pula de propósito regenerate/swipe (`if (type)
return`, decisão antiga, mantida) — ninguém nunca desmarcava depois. Fix:
`GENERATION_STOPPED`/`GENERATION_ENDED` agora fecham essa atividade órfã,
guardado por token pra nunca fechar uma geração de verdade em andamento
por engano (mesmo se o evento disparar no meio do streaming custom).

**4. Editar "minha última fala" podia acertar a fala errada.**
`lastCharMessageIdx` só era atualizado dentro do caminho próprio da Spade
— depois de um regenerate/swipe nativo (que ela pula, ver #3), ficava
apontando pra uma fala antiga. Fix: resincroniza com o índice real toda vez
que `CHARACTER_MESSAGE_RENDERED` dispara, cobrindo os dois caminhos.

**Teste**: `node --check` limpo, CSS não foi tocado (0 mudança). Li cada
um dos 4 pontos de acoplamento com atenção antes de decidir o fix, não só
apliquei um patch genérico — mas **não rodei isso num SillyTavern de
verdade**, mesma ressalva de sempre. #1 e #4 eu testaria primeiro: são os
que mais batem com o que você relatou.
