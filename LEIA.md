## v7.20.0 — reasoning_content: captura e reenvia, pros modelos que exigem isso em tool call multi-turn

Confirmei a informação que o usuário trouxe direto na documentação oficial
da Xiaomi (mimo.mi.com/docs) antes de implementar, mais um bug real
reportado por outro projeto (openclaw #81419) batendo com o mesmo
sintoma: quando thinking tá ligado e a conversa tem tool_calls, toda
mensagem assistant com tool_calls que volta numa chamada seguinte
reusando esse histórico PRECISA levar o `reasoning_content` daquele
turno junto — sem isso, 400. Confirmado também: `thinking` é objeto
(`{"type":"enabled"|"disabled"}`), não `use_thinking` boolean (isso é
nome de um SDK de terceiro, não da API da Xiaomi em si) — e mimo-v2.6-pro
especificamente vem com thinking HABILITADO por padrão (lista oficial:
v2.6-flash/pro/pro-ultraspeed, v2.5-pro, v2.5, v2-pro, v2-omni; só
v2-flash vem desligado).

**Onde isso afeta a Spade de verdade**: só 2 lugares reusam um array de
mensagens ACUMULANDO tool_calls entre chamadas — `passadaComFerramentas`
(Espaço e Pensamento, dentro do próprio loop de passos) e
`passoConstrutora` (entre passos separados, via `construtoraHistory`
persistido — esse é o caso mais exposto, já que o histórico cresce ao
longo de várias chamadas de verdade, não só dentro de uma passada). RP
(`generateStream`) e as chamadas single-shot (classificador, Ingestão)
não precisam — não reusam um histórico com tool_calls de um turno
anterior.

**Fix**: `generateWithTools` agora também devolve `reasoningContent` (o
campo `reasoning_content` da resposta, se vier). Os dois lugares que
reconstroem a mensagem assistant pra reenviar (`passadaComFerramentas` e
`passoConstrutora`) anexam esse campo de volta quando presente. Pra
modelo que não manda isso (a maioria), fica `null` e nada muda — puramente
aditivo, zero risco pros outros modelos configurados.

**Teste**: `node --check` limpo. Não testado ao vivo com a MiMo
especificamente. Como o usuário mesmo sugeriu: testa primeiro numa coisa
com pouca chamada de ferramenta em sequência antes de confiar no loop de
Pensamento/Construtora, que é onde esse bug realmente morde.
