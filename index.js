...
async function respondEspaco(userMessage) {
    ...
    const systemPrompt =
        ...
        'própria de resumo, já pronta, na conversa). Se o usuário perguntar sobre algo que já foi guardado assim, ' +
        'use biblioteca_buscar/biblioteca_listar; se quiser corrigir ou remover algo que a extração entendeu errado, ' +
        'use biblioteca_editar/biblioteca_apagar — funciona igual pra entrada vinda de arquivo ou escrita por você.\n\n' +
...
async function renderBiblioteca() {
    ...
    el.innerHTML = categorias.map(([categoria, entries]) => {
        ...
        return '<div class="axis-library-folder"><div class="axis-library-folder-header">' + esc(categoria) +
...
async function ingerirComVisual(files, { statusEl = null } = {}) {
    ...
    for (const r of resultados) {
        const linha = !r.ok ? ('⚠️ ' + r.arquivo + ': ' + r.erro)
            ...
            : ('📎 ' + r.arquivo + ': ' + [(r.guardados || 0) + ' novo(s)', r.atualizados ? r.atualizados + ' atualizado(s)' : null, r.ignoradosDuplicata ? r.ignoradosDuplicata + ' duplicata(s) ignorada(s)' : null].filter(Boolean).join(', ') + '.');
