...
// ====================================
// CHATS — "colunas" tipo Claude.ai: cada chat tem memória própria de
// Espaço/Treino. Espelha o chat do SillyTavern automaticamente (troca de
// chat no RP = troca aqui também), mas também aceita chat criado só na
// extensão (sem RP correspondente — útil pra falar com o Spade sobre algo
// sem precisar abrir um RP novo pra isso). Pensamento NÃO é por chat — ela
// pensa sobre a extensão em geral, é uma coisa só, sempre.
// ====================================
let chatAtivoIdCache = 'default';
function chaveEspaco() { return 'espacoHistory:' + chatAtivoIdCache; }
function chaveTom() { return 'tomHistory:' + chatAtivoIdCache; }

async function registrarChatsSeNecessario() {
    const chats = await estadoGet('chats', []);
    const stChatId = (typeof ctx().getCurrentChatId === 'function' && ctx().getCurrentChatId()) ? String(ctx().getCurrentChatId()) : 'default';
    let mudou = false;
    if (!chats.some((c) => c.id === stChatId)) {
        chats.push({ id: stChatId, nome: 'Chat ' + (chats.length + 1), criadoEm: Date.now(), origemST: true });
        mudou = true;
    }
    if (mudou) await estadoSet('chats', chats);

    let ativo = await estadoGet('chatAtivoId', null);
    // Só segue o chat do ST automaticamente se o ativo atual TAMBÉM for de
    // origem ST (ou não existir ainda) — assim um chat criado manual na
    // extensão não é abandonado só porque o RP trocou de aba internamente.
    const chatsAtualizados = await estadoGet('chats', chats);
    const ativoAtual = chatsAtualizados.find((c) => c.id === ativo);
    if (!ativo || !ativoAtual) {
        ativo = stChatId;
        await estadoSet('chatAtivoId', ativo);
    }
    chatAtivoIdCache = ativo;
    return ativo;
}
async function listarChats() {
    return estadoGet('chats', []);
}
async function criarNovoChat() {
    const chats = await estadoGet('chats', []);
    const id = 'ext-' + newId();
    chats.push({ id, nome: 'Novo chat', criadoEm: Date.now(), origemST: false });
    await estadoSet('chats', chats);
    await estadoSet('chatAtivoId', id);
    chatAtivoIdCache = id;
    return id;
}
async function apagarChat(id) {
    let chats = await estadoGet('chats', []);
    chats = chats.filter((c) => c.id !== id);
    await estadoSet('chats', chats);
    await estadoSet('espacoHistory:' + id, []);
    await estadoSet('tomHistory:' + id, []);
    if (chatAtivoIdCache === id) {
        const novoAtivo = chats[0]?.id || 'default';
        await estadoSet('chatAtivoId', novoAtivo);
        chatAtivoIdCache = novoAtivo;
    }
    return chatAtivoIdCache;
}
async function trocarChat(id) {
    await estadoSet('chatAtivoId', id);
    chatAtivoIdCache = id;
}
...
const Biblioteca = {
    ...
    async buscarMemoria(consulta, { tipos = null, paraPersonagem = null, personagem = null, k = 8 } = {}) {
        ...
        // texto mas nunca aparecendo em biblioteca_buscar.
...
// ====================================
// DOCUMENTO CRU — guarda o texto INTEIRO do arquivo lido como entradas tipo
// "documento" na Biblioteca, em chunks buscáveis, sempre — independente do
// que a extração de fatos abaixo decidir que "vale destacar". Sem isso, um
// arquivo cujo conteúdo não virasse nenhum fato solto simplesmente sumia:
// a IA não tinha nenhum jeito de "ver" ele de novo depois, e é exatamente
// esse buraco que fazia ela dizer que não conseguia ver um .txt que você
// tinha acabado de mandar. Isso não substitui a extração — as duas
// convivem: o documento cru fica como "a folha inteira" pesquisável, os
// fatos extraídos ficam como destaques em cima dela.
// ====================================
async function guardarDocumentoBruto(nomeArquivo, textoCompleto) {
    const chunks = chunkText(textoCompleto, 900, 100);
    if (!chunks.length) return 0;
    let embeddings = [];
    try { embeddings = await embedNanoGPT(chunks.map((c) => c.slice(0, 8000))); }
    catch (e) { console.warn('[Ingestão] embedding do documento cru falhou, guardando sem busca semântica:', e.message); }
    const personagem = personagemAtual();
    const agora = Date.now();
    for (let i = 0; i < chunks.length; i++) {
        await idbPut('biblioteca', {
            id: newId(), tipo: 'documento', texto: chunks[i],
            embedding: embeddings?.[i] || null,
            metadata: { arquivo: nomeArquivo, chunk: i + 1, deChunks: chunks.length, ativo: true },
            personagem, createdAt: agora, updatedAt: agora,
            importancia: 0.35, ultimoAcesso: agora, acessos: 0,
            validoDesde: agora, validoAte: null, substituiId: null,
            entidades: [], sabemDisso: [personagem],
        });
    }
...
async function ingerirArquivo(file, onProgresso) {
    ...
    notificar('concluido', guardados + ' novo(s), ' + atualizados + ' atualizado(s), ' + ignoradosDuplicata + ' duplicata(s) ignorada(s)' + (chunksDocumento ? ', documento em ' + chunksDocumento + ' trecho(s)' : ''));
    return { arquivo: file.name, ok: true, guardados, atualizados, ignoradosDuplicata, chunksDocumento, totalExtraido: itensExtraidos.length };
}
...
const TOOLS = [
    ...
    { type: 'function', function: { name: 'biblioteca_ler_documento', description: 'Lê um documento INTEIRO da Biblioteca, na ordem certa, pelo nome exato do arquivo — use ANTES de biblioteca_editar_documento, é mais confiável que buscar por significado (que só traz pedaços parecidos, não o documento inteiro em ordem).', parameters: { type: 'object', properties: { arquivo: { type: 'string' } }, required: ['arquivo'] } } },
    { type: 'function', function: { name: 'biblioteca_editar_documento', description: 'Reescreve um documento INTEIRO guardado na Biblioteca (os chunks de um arquivo que foi lido, tratados como uma folha só) — útil pra limpar/organizar um documento bagunçado, ou deixar só o que interessa. SEMPRE chame biblioteca_ler_documento com esse mesmo nome de arquivo antes, pra não perder conteúdo por engano.', parameters: { type: 'object', properties: { arquivo: { type: 'string', description: 'Nome do arquivo, exatamente como aparece em metadata.arquivo dos resultados de busca.' }, novoTexto: { type: 'string' } }, required: ['arquivo', 'novoTexto'] } } },
    ...
    { type: 'function', function: { name: 'redirecionar_biblioteca', description: 'Só existe no Espaço. Abre a aba de Biblioteca — use quando o usuário mandar arquivo/foto de REFERÊNCIA geral (lore, fatos, perfil) pra guardar. Se o conteúdo forem exemplos de FALA/DIÁLOGO da personagem (treino de tom/voz), use redirecionar_tom em vez disso.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'redirecionar_treino', description: 'Só existe no Espaço. Abre o mini-chat de Treino (documentos de referência pro RP: tom de voz, lore, regra de mundo, exemplo de diálogo, o que for) — use quando o usuário mandar material de referência ou pedir explicitamente pra treinar/ensinar algo sobre a personagem ou o mundo. Se não tiver certeza se é isso ou uma referência avulsa pra Biblioteca geral, PERGUNTE antes de chamar qualquer uma das duas.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'documento_listar', description: 'Lista os documentos de referência já guardados pra personagem atual (qualquer categoria — tom, lore, regra de mundo, o que for).', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'documento_ler', description: 'Lê o conteúdo inteiro de um documento específico.', parameters: { type: 'object', properties: { slug: { type: 'string', description: 'Identificador do documento, como aparece em documento_listar.' } }, required: ['slug'] } } },
    { type: 'function', function: { name: 'documento_escrever', description: 'Cria ou reescreve um documento inteiro — o material curado, já organizado e bem escrito (não é despejo do arquivo original). Isso sincroniza direto com a busca usada no RP de verdade — o que você escrever aqui pode aparecer na cena. Chame documento_ler antes se for editar um que já existe, pra não perder conteúdo. Escreva com qualidade de verdade: prosa clara, organizada, rica o suficiente pra colorir um diálogo de RP — não uma lista seca de fragmentos soltos.', parameters: { type: 'object', properties: { slug: { type: 'string', description: 'Se omitido num documento NOVO, invente um curto a partir do título.' }, titulo: { type: 'string' }, categoria: { type: 'string', description: 'Livre — ex: tom, lore, regra_mundo, exemplo_dialogo, aparencia, relacionamento. Invente uma se nenhuma encaixar.' }, tags: { type: 'array', items: { type: 'string' } }, conteudo: { type: 'string' } }, required: ['titulo', 'conteudo'] } } },
    { type: 'function', function: { name: 'documento_apagar', description: 'Apaga um documento inteiro (vira backup, recuperável pelo lado do servidor se precisar).', parameters: { type: 'object', properties: { slug: { type: 'string' } }, required: ['slug'] } } },
...
async function executeTool(name, args) {
    switch (name) {
        ...
        case 'biblioteca_ler_documento': return Biblioteca.lerDocumento(args.arquivo);
        case 'biblioteca_editar_documento': return Biblioteca.editarDocumento(args.arquivo, args.novoTexto);
        ...
        case 'biblioteca_buscar': { const r = await Biblioteca.buscar({ consulta: args.consulta, tipos: args.tipos || null, k: 6, paraNpc: args.paraNpc || null }); return r.map((e) => ({ id: e.id, tipo: e.tipo, texto: e.texto, similaridade: Number(e.score.toFixed(2)), arquivo: e.metadata?.arquivo || null })); }
        ...
        case 'extensao_editar_arquivo': { const r = await chamarSpadeFs('/escrever', { arquivo: args.arquivo, conteudo: args.conteudo }); mostrarBannerAtualizacao(args.arquivo); return Object.assign({ aviso: 'escrito — só vale de verdade depois que a página recarregar.' }, r); }
        case 'extensao_restaurar_backup': { const r = await chamarSpadeFs('/restaurar', { arquivo: args.arquivo }); mostrarBannerAtualizacao(args.arquivo); return r; }
        ...
        // qualquer (ver ali: pausaSegundos / redirecionarBiblioteca).
        ...
        case 'redirecionar_biblioteca': return { redirecionar: 'biblioteca' };
        case 'redirecionar_treino': return { redirecionar: 'tom' };
        case 'documento_listar': return chamarSpadeFs('/documento/listar', { personagem: personagemAtual() });
        case 'documento_ler': return chamarSpadeFs('/documento/ler', { personagem: personagemAtual(), slug: args.slug });
        case 'documento_escrever': {
            const slug = args.slug || slugifyLocal(args.titulo);
            const categoria = args.categoria || 'documento';
            const r = await chamarSpadeFs('/documento/escrever', { personagem: personagemAtual(), slug, titulo: args.titulo, categoria, tags: args.tags || [], conteudo: args.conteudo });
            atualizarDocumentoAoVivo({ slug, titulo: args.titulo, conteudo: args.conteudo });
            atualizarContadorArtefatos();
            const chunks = await sincronizarDocumentoNaBiblioteca(slug, args.titulo, categoria, args.conteudo);
            return Object.assign(r, { sincronizadoNaBiblioteca: chunks + ' trecho(s) — já buscável no RP de verdade agora.' });
        }
        case 'documento_apagar': {
...
const ACTION_LABEL = {
    ...
    biblioteca_ler_documento: '📖 lendo um documento inteiro',
    biblioteca_editar_documento: '📄 reorganizando um documento',
    ...
    redirecionar_biblioteca: '📚 abrindo a biblioteca',
    redirecionar_treino: '📄 abrindo o treino',
    documento_listar: '📄 vendo os documentos',
    documento_ler: '📄 lendo um documento',
    documento_escrever: '📄 escrevendo no documento',
    documento_apagar: '📄 apagando um documento',
...
const TOOLS_CONSTRUTORA_NOMES = ['sistema_real_testar', 'sistema_real_publicar', 'sistema_real_ativar', 'sistema_real_listar', 'biblioteca_buscar', 'estudar_codigo', 'extensao_listar_arquivos', 'extensao_ler_arquivo', 'extensao_editar_arquivo', 'extensao_restaurar_backup'];
...
async function passoConstrutora(atividade, instrucao) {
    ...
    // Mesmo remédio do sendTomMessage/pensarUmaVez: erro de rede/API não
...
// ====================================
// BANNER DE ATUALIZAÇÃO — aviso FORTE pedido explicitamente: depois que o
// código da própria extensão é editado (Construtora), isso fica bem
// visível até você decidir aplicar — não é só uma linha discreta no
// journal. Botão faz commit+push de verdade (se o repo já tiver remote
// configurado — ver spade-fs LEIA) e recarrega a página, que é o único
// jeito real de um código novo substituir o que já tá rodando.
// ====================================
function garantirBannerAtualizacao() {
    let banner = document.getElementById('axis-banner-atualizacao');
    if (banner) return banner;
    banner = document.createElement('div');
    banner.id = 'axis-banner-atualizacao';
    banner.className = 'axis-banner-atualizacao axis-hidden';
    banner.innerHTML =
        '<div class="axis-banner-texto">⚠️ Código da extensão editado (<b id="axis-banner-arquivo"></b>) — precisa recarregar pra valer.</div>' +
        '<div class="axis-banner-status" id="axis-banner-status"></div>' +
        '<button type="button" class="axis-banner-btn" id="axis-banner-btn">🔄 Atualizar agora</button>';
    document.body.appendChild(banner);
    document.getElementById('axis-banner-btn').addEventListener('click', aplicarAtualizacao);
    return banner;
}
function mostrarBannerAtualizacao(arquivo) {
    garantirBannerAtualizacao();
    document.getElementById('axis-banner-arquivo').textContent = arquivo;
    document.getElementById('axis-banner-atualizacao').classList.remove('axis-hidden');
}
async function aplicarAtualizacao() {
    const statusEl = document.getElementById('axis-banner-status');
    const btnEl = document.getElementById('axis-banner-btn');
    btnEl.disabled = true;
    try {
        if (statusEl) statusEl.textContent = 'publicando (git commit + push)...';
        await chamarSpadeFs('/git/publicar', { mensagem: 'Atualização via Spade Construtora' });
        if (statusEl) statusEl.textContent = 'publicado — recarregando...';
    } catch (e) {
        // Falha de git não impede o reload — a escrita local já aconteceu
        // e já vale; git é só o "compartilha com o GitHub" por cima disso.
        if (statusEl) statusEl.textContent = '⚠️ git não publicou (' + e.message.slice(0, 90) + ') — a mudança local já tá salva mesmo assim, recarregando...';
        await sleep(2500);
    }
    location.reload();
}
...
async function renderJournalEm(elId, contexto, vazioTexto) {
    ...
    const rotulo = { construtora: '🔧 Construtora', ingestao: '📎 Ingestão' };
    el.innerHTML = entradas.slice(-60).map((e) => renderBolhaV2('ia', e.texto, rotulo[e.autor] || null)).join('')
        || '<div class="axis-empty">' + vazioTexto + '</div>';
...
async function passadaComFerramentas(messages, atividade, { onNarracao } = {}, opts = {}) {
    ...
    let redirecionarBiblioteca = false;
    let redirecionarTom = false;
    const toolsChamadas = [];
    ...
    for (let i = 0; i < teto; i++) {
        ...
        for (const call of resp.toolCalls) {
            ...
            if (nome === 'redirecionar_biblioteca') redirecionarBiblioteca = true;
            if (nome === 'redirecionar_treino') redirecionarTom = true;
    ...
    return { respostaFinal, pausaSegundos, redirecionarBiblioteca, redirecionarTom, toolsChamadas };
}
...
async function respondEspaco(userMessage) {
    ...
    const history = await estadoGet(chaveEspaco(), []);
    ...
    const systemPrompt =
        ...
        'arquivo por texto: ele passa pelo pipeline de Ingestão sozinho (você vê o resultado como uma mensagem sua ' +
        'própria de resumo, já pronta, na conversa). O texto INTEIRO do arquivo fica guardado na Biblioteca como ' +
        'entradas tipo "documento" (buscável por biblioteca_buscar, sem precisar filtrar tipo — vem junto com o ' +
        'resto), além de qualquer fato solto que a extração tenha destacado. Se o usuário perguntar o que tinha no ' +
        'arquivo ou pedir pra usar algo de lá, SEMPRE chame biblioteca_buscar antes de dizer que não tem acesso — ' +
        'na grande maioria dos casos o conteúdo está lá, mesmo quando nenhum fato "importante" foi extraído dele. ' +
        'Se quiser corrigir ou remover algo que a extração entendeu errado, use biblioteca_editar/biblioteca_apagar ' +
        '— funciona igual pra entrada vinda de arquivo ou escrita por você.\n\n' +
        'Se o usuário mandar exemplos de FALA/DIÁLOGO da personagem, ou pedir explicitamente pra treinar como ela ' +
        'fala, chame redirecionar_tom (não redirecionar_biblioteca) — isso abre um mini-chat dedicado só pra isso. ' +
        'Se não tiver certeza se é treino de tom ou referência geral, pergunte antes de chamar qualquer um dos dois.\n\n' +
        'Cena recente do RP, pra contexto:\n' + (cena || '(nada ainda)');
    const messages = [{ role: 'system', content: systemPrompt }, ...history];

    let respostaFinal = '', erro = null, redirecionarBiblioteca = false, redirecionarTom = false, toolsChamadas = [];
    try {
        const r = await passadaComFerramentas(messages, atividade, {
            onNarracao: (texto) => escreverTicker(texto),
        });
        respostaFinal = r.respostaFinal;
        redirecionarBiblioteca = r.redirecionarBiblioteca;
        redirecionarTom = r.redirecionarTom;
    ...
    if (!erro) {
        ...
        const historicoAgora = await estadoGet(chaveEspaco(), []);
        await estadoSet(chaveEspaco(), [...historicoAgora, { role: 'assistant', content: respostaFinal }].slice(-80));
    }
    dotsIdle();
    return { resposta: respostaFinal, erro, abandoned: isStale(atividade.token), redirecionarBiblioteca, redirecionarTom, toolsChamadas };
}
...
// Card de passos — <details>/<summary> nativo, sem JS de toggle: o chat
// inteiro é re-renderizado do zero a cada mensagem nova (padrão já usado
// em todo o arquivo), então um listener de clique individual se perderia
// a cada render; o navegador já resolve abrir/fechar sozinho com isso.
function renderCardPassos(toolsChamadas) {
    if (!toolsChamadas?.length) return '';
    const itens = toolsChamadas.map((nome) => '<div class="axis-passo-item">' + labelFerramenta(nome) + '</div>').join('');
    return '<details class="axis-passos-card"><summary class="axis-passos-cabecalho">' +
        '<span class="axis-passos-seta">▶</span>' + toolsChamadas.length + ' passo' + (toolsChamadas.length > 1 ? 's' : '') +
        '</summary><div class="axis-passos-corpo">' + itens + '</div></details>';
}
// Bolha tipo Claude.ai — usuário em bolha preenchida, IA em texto solto
// serifado, sem fundo, sem listra de cor por autor (pedido explícito:
// "sem aquelas listras feias"). `autor` só aparece pra IA quando não é a
// principal falando (ex: Construtora, Treino) — pequeno, discreto, maiúsculo.
function renderBolhaV2(role, texto, autor, toolsChamadas) {
    const cardPassos = renderCardPassos(toolsChamadas);
    if (role === 'user') return '<div class="axis-msgv2">' + cardPassos + '<div class="axis-msgv2-user">' + renderMarkers(texto) + '</div></div>';
    const rotuloAutor = autor && autor !== 'principal' ? '<div class="axis-msgv2-autor">' + esc(autor) + '</div>' : '';
    return '<div class="axis-msgv2">' + cardPassos + rotuloAutor + '<div class="axis-msgv2-ia">' + renderMarkers(texto) + '</div></div>';
}
function renderEspacoChat() {
    const chatEl = document.getElementById('axis-espaco-chat');
    if (!chatEl) return;
    chatEl.innerHTML = espacoLocalLog.map((m) => renderBolhaV2(m.role, m.display || m.content, null, m.toolsChamadas)).join('')
        || '<div class="axis-empty">Fala alguma coisa pra começar.</div>';
    chatEl.scrollTop = chatEl.scrollHeight;
}
async function carregarEspacoHistorico() {
    const hist = await estadoGet(chaveEspaco(), []);
...
async function sendEspacoMessage(text) {
    ...
    // do caminho (ou CHAT_CHANGED disparar carregarEspacoHistorico de novo),
    // a mensagem do usuário não some: já tá salva, só a resposta que falta.
    const historicoAtual = await estadoGet(chaveEspaco(), []);
    await estadoSet(chaveEspaco(), [...historicoAtual, { role: 'user', content: text }].slice(-80));
    ...
    try {
        ...
        espacoLocalLog.push({ role: 'agent', content: result.erro ? '⚠️ ' + result.erro : (result.resposta || '(sem resposta)'), toolsChamadas: result.toolsChamadas });
        renderEspacoChat();
        if (result.redirecionarBiblioteca) switchTab('biblioteca'); // ela mesma decidiu redirecionar
        if (result.redirecionarTom) switchTab('tom'); // idem, mas pro mini-chat de treino de tom
    } finally {
...
async function handleUploadEspaco(files) {
    ...
    renderEspacoChat();
    const historicoAntes = await estadoGet(chaveEspaco(), []);
    await estadoSet(chaveEspaco(), [...historicoAntes, { role: 'user', content: '[enviou arquivo(s) pra estudar: ' + nomes + ']' }].slice(-80));
    ...
    const resumo = resultados.map((r) => {
        ...
        if (r.chunksDocumento) partes.push('documento em ' + r.chunksDocumento + ' trecho(s)');
        return r.arquivo + ': ' + partes.join(', ');
    }).join(' · ');
    const respostaFinal = 'Terminei de estudar isso — ' + resumo + '. Se quiser ver os detalhes ou ajustar algo, é só pedir ou dar uma olhada na Biblioteca.';
    espacoLocalLog.push({ role: 'agent', content: respostaFinal });
    renderEspacoChat();
    const historicoDepois = await estadoGet(chaveEspaco(), []);
    await estadoSet(chaveEspaco(), [...historicoDepois, { role: 'assistant', content: respostaFinal }].slice(-80));
    switchTab('espaco');
}

// ====================================
// TREINO — mini-chat dedicado, separado do Espaço e da Sala de Pensamento
// de propósito (era exatamente essa mistura que fazia a animação/sala
// errada aparecer quando o assunto era treinar algo pro RP). Documento é
// sempre reescrito INTEIRO (documento_escrever) — nunca por chunk solto,
// é "o documento", não pedaços.
// ====================================
// Ponte real com o RP — sem isso o documento só existe dentro do mini-chat.
// O documento (arquivo .md, fonte editável de verdade) sincroniza pra dentro
// da Biblioteca (índice buscável, chunk + embedding) toda vez que muda —
// é o mesmo padrão de "arquivo fonte + índice de busca" que qualquer app
// de notas sério usa. buscarVoz (MMR) já roda em TODA rodada de RP; a
// folha só precisa aparecer lá dentro, não precisa de caminho novo.
// Ponte real com o RP — sem isso o documento só existe dentro do mini-chat.
// O documento (arquivo .md, fonte editável de verdade) sincroniza pra
// dentro da Biblioteca (índice buscável, chunk + embedding) toda vez que
// muda — é o mesmo padrão de "arquivo fonte + índice de busca" que
// qualquer app de notas sério usa. buscarVoz (MMR) já roda em TODA rodada
// de RP; o documento só precisa aparecer lá dentro, não precisa de
// caminho novo.
...
let tomLocalLog = [];
let documentoAtual = null; // { slug, titulo, conteudo } | null — o que o painel ao vivo mostra agora
function slugifyLocal(texto) {
    return String(texto || 'documento').trim().toLowerCase()
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'documento';
}
function renderTomChat() {
    const chatEl = document.getElementById('axis-tom-chat');
    if (!chatEl) return;
    chatEl.innerHTML = tomLocalLog.map((m) => renderBolhaV2(m.role, m.display || m.content, null, m.toolsChamadas)).join('')
        || '<div class="axis-empty">Manda um exemplo de fala dela, um trecho de lore, uma regra do mundo — o que for material de referência pro RP — colado ou por arquivo.</div>';
    chatEl.scrollTop = chatEl.scrollHeight;
}
// Painel ao lado — o documento "ao vivo". Diff de verdade por palavra
// (diffPalavras, LCS — ver comentário na função), mesmo efeito visual de
// "+ apareceu algo novo" que um diff de VS Code dá, sem precisar de
// biblioteca nenhuma.
function atualizarDocumentoAoVivo({ slug, titulo, conteudo }) {
    const anterior = documentoAtual?.slug === slug ? (documentoAtual.conteudo || '') : '';
    documentoAtual = { slug, titulo, conteudo };
    const el = document.getElementById('axis-tom-folha-corpo');
    const tituloEl = document.getElementById('axis-tom-folha-titulo');
    ...
    el.scrollTop = el.scrollHeight;
    document.getElementById('axis-tom-folha-vazio')?.classList.add('axis-hidden');
}
async function renderListaDocumentos() {
    const selectEl = document.getElementById('axis-tom-folha-select');
    if (!selectEl) return;
    try {
        const r = await chamarSpadeFs('/documento/listar', { personagem: personagemAtual() });
        const documentos = r.documentos || [];
        selectEl.innerHTML = '<option value="">+ novo documento</option>' + documentos.map((d) =>
            '<option value="' + esc(d.slug) + '">' + esc(d.titulo) + (d.categoria && d.categoria !== 'documento' ? ' · ' + esc(d.categoria) : '') + '</option>').join('');
        // Segue o que ela acabou de escrever — não precisa escolher manual
        // pra ver o que aconteceu.
        if (documentoAtual?.slug) selectEl.value = documentoAtual.slug;
    } catch (e) { console.warn('[documento] listar falhou:', e.message); }
}
async function carregarTomHistorico() {
    const hist = await estadoGet(chaveTom(), []);
    tomLocalLog = hist.map((m) => ({ role: m.role, content: m.content }));
    renderTomChat();
    renderListaDocumentos();
}
async function respondTom(userMessage) {
    const atividade = comecarAtividade('tom');
    dotsThinking();
    const charName = personagemAtual();
    const history = await estadoGet(chaveTom(), []);
    const documentosExistentes = await chamarSpadeFs('/documento/listar', { personagem: charName }).catch(() => ({ documentos: [] }));
    const systemPrompt =
        'Você é o Spade, num mini-chat dedicado a preparar material de referência pro RP de ' + charName + ' — não é ' +
        'o Espaço geral nem a Sala de Pensamento, aqui é sobre TREINAR: tom de voz, lore, regra de mundo, exemplo de ' +
        'diálogo, o que o usuário trouxer. Ele vai mandar material colado ou por arquivo — se mandar arquivo, o ' +
        'texto bruto inteiro já vem na mensagem dele, você lê direto, não precisa de ferramenta pra "abrir" nada.\n\n' +
        'Seu trabalho é CURADORIA DE VERDADE, não recorte raso: separe o que vale (voz, cadência, fato de mundo, ' +
        'regra, exemplo) do que é ruído (narração de terceiros, metadado, lixo do arquivo original) — e então ' +
        'ESCREVA um documento bem feito com documento_escrever, na categoria que fizer sentido (tom, lore, ' +
        'regra_mundo, o que for — invente uma se precisar). "Bem feito" quer dizer: prosa organizada, com começo/' +
        'meio, que já lê BEM sozinha — pense em como esse texto vai aparecer influenciando uma cena de RP de ' +
        'verdade; um amontoado de frases soltas sem transição fica ruim ali, mesmo que a informação esteja certa. ' +
        'Reescreva com as próprias palavras onde ajudar a clareza, mas preserve exemplo de fala literal quando o ' +
        'valor DELE é justamente ser literal.\n\n' +
        'Um documento é o INTEIRO, sempre — se já existe um sobre o mesmo assunto, chame documento_ler nele primeiro ' +
        'e reescreva completo (documento_escrever de novo com o mesmo slug), nunca deixe pedaço perdido. Se ' +
        'genuinamente não tiver nada aproveitável no que o usuário mandou, diga isso direto em vez de forçar um ' +
        'documento vazio.\n\n' +
        'Documentos já existentes pra ' + charName + ': ' + (documentosExistentes.documentos?.length ? documentosExistentes.documentos.map((d) => d.titulo + (d.categoria && d.categoria !== 'documento' ? ' (' + d.categoria + ')' : '')).join(', ') : '(nenhum ainda)') + '.';
    const messages = [{ role: 'system', content: systemPrompt }, ...history, { role: 'user', content: userMessage }];

    let respostaFinal = '', erro = null, toolsChamadas = [];
    try {
        const r = await passadaComFerramentas(messages, atividade, { onNarracao: (texto) => escreverTicker(texto) });
        respostaFinal = r.respostaFinal;
        toolsChamadas = r.toolsChamadas;
    } catch (e) {
        erro = e.message;
    ...
    if (!erro) {
        const historicoAgora = await estadoGet(chaveTom(), []);
        await estadoSet(chaveTom(), [...historicoAgora, { role: 'user', content: userMessage }, { role: 'assistant', content: respostaFinal }].slice(-80));
    }
    dotsIdle();
    return { resposta: respostaFinal, erro, abandoned: isStale(atividade.token), toolsChamadas };
}
async function sendTomMessage(text, displayText, tentativa = 1) {
    if (tentativa === 1) {
        tomLocalLog.push({ role: 'user', content: text, display: displayText || text });
        renderTomChat();
    }
    const inputEl = document.getElementById('axis-tom-input');
    const sendBtn = document.getElementById('axis-tom-send');
    if (inputEl) inputEl.disabled = true;
    if (sendBtn) sendBtn.disabled = true;
    try {
        const result = await respondTom(text);
        if (result.abandoned) return;
        if (result.erro) {
            // Erro de verdade (já sobreviveu aos retries de generate/
            // generateWithTools) — em vez de jogar o JSON cru na tela e
            // esperar o usuário mandar de novo, ela mesma tenta de novo,
            // com espera crescente. Só desiste e avisa depois de 6x.
            if (tentativa < 6) {
                const esperaMs = Math.min(3000 * tentativa, 20000);
                escreverTicker('tropeçou (' + result.erro.slice(0, 60) + ') — tentando de novo em ' + Math.round(esperaMs / 1000) + 's...');
                await new Promise((r) => setTimeout(r, esperaMs));
                if (inputEl) inputEl.disabled = false;
                if (sendBtn) sendBtn.disabled = false;
                return sendTomMessage(text, displayText, tentativa + 1);
            }
            tomLocalLog.push({ role: 'agent', content: '⚠️ isso não voltou depois de ' + tentativa + ' tentativas (' + result.erro + ') — pode precisar da sua atenção (chave de API, modelo configurado em Config, etc).' });
        } else {
            tomLocalLog.push({ role: 'agent', content: result.resposta || '(sem resposta)', toolsChamadas: result.toolsChamadas });
        }
        renderTomChat();
        renderListaDocumentos();
    } finally {
...
// Upload aqui NÃO passa pela Ingestão geral — o texto bruto vai direto pro
// chat como mensagem do usuário (ela já lê arquivo de sobra, é só texto),
// que é o que deixa o mini-chat realmente conversar sobre o conteúdo em
// vez de só devolver um resumo enlatado.
async function handleUploadTom(files) {
    const lidos = [];
    const nomes = [];
    for (const file of files) {
        const lido = await lerArquivo(file).catch((e) => ({ ok: false, erro: e.message }));
        if (!lido.ok) { tomLocalLog.push({ role: 'agent', content: '⚠️ não consegui ler ' + file.name + ': ' + lido.erro }); continue; }
        lidos.push('[arquivo: ' + file.name + ']\n' + (lido.conteudo || ''));
        nomes.push(file.name);
    }
    renderTomChat();
    if (!lidos.length) return;
    const textoCompleto = lidos.join('\n\n---\n\n');
    // O que vai pro modelo é o texto inteiro; o que aparece na tela é só um
    // chip — "não envia como arquivo, envia como texto" era isso: a bolha
    // virava uma parede de texto cru em vez de parecer um anexo de verdade.
    const display = '📎 ' + nomes.join(', ') + ' (' + textoCompleto.length.toLocaleString('pt-BR') + ' caracteres)';
    await sendTomMessage(textoCompleto, display);
}
async function carregarDocumentoSelecionado(slug) {
    if (!slug) {
        documentoAtual = null;
        const el = document.getElementById('axis-tom-folha-corpo');
        if (el) el.innerHTML = '';
        document.getElementById('axis-tom-folha-titulo').textContent = 'novo documento';
        document.getElementById('axis-tom-folha-vazio')?.classList.remove('axis-hidden');
        return;
    }
    try {
    ...
    } catch (e) { console.warn('[documento] ler falhou:', e.message); }
}

// ====================================
// BIBLIOTECA — UI de upload/lista/busca/apagar. Agrupada por categoria
// (accordion de verdade, não só CSS solto), filtrável por uso, arquivar
// como ação explícita (não checkbox por linha — isso foi retirado de
// propósito, era o "jeito horrível de ativar e desativar").
// ====================================
const USO_LABEL = { perfil: '👤 perfil', tom: '🎭 tom/voz', sistemas: '⚙️ sistemas', geral: '📌 geral' };
let bibliotecaFiltroUso = null; // null = todos
let bibliotecaMostrarArquivados = false;
...
async function renderBiblioteca() {
    ...
    const filtrosEl = document.getElementById('axis-biblioteca-filtros');
    if (!el) return;
    const todas = await Biblioteca.listar({});
    if (!todas.length) {
        el.innerHTML = '<div class="axis-empty">Biblioteca vazia — sobe um arquivo ou deixa ela escrever sozinha.</div>';
        if (filtrosEl) filtrosEl.innerHTML = '';
        return;
    }

    if (filtrosEl) {
        const usosPresentes = new Set();
        todas.forEach((e) => (e.metadata?.usos?.length ? e.metadata.usos : ['geral']).forEach((u) => usosPresentes.add(u)));
        const botoesUso = ['todos', ...usosPresentes].map((u) => {
            const ativo = bibliotecaFiltroUso === (u === 'todos' ? null : u);
            return '<button type="button" class="axis-btn axis-btn-sm axis-bib-filtro' + (ativo ? ' axis-bib-filtro-ativo' : '') + '" data-uso="' + esc(u) + '">' + esc(USO_LABEL[u] || u) + '</button>';
        }).join('');
        filtrosEl.innerHTML = botoesUso +
            '<button type="button" class="axis-btn axis-btn-sm' + (bibliotecaMostrarArquivados ? ' axis-bib-filtro-ativo' : '') + '" id="axis-bib-arquivados">🗄️ arquivados</button>';
        filtrosEl.querySelectorAll('.axis-bib-filtro').forEach((b) => b.addEventListener('click', () => {
            bibliotecaFiltroUso = b.dataset.uso === 'todos' ? null : b.dataset.uso;
            renderBiblioteca();
        }));
        const arqBtn = document.getElementById('axis-bib-arquivados');
        if (arqBtn) arqBtn.addEventListener('click', () => { bibliotecaMostrarArquivados = !bibliotecaMostrarArquivados; renderBiblioteca(); });
    }

    const filtradas = todas.filter((e) => {
        const ativo = e.metadata?.ativo !== false;
        if (bibliotecaMostrarArquivados ? ativo : !ativo) return false;
        if (bibliotecaFiltroUso && !(e.metadata?.usos?.length ? e.metadata.usos : ['geral']).includes(bibliotecaFiltroUso)) return false;
        return true;
    });
    if (!filtradas.length) { el.innerHTML = '<div class="axis-empty">Nada aqui com esse filtro.</div>'; return; }

    // agrupa por categoria (tipo/uso livre da extração) — é o folder de
    // verdade agora, não só uma div com nome de folder que nunca abria nada.
    const porCategoria = new Map();
    for (const e of filtradas) {
        const cat = e.tipo || 'outro';
        if (!porCategoria.has(cat)) porCategoria.set(cat, []);
        porCategoria.get(cat).push(e);
    }
    const categorias = [...porCategoria.entries()].sort((a, b) => b[1].length - a[1].length);

    el.innerHTML = categorias.map(([categoria, entries]) => {
        let corpo;
        if (categoria === 'documento') {
            // Agrupa os chunks por arquivo — "a folha inteira", não N pedaços
            // soltos com o mesmo nome repetido. Ordena chunk por índice antes
            // de juntar, pra reconstruir o texto na ordem certa.
            const porArquivo = new Map();
            for (const e of entries) {
                const nome = e.metadata?.arquivo || '(sem nome)';
                if (!porArquivo.has(nome)) porArquivo.set(nome, []);
                porArquivo.get(nome).push(e);
            }
            corpo = [...porArquivo.entries()].map(([nome, chunks]) => {
                chunks.sort((a, b) => (a.metadata?.chunk || 0) - (b.metadata?.chunk || 0));
                const textoCompleto = chunks.map((c) => c.texto).join('\n\n');
                const arquivado = chunks[0].metadata?.ativo === false;
                const preview = textoCompleto.length > 220 ? textoCompleto.slice(0, 220) + '…' : textoCompleto;
                return '<div class="axis-bib-item axis-bib-documento' + (arquivado ? ' axis-bib-item-arquivado' : '') + '" data-arquivo="' + esc(nome) + '">' +
                    '<div class="axis-bib-texto axis-bib-doc-preview">📄 <strong>' + esc(nome) + '</strong><br>' + esc(preview) + '</div>' +
                    '<div class="axis-bib-doc-full" style="display:none;"><textarea class="axis-bib-doc-textarea" rows="10">' + esc(textoCompleto) + '</textarea>' +
                    '<div class="axis-bib-doc-actions"><button type="button" class="axis-btn axis-btn-sm axis-bib-doc-salvar">salvar</button>' +
                    '<button type="button" class="axis-btn axis-btn-sm axis-bib-doc-cancelar">cancelar</button></div></div>' +
                    '<div class="axis-bib-meta">' +
                    '<span class="axis-bib-data">' + chunks.length + ' trecho(s) · ' + formatarRelativo(chunks[0].updatedAt || chunks[0].createdAt) + '</span>' +
                    '<button type="button" class="axis-btn axis-btn-sm axis-bib-doc-ver">ver/editar</button>' +
                    '<button type="button" class="axis-btn axis-btn-sm axis-system-delete axis-bib-doc-del">apagar</button>' +
                    '</div></div>';
            }).join('');
        } else {
            corpo = entries.map((e) => {
            const usos = (e.metadata?.usos?.length ? e.metadata.usos : ['geral'])
                .map((u) => '<span class="axis-bib-tag">' + esc(USO_LABEL[u] || u) + '</span>').join('');
            const entidades = (e.entidades || []).length ? '<span class="axis-bib-entidades">🏷️ ' + esc(e.entidades.join(', ')) + '</span>' : '';
            const importanciaPct = Math.round((e.importancia ?? 0.5) * 100);
            const arquivado = e.metadata?.ativo === false;
            const origem = e.metadata?.origemArquivo ? ' · ' + esc(e.metadata.origemArquivo) : '';
            return '<div class="axis-bib-item' + (arquivado ? ' axis-bib-item-arquivado' : '') + '">' +
                '<div class="axis-bib-texto">' + esc(e.texto.length > 220 ? e.texto.slice(0, 220) + '…' : e.texto) + '</div>' +
                '<div class="axis-bib-meta">' +
                '<span class="axis-bib-importancia" title="importância ' + importanciaPct + '%"><span class="axis-bib-importancia-fill" style="width:' + importanciaPct + '%;"></span></span>' +
                usos + entidades +
                '<span class="axis-bib-data">' + formatarRelativo(e.updatedAt || e.createdAt) + origem + '</span>' +
                '<button type="button" class="axis-btn axis-btn-sm axis-bib-arquivar" data-id="' + esc(e.id) + '" data-set="' + (arquivado ? 'true' : 'false') + '">' + (arquivado ? 'restaurar' : 'arquivar') + '</button>' +
                '<button type="button" class="axis-btn axis-btn-sm axis-system-delete axis-bib-del" data-id="' + esc(e.id) + '">apagar</button>' +
                '</div></div>';
            }).join('');
        }
        return '<div class="axis-library-folder"><div class="axis-library-folder-header">' + esc(categoria === 'documento' ? '📄 documentos' : categoria) +
            ' <span class="axis-bib-count">(' + entries.length + ')</span></div>' +
            '<div class="axis-library-folder-body" style="display:none;">' + corpo + '</div></div>';
    }).join('');

    el.querySelectorAll('.axis-library-folder-header').forEach((h) => h.addEventListener('click', () => {
        const folder = h.parentElement;
        const body = folder.querySelector('.axis-library-folder-body');
        const aberto = folder.classList.toggle('axis-library-folder-open');
    ...
    el.querySelectorAll('.axis-bib-arquivar').forEach((btn) => btn.addEventListener('click', async () => {
        await Biblioteca.definirAtivo(btn.dataset.id, btn.dataset.set === 'true');
        renderBiblioteca();
    }));
    el.querySelectorAll('.axis-bib-doc-ver').forEach((btn) => btn.addEventListener('click', () => {
        const item = btn.closest('.axis-bib-documento');
        const preview = item.querySelector('.axis-bib-doc-preview');
        const full = item.querySelector('.axis-bib-doc-full');
        const abrindo = full.style.display === 'none';
        full.style.display = abrindo ? 'block' : 'none';
        preview.style.display = abrindo ? 'none' : 'block';
        btn.textContent = abrindo ? 'fechar' : 'ver/editar';
    }));
    el.querySelectorAll('.axis-bib-doc-cancelar').forEach((btn) => btn.addEventListener('click', () => {
        const item = btn.closest('.axis-bib-documento');
        item.querySelector('.axis-bib-doc-full').style.display = 'none';
        item.querySelector('.axis-bib-doc-preview').style.display = 'block';
        const verBtn = item.querySelector('.axis-bib-doc-ver');
        if (verBtn) verBtn.textContent = 'ver/editar';
    }));
    el.querySelectorAll('.axis-bib-doc-salvar').forEach((btn) => btn.addEventListener('click', async () => {
        const item = btn.closest('.axis-bib-documento');
        const nome = item.dataset.arquivo;
        const textarea = item.querySelector('.axis-bib-doc-textarea');
        btn.disabled = true; btn.textContent = 'salvando…';
        try {
            await Biblioteca.editarDocumento(nome, textarea.value);
            renderBiblioteca();
        } catch (e) {
    ...
    el.querySelectorAll('.axis-bib-doc-del').forEach((btn) => btn.addEventListener('click', async () => {
        const item = btn.closest('.axis-bib-documento');
        await Biblioteca.apagarDocumento(item.dataset.arquivo);
        renderBiblioteca();
    }));
    // primeira categoria já abre sozinha — não precisa clicar pra ver o que tem
    const primeiroFolder = el.querySelector('.axis-library-folder');
    if (primeiroFolder) {
        primeiroFolder.classList.add('axis-library-folder-open');
        primeiroFolder.querySelector('.axis-library-folder-body').style.display = 'block';
    }
}
...
const FASE_INGESTAO_LABEL = {
    lendo: 'lendo', lido: 'arquivo lido', descrevendo_imagem: 'descrevendo imagem', imagem_descrita: 'imagem descrita',
    extraindo: 'extraindo fatos', processando_item: 'processando', vazio: 'nada pra segmentar', nada_relevante: 'nada relevante achado',
    concluido: 'concluído', erro: 'erro',
};

// ====================================
// CARTÃO DE INGESTÃO AO VIVO — vitrine na Sala de Pensamento mostrando o
// arquivo sendo processado fase a fase: abrindo, lendo (com prévia),
// extraindo, item por item, concluído. Consome só o onProgresso que a
// Ingestão já expõe (o campo `extra` é aditivo — ver ingerirArquivo, seção
// INGESTÃO) — o pipeline em si não muda nada aqui, isso é vitrine pura.
// ====================================
function elCartaoIngestao() {
    let el = document.getElementById('axis-ingestao-live');
    if (el) return el;
    const journalEl = document.getElementById('axis-journal');
    if (!journalEl?.parentElement) return null;
    el = document.createElement('div');
    el.id = 'axis-ingestao-live';
    el.className = 'axis-ingestao-live';
    el.style.display = 'none';
    journalEl.parentElement.insertBefore(el, journalEl);
    return el;
}
function renderCartaoIngestao({ arquivo, fase, detalhe, extra }) {
    const el = elCartaoIngestao();
    if (!el) return;
    if (fase === 'lendo') el.dataset.itens = ''; // arquivo novo — zera o acumulado do arquivo anterior
    el.style.display = 'block';
    el.dataset.arquivoAtual = arquivo;

    const concluiu = ['concluido', 'erro', 'vazio', 'nada_relevante'].includes(fase);
    const statusClasse = fase === 'erro' ? 'erro' : (concluiu ? 'ok' : 'ativo');

    let corpo = '';
    if (fase === 'lendo') corpo = '<div class="axis-ingestao-linha">📂 abrindo…</div>';
    else if (fase === 'lido') corpo = extra?.preview ? '<div class="axis-ingestao-preview">' + esc(extra.preview) + (extra.preview.length >= 300 ? '…' : '') + '</div>' : '<div class="axis-ingestao-linha">👁️ lendo…</div>';
    else if (fase === 'descrevendo_imagem') corpo = '<div class="axis-ingestao-linha">🖼️ olhando a imagem…</div>';
    else if (fase === 'imagem_descrita') corpo = '<div class="axis-ingestao-preview">' + esc(extra?.preview || '') + '</div>';
    else if (fase === 'extraindo') corpo = '<div class="axis-ingestao-linha">🧠 decidindo o que vale guardar (' + esc(detalhe || '') + ')…</div>';
    else if (fase === 'processando_item') {
        el.dataset.itens = (el.dataset.itens || '') + '<div class="axis-ingestao-item-novo">✓ ' + esc((extra?.fato || detalhe || '').slice(0, 90)) + '</div>';
        corpo = el.dataset.itens;
    } else if (fase === 'vazio' || fase === 'nada_relevante') corpo = '<div class="axis-ingestao-linha axis-ingestao-final">🤷 ' + esc(detalhe || 'nada pareceu valer guardar.') + '</div>';
    else if (fase === 'concluido') corpo = (el.dataset.itens || '') + '<div class="axis-ingestao-linha axis-ingestao-final">✅ ' + esc(detalhe || '') + '</div>';
    else if (fase === 'erro') corpo = '<div class="axis-ingestao-linha axis-ingestao-final">⚠️ ' + esc(detalhe || '') + '</div>';

    el.innerHTML =
        '<div class="axis-ingestao-tab axis-ingestao-tab-' + statusClasse + '"><span class="axis-ingestao-dot"></span>' +
        '<span class="axis-ingestao-nome">' + esc(arquivo) + '</span></div>' +
        '<div class="axis-ingestao-corpo">' + corpo + '</div>';

    if (concluiu) {
        const arquivoFinal = arquivo;
        setTimeout(() => { if (el.dataset.arquivoAtual === arquivoFinal) el.style.display = 'none'; }, 4000);
    }
}

// Ponto de entrada único de upload — Biblioteca e Espaço (mini chat) chamam
// isso, não ingerirArquivos direto: aqui é onde a parte visual (cartão +
// registro no journal) se pluga em cima do onProgresso, sem duplicar em
// dois lugares.
async function ingerirComVisual(files, { statusEl = null } = {}) {
    const resultados = await ingerirArquivos(files, (evento) => {
        renderCartaoIngestao(evento);
        if (statusEl) statusEl.textContent = evento.arquivo + ': ' + (FASE_INGESTAO_LABEL[evento.fase] || evento.fase) + (evento.detalhe ? ' — ' + evento.detalhe : '') + '...';
    });
    for (const r of resultados) {
        const linha = !r.ok ? ('⚠️ ' + r.arquivo + ': ' + r.erro)
            : r.aviso ? ('📎 ' + r.arquivo + ': ' + r.aviso)
            : ('📎 ' + r.arquivo + ': ' + [(r.guardados || 0) + ' novo(s)', r.atualizados ? r.atualizados + ' atualizado(s)' : null, r.ignoradosDuplicata ? r.ignoradosDuplicata + ' duplicata(s) ignorada(s)' : null, r.chunksDocumento ? 'documento em ' + r.chunksDocumento + ' trecho(s)' : null].filter(Boolean).join(', ') + '.');
        await journalAdicionar(linha, 'pensamento', 'ingestao');
    }
    return resultados;
}
async function handleUploadArquivos(files) {
    const statusEl = document.getElementById('axis-biblioteca-status');
    await ingerirComVisual(files, { statusEl });
    renderBiblioteca();
}
...
// ====================================
const TABS = ['espaco', 'tom', 'pensamento', 'biblioteca', 'sistemas', 'config'];
const TAB_TITULO = { espaco: 'Espaço', tom: 'Treino', pensamento: 'Pensamento', biblioteca: 'Biblioteca', sistemas: 'Sistemas', config: 'Config' };
function switchTab(tab) {
    ...
    // O rodapé de envio principal só serve o Espaço; a aba de tom tem o
    // próprio rodapé embutido, o resto não precisa digitar nada.
    ...
    const tituloEl = document.getElementById('axis-topbar-titulo');
    if (tituloEl) tituloEl.textContent = TAB_TITULO[tab] || tab;
    fecharArtefatoViewer();
    if (tab === 'tom') carregarTomHistorico();
...
function atualizarTopbarTitulo() {
    const tabAtual = TABS.find((t) => document.getElementById('axis-tab-' + t)?.style.display === 'flex') || 'espaco';
    const tituloEl = document.getElementById('axis-topbar-titulo');
    if (tituloEl) tituloEl.textContent = TAB_TITULO[tabAtual] || tabAtual;
}

// ====================================
// SIDEBAR — lista de chats, tipo Claude.ai. Cada chat tem memória própria
// de Espaço/Treino (Pensamento não entra aqui — ela é sobre a extensão
// toda, não por chat).
// ====================================
async function renderSidebar() {
    const listaEl = document.getElementById('axis-sidebar-lista');
    if (!listaEl) return;
    const chats = await listarChats();
    listaEl.innerHTML = chats.map((c) =>
        '<div class="axis-sidebar-item' + (c.id === chatAtivoIdCache ? ' axis-ativo' : '') + '" data-id="' + esc(c.id) + '">' +
        '<span class="axis-sidebar-item-nome">' + esc(c.nome) + (c.origemST ? '' : ' ✦') + '</span>' +
        '<button type="button" class="axis-sidebar-item-apagar" data-id="' + esc(c.id) + '" title="Apagar chat">🗑️</button>' +
        '</div>'
    ).join('') || '<div class="axis-empty" style="padding:12px;">Nenhum chat ainda.</div>';

    listaEl.querySelectorAll('.axis-sidebar-item').forEach((item) => item.addEventListener('click', async (ev) => {
        if (ev.target.closest('.axis-sidebar-item-apagar')) return; // o clique de apagar trata sozinho, não troca de chat
        await trocarChat(item.dataset.id);
        fecharSidebar();
        await carregarEspacoHistorico();
        await carregarTomHistorico();
        renderSidebar();
    }));
    listaEl.querySelectorAll('.axis-sidebar-item-apagar').forEach((btn) => btn.addEventListener('click', async (ev) => {
        ev.stopPropagation();
        await apagarChat(btn.dataset.id);
        await carregarEspacoHistorico();
        await carregarTomHistorico();
        renderSidebar();
    }));
...
function abrirSidebar() {
    document.getElementById('axis-sidebar')?.classList.add('axis-aberta');
    document.getElementById('axis-sidebar-overlay')?.classList.add('axis-aberta');
}
function fecharSidebar() {
    document.getElementById('axis-sidebar')?.classList.remove('axis-aberta');
    document.getElementById('axis-sidebar-overlay')?.classList.remove('axis-aberta');
}

// ====================================
// VISUALIZADOR DE ARTEFATO — documento inteiro, painel cheio, tipo Claude.
// A pílula no topo mostra quantos existem; clicar nela lista; clicar num
// item abre o texto inteiro aqui, sem sair da aba onde você tava.
// ====================================
async function atualizarContadorArtefatos() {
    try {
        const r = await chamarSpadeFs('/documento/listar', { personagem: personagemAtual() });
        const el = document.getElementById('axis-topbar-artefatos-count');
        if (el) el.textContent = String(r.documentos?.length || 0);
    } catch (_) { /* silencioso — não é crítico, só o contador */ }
}
async function abrirListaArtefatos() {
    try {
        const r = await chamarSpadeFs('/documento/listar', { personagem: personagemAtual() });
        const documentos = r.documentos || [];
        const corpo = document.getElementById('axis-artefato-viewer-corpo');
        const titulo = document.getElementById('axis-artefato-viewer-titulo');
        titulo.textContent = 'Documentos (' + documentos.length + ')';
        corpo.innerHTML = documentos.length
            ? documentos.map((d) =>
                '<div class="axis-artefato-card" data-slug="' + esc(d.slug) + '">' +
                '<div class="axis-artefato-icone">📄</div>' +
                '<div class="axis-artefato-info"><div class="axis-artefato-titulo">' + esc(d.titulo) + '</div>' +
                '<div class="axis-artefato-meta">' + esc(d.categoria) + ' · ' + formatarRelativo(new Date(d.atualizadoEm || Date.now()).getTime()) + '</div></div></div>'
            ).join('')
            : '<div class="axis-empty">Nenhum documento ainda — vai aparecer aqui assim que ela guardar algo no Treino.</div>';
        corpo.querySelectorAll('.axis-artefato-card').forEach((card) => card.addEventListener('click', () => abrirArtefato(card.dataset.slug)));
        document.getElementById('axis-artefato-viewer').classList.remove('axis-hidden');
    } catch (e) { reportFatalError(e); }
}
async function abrirArtefato(slug) {
    try {
        const r = await chamarSpadeFs('/documento/ler', { personagem: personagemAtual(), slug });
        document.getElementById('axis-artefato-viewer-titulo').textContent = r.titulo || slug;
        document.getElementById('axis-artefato-viewer-corpo').textContent = r.conteudo || '';
        document.getElementById('axis-artefato-viewer').classList.remove('axis-hidden');
    } catch (e) { reportFatalError(e); }
}
function fecharArtefatoViewer() {
    document.getElementById('axis-artefato-viewer')?.classList.add('axis-hidden');
}
...
function createPanel() {
    ...
    panel.innerHTML =
        '<div class="axis-topbar">' +
        '<button type="button" class="axis-topbar-icone" id="axis-sidebar-abrir" title="Chats">☰</button>' +
        '<span class="axis-topbar-titulo" id="axis-topbar-titulo">Espaço</span>' +
        ...
        '</div>' +
        '<button type="button" class="axis-topbar-icone" id="axis-topbar-novo-chat" title="Novo chat">＋</button>' +
        '<button type="button" class="axis-pilula-artefatos" id="axis-topbar-artefatos" title="Documentos">📄 <span id="axis-topbar-artefatos-count">0</span></button>' +
        '<button type="button" class="axis-btn axis-btn-close" id="axis-espaco-close">✕</button>' +
        '</div>' +
        '<div class="axis-sidebar-overlay" id="axis-sidebar-overlay"></div>' +
        '<div class="axis-sidebar" id="axis-sidebar">' +
        '<div class="axis-sidebar-header"><button type="button" class="axis-sidebar-novo" id="axis-sidebar-novo-chat">＋ Novo chat</button></div>' +
        '<div class="axis-sidebar-lista" id="axis-sidebar-lista"></div>' +
        '</div>' +
        ...
        '<div id="axis-tab-espaco" class="axis-espaco-body" style="display:flex;flex-direction:column;">' +
        '<div class="axis-espaco-chat" id="axis-espaco-chat"></div>' +
        '<button type="button" class="axis-pilula-modo" id="axis-pilula-treino">🎭 Treino <span class="axis-pilula-modo-x">— documentos de referência</span></button>' +
        '</div>' +

        '<div id="axis-tab-tom" class="axis-espaco-body" style="display:none;flex-direction:column;">' +
        '<div style="padding:6px 12px;border-bottom:1px solid var(--axis-borda);display:flex;align-items:center;gap:8px;">' +
        '<button type="button" class="axis-pilula-modo" id="axis-pilula-sair-treino" style="margin:0;">← Espaço</button>' +
        '<select id="axis-tom-folha-select" style="flex:1;background:var(--axis-bg-bolha);color:var(--axis-texto);border:1px solid var(--axis-borda);border-radius:8px;padding:5px 6px;font-size:12px;"><option value="">+ novo documento</option></select>' +
        '</div>' +
        '<div class="axis-documento-split">' +
        '<div class="axis-documento-chat" id="axis-tom-chat"></div>' +
        '<div class="axis-documento-folha-panel">' +
        '<div class="axis-documento-folha-header" id="axis-tom-folha-titulo">novo documento</div>' +
        '<div class="axis-documento-folha-corpo" id="axis-tom-folha-corpo"></div>' +
        '<div class="axis-documento-folha-vazio" id="axis-tom-folha-vazio">o documento aparece aqui, ao vivo, enquanto ela escreve</div>' +
        '</div></div>' +
        '<div class="axis-espaco-footer">' +
        '<input type="file" id="axis-tom-upload" multiple accept=".txt,.md,.markdown,.json,.csv,.tsv,.yaml,.yml,.log" style="display:none;">' +
        '<button type="button" class="axis-btn axis-btn-attach" id="axis-tom-attach-btn" title="Anexar arquivo de referência">📎</button>' +
        '<div class="axis-espaco-input-wrap">' +
        '<textarea class="axis-espaco-input" id="axis-tom-input" placeholder="Cola um exemplo, um trecho de lore, uma regra..." rows="1"></textarea>' +
        '<button type="button" class="axis-btn axis-btn-send" id="axis-tom-send">Enviar</button>' +
        '</div></div></div>' +

        '<div id="axis-tab-pensamento" class="axis-espaco-body" style="display:none;">' +
        '<div class="axis-espaco-chat" id="axis-journal" style="flex:1;"></div></div>' +

        '<div id="axis-tab-biblioteca" class="axis-espaco-body" style="display:none;flex-direction:column;">' +
        '<div style="padding:8px 12px;border-bottom:1px solid var(--axis-borda);">' +
        '<input type="file" id="axis-biblioteca-upload" multiple accept=".txt,.md,.markdown,.json,.csv,.tsv,.yaml,.yml,.log,image/*" style="color:var(--axis-texto-fraco);font-size:12px;">' +
        '<div style="font-size:10px;color:var(--axis-texto-fraco2);margin-top:3px;">texto, PNG de character card ou foto — a IA decide o que vale guardar e pra que serve</div>' +
        '<div id="axis-biblioteca-status" style="font-size:11px;color:var(--axis-texto-fraco);margin-top:4px;"></div>' +
        '<div id="axis-biblioteca-filtros" style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap;"></div></div>' +
        ...
        '<div style="padding:8px 12px;border-top:1px solid var(--axis-borda);border-bottom:1px solid var(--axis-borda);display:flex;align-items:center;gap:8px;flex-wrap:wrap;">' +
        '<span style="font-size:11px;color:var(--axis-texto-fraco);">Sistemas reais (código):</span>' +
        ...
        '</div>' +
        '<div id="axis-sistemas-reais-status" style="font-size:11px;color:var(--axis-texto-fraco);padding:4px 12px;"></div>' +
        ...
        '<div class="axis-artefato-viewer axis-hidden" id="axis-artefato-viewer">' +
        '<div class="axis-artefato-viewer-header">' +
        '<button type="button" class="axis-topbar-icone" id="axis-artefato-viewer-fechar">←</button>' +
        '<span class="axis-artefato-viewer-titulo" id="axis-artefato-viewer-titulo"></span>' +
        '</div>' +
        '<div class="axis-artefato-viewer-corpo" id="axis-artefato-viewer-corpo"></div>' +
        '</div>' +
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        if (panel.classList.contains('axis-visible')) { carregarEspacoHistorico(); switchTab('espaco'); renderSidebar(); atualizarContadorArtefatos(); }
    });
    ...
    // Nem toda aba tem botão na mini-barra (Treino agora é só pílula) —
    // sem essa checagem, addEventListener em elemento inexistente quebra
    // o createPanel inteiro.
    TABS.forEach((t) => document.getElementById('axis-mini-' + t)?.addEventListener('click', () => switchTab(t)));

    document.getElementById('axis-sidebar-abrir').addEventListener('click', () => { renderSidebar(); abrirSidebar(); });
    document.getElementById('axis-sidebar-overlay').addEventListener('click', fecharSidebar);
    async function novoChatEClicaEspaco() {
        await criarNovoChat();
        fecharSidebar();
        switchTab('espaco');
        await carregarEspacoHistorico();
    ...
    document.getElementById('axis-topbar-novo-chat').addEventListener('click', novoChatEClicaEspaco);
    document.getElementById('axis-sidebar-novo-chat').addEventListener('click', novoChatEClicaEspaco);
    document.getElementById('axis-topbar-artefatos').addEventListener('click', abrirListaArtefatos);
    document.getElementById('axis-artefato-viewer-fechar').addEventListener('click', fecharArtefatoViewer);
    document.getElementById('axis-pilula-treino').addEventListener('click', () => switchTab('tom'));
    document.getElementById('axis-pilula-sair-treino').addEventListener('click', () => switchTab('espaco'));
    ...
    document.getElementById('axis-tom-attach-btn').addEventListener('click', () => {
        document.getElementById('axis-tom-upload').click();
    });
    document.getElementById('axis-tom-upload').addEventListener('change', (e) => {
        const files = Array.from(e.target.files || []);
        if (files.length) handleUploadTom(files).catch(reportFatalError);
        e.target.value = '';
    });
    document.getElementById('axis-tom-folha-select').addEventListener('change', (e) => {
        carregarDocumentoSelecionado(e.target.value).catch(reportFatalError);
    });
    ...
    const tomInputEl = document.getElementById('axis-tom-input');
    const tomSendBtn = document.getElementById('axis-tom-send');
    tomInputEl.addEventListener('input', () => comecarAtividade('tom'));
    function doSendTom() {
        const text = tomInputEl.value.trim();
        if (!text) return;
        tomInputEl.value = '';
        sendTomMessage(text).catch(reportFatalError);
    }
    tomSendBtn.addEventListener('click', doSendTom);
    tomInputEl.addEventListener('keydown', (e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); doSendTom(); } });
}
...
eventSource.on(event_types.APP_READY, () => {
    try { registrarChatsSeNecessario().then(() => { createPanel(); renderSidebar(); }); wireRpPresence(); garantirTicker(); loopPensamento(); } catch (e) { reportFatalError(e); }
});
eventSource.on(event_types.CHAT_CHANGED, () => {
    try { registrarChatsSeNecessario().then(() => { renderEspacoChat(); renderSidebar(); atualizarTopbarTitulo(); }); wireRpPresence(); } catch (e) { reportFatalError(e); }
});

registrarChatsSeNecessario().then(() => { createPanel(); renderSidebar(); });
wireRpPresence();
garantirTicker();
