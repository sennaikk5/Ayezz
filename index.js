...
async function escreverArtefato({ slug, titulo, categoria, tags, conteudo, escopo = 'chat' }) {
    ...
    const r = await chamarSpadeFs('/documento/escrever', { personagem: personagemAtual(), slug: slugFinal, titulo, categoria: categoriaFinal, tags: tags || [], conteudo, chatId });
...
async function runCantos(userMessage) {
    ...
    const [bibliotecaResult, repeticaoResult, estadoResult, mundoResult, sistemasResult, sistemasReaisResult, espacoRecenteResult] = await Promise.allSettled([
...
async function buildSystemPrompt(cantos) {
    ...
    const parts = [
        ...
        'engordar a fala com narração ou explicação que ninguém pediu. Se ficar em dúvida entre uma versão mais ' +
        'longa e uma mais curta da mesma fala, escolha a curta.' +
        (cantos.espacoRecente?.value ? ' Se a conversa recente aí em cima pediu algo específico de ritmo/tom/comportamento, é a última checagem antes de falar — não um detalhe do começo do prompt pra esquecer.' : ''),
    ];
...
async function executeTool(name, args) {
    switch (name) {
        ...
        case 'artefato_escrever': {
            const r = await escreverArtefato({ slug: args.slug, titulo: args.titulo, categoria: args.categoria || 'documento', tags: args.tags || [], conteudo: args.conteudo, escopo: args.escopo || 'chat' });
...
let opToken = 0;
// FIX (a raiz do "duas IAs"): digitar na caixa nativa do RP (send_textarea)
// chamava comecarAtividade('rp') em toda tecla — e comecarAtividade ABORTA
// a atividade atual quando troca de "onde". Se o Espaço estava no meio de
// processar um pedido (ex: "manda uma mensagem"), só o usuário TOCAR na
// caixa de digitar do RP — sem enviar nada, só focar/digitar — matava esse
// processamento no meio, silenciosamente (o controller.signal.aborted vira
// true, passadaComFerramentas para no próximo `break`). Era isso que fazia
// o pedido morrer "durante" e só ir "depois" — na segunda tentativa, se o
// usuário não estava mais tocando na caixa do RP nesse meio-tempo, nada
// abortava o processamento e ele completava. Duas fontes disputando o
// mesmo slot de atividade por um sinal que devia ser só informativo.
// `usuarioDigitandoRp` separa isso: é só um aviso leve pro Pensamento não
// começar algo NOVO enquanto o usuário compõe a própria fala — nunca
// aborta nada que já esteja rodando de verdade (Espaço, RP, Pensamento).
// Some sozinho depois de alguns segundos sem tecla nova, ou quando o campo
// fica vazio, então nunca fica pendurado se o usuário digitar e desistir.
...
// FIX: lastCharMessageIdx só era atualizado dentro de finalizeCharMessage —
// ou seja, só quando a PRÓPRIA Spade posta a fala (postCharacterMessage-
// Streaming). Um regenerate/swipe nativo do ST (que Spade pula de propósito,
// ver Spade_interceptGeneration) troca a última fala sem passar por ali —
// lastCharMessageIdx ficava apontando pra uma fala antiga, e "editar minha
// última fala" (chat_agir sem idx) editava/apagava a coisa errada. Isso
// resincroniza com o índice REAL toda vez que uma mensagem é desenhada —
// cobre swipe e regenerate também, não só o caminho próprio da Spade.
...
async function respondEspaco(userMessage, chatId) {
    ...
    const systemPrompt =
        ...
        'AJUSTE NA HORA — não precisa fazer nada especial pra isso valer. Essa conversa (as últimas mensagens ' +
        'daqui) é lida direto na hora de gerar o RP, sempre, sem busca e sem prazo — não tem flag pra marcar nem ' +
        'ferramenta especial pra chamar: Espaço e RP são a MESMA cabeça, só em lugares diferentes, e o que o ' +
        'usuário pede aqui ("fala mais desse jeito", "ela devia ficar com ciúme disso", ritmo, o que for) já ' +
        'vale a partir da próxima fala dela só por ter sido dito. Pode confirmar pro usuário que já vale.\n\n' +
        'Se for algo que precisa sobreviver ALÉM dessa conversa (um traço central da personagem, não só o assunto ' +
        'de agora — porque a janela de conversa recente é limitada, ela não volta no tempo pra sempre), aí sim ' +
        'escreva como Artefato normal (artefato_escrever, sem flag nenhuma) pra durar. Isso é sobre durabilidade, ' +
        'não sobre fazer a instrução funcionar — ela já funciona.\n\n' +
...
// enquanto.
// ====================================
// FIX: token da atividade 'rp' que essa função REALMENTE assumiu (vai
// chamar terminarAtividade sozinha, sempre, no finally abaixo — sucesso ou
// erro). Existe só pra distinguir isso de uma atividade 'rp' ÓRFÃ: quando
// `type` vem preenchido (regenerate/swipe/impersonate — ela pula de
// propósito, ver comentário abaixo), o listener global de GENERATION_STARTED
// já marcou ocupado antes de chegar aqui, mas ninguém nunca ia desmarcar —
// activityBusy() ficava travado em true PRA SEMPRE depois de um regenerate,
// e só uma mensagem nova em QUALQUER lugar (Espaço, RP) sobrescrevia e
// destravava por acidente. É exatamente o "trava e só volta se eu mandar
// qualquer mensagem" relatado. Ver fecharAtividadeRpOrfa mais abaixo, ligada
// em GENERATION_STOPPED/GENERATION_ENDED — só fecha quando o token NÃO bate
// com o que essa função adotou, então nunca mexe numa geração de verdade em
// andamento (o streaming custom abaixo pode levar vários segundos).
let tokenRpAdotadoPeloIntercept = null;
async function Spade_interceptGeneration(chatArr, contextSize, abort, type) {
    // Gap conhecido (herdado, decisão consciente): só assume geração em
    // type == null/undefined — regenerate/swipe ficam pro ST nativo.
    if (type) return;
    const atividade = comecarAtividade('rp');
    tokenRpAdotadoPeloIntercept = atividade.token;
    try {
        ...
        // FIX: antes, um erro aqui (rede caiu no meio do streaming custom,
        // por exemplo) ia direto pro catch sem passar por terminarAtividade
        // — a atividade 'rp' ficava pendurada pra sempre, exatamente como o
        // caso do regenerate acima, só que por um erro de verdade em vez de
        // um tipo pulado de propósito. "Erro em segundo plano trava tudo até
        // mandar mensagem nova" tinha essas DUAS causas raiz — essa e a
        // órfã do regenerate — não fraqueza de prompt.
        reportFatalError(e);
    } finally {
        terminarAtividade(atividade);
...
// FIX: fecha uma atividade 'rp' que ficou órfã — GENERATION_STARTED marcou
// ocupado (linha lá embaixo, dispara pra QUALQUER tipo de geração), mas
// Spade_interceptGeneration pulou essa (regenerate/swipe/impersonate) e
// nunca chegou a adotar/fechar. Guardado pelo token: só fecha quando NINGUÉM
// reivindicou essa atividade de verdade — se o intercept tá no meio de uma
// geração de verdade (token bate com tokenRpAdotadoPeloIntercept), isso não
// toca em nada, mesmo que o evento dispare enquanto o streaming custom ainda
// tá rodando.
function fecharAtividadeRpOrfa() {
    if (atividadeAtual && atividadeAtual.onde === 'rp' && atividadeAtual.token !== tokenRpAdotadoPeloIntercept) {
        terminarAtividade(atividadeAtual);
    }
}
...
async function resolverEspacoChatAtivo() {
    ...
    // FIX: só cai pro "mais recente" quando NEM SABEMOS qual é o chat do ST
    // agora (stId nulo — sem contexto nenhum pra decidir). Se SABEMOS o stId
    // e ele não bate com nenhum Espaço já linkado, isso é uma conversa de RP
    // que ainda não tem Espaço próprio — precisa começar um rascunho NOVO
    // linkado a ELA, nunca reusar por engano o Espaço de outra conversa só
    // porque foi o mexido mais recente. Era exatamente isso que quebrava o
    // "um só": a mensagem virava rascunho na conversa errada, com um
    // stChatId que não é o do RP em teste — na hora de gerar, compiledEspaco-
    // RecenteBlock procura por stChatId EXATO e não acha nada (retorna ''),
    // e o ajuste dito no Espaço nunca aparece na fala dela.
...
async function garantirEspacoChatEscreve(primeiraMensagem) {
    ...
    if (espacoChatIdAtual) {
        ...
        // FIX: só reusa o rascunho/chat já ativo se ele bate de verdade com
        // o chat REAL do ST agora — sem essa checagem, qualquer dessincronia
        // (extensão recarregada na MESMA conversa sem CHAT_CHANGED disparar,
        // troca de chat que por algum motivo não reprocessou a tempo) fazia
        // a mensagem nova entrar num Espaço com stChatId errado — e ela
        // nunca era achada de volta na hora de montar o prompt do RP
        // (compiledEspacoRecenteBlock procura por stChatId EXATO). Isso é o
        // que fazia o "231"/ajuste pedido no Espaço nunca aparecer.
        if (atual && (!stIdReal || atual.stChatId === stIdReal)) return espacoChatIdAtual;
        espacoChatIdAtual = null;
        espacoChatStPendente = stIdReal;
    }
    // FIX: nunca confia só em espacoChatStPendente pra decidir o vínculo —
    // essa variável só é populada quando resolverEspacoChatAtivo roda antes
    // (CHAT_CHANGED, ou abrir o painel), e podia nunca ter rodado ainda
    // (ex: página recarregada na MESMA conversa, sem trocar de chat). Pega
    // o stChatId real agora, no exato momento de criar o vínculo — é o que
    // de fato importa pra essa conversa ser achada de volta depois.
...
eventSource.on(event_types.GENERATION_STOPPED, () => { try { dotsIdle(); fecharAtividadeRpOrfa(); } catch (e) { reportFatalError(e); } });
eventSource.on(event_types.GENERATION_ENDED, () => { try { dotsIdle(); fecharAtividadeRpOrfa(); } catch (e) { reportFatalError(e); } });

eventSource.on(event_types.APP_READY, () => {
    // FIX: faltava resolver o vínculo do Espaço já aqui — antes só rodava em
    // CHAT_CHANGED ou ao clicar pra abrir o painel. Recarregar a extensão
    // SEM trocar de chat (o caso mais comum ao testar um fix) deixava
    // espacoChatIdAtual/espacoChatStPendente nos valores default (null) até
    // algum desses dois gatilhos disparar — a proteção nova em
    // garantirEspacoChatEscreve já cobre isso na escrita, mas resolver aqui
    // também evita depender só dela.
    try { createPanel(); wireRpPresence(); garantirTicker(); loopPensamento(); abrirEspacoNoContextoAtual().catch(reportFatalError); } catch (e) { reportFatalError(e); }
});
