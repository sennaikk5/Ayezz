...
const Biblioteca = {
    ...
    async escrever({ tipo, texto, metadata = {}, chaveFixa = null, sabemDisso = null, importancia = 0.5, entidades = [], substitui = null, personagem: personagemOverride = null }) {
...
const SistemasReais = {
    ...
    // 4.1 — serializa TODAS as versões (todos os hooks) do personagem atual
    // num manifesto único e dispara o download como .json. Não seleciona
    // nada: é sempre "tudo que existe agora", mesmo escopo de listar().
    async exportar() {
        const sistemas = await this.listar();
        const manifesto = {
            formato: 'spade-sistemas-reais',
            versaoManifesto: 1,
            versaoSDK: SDK_VERSAO,
            personagem: personagemAtual(),
            exportadoEm: Date.now(),
            sistemas: sistemas.map((s) => ({
                familia: s.familia, nome: s.nome, versao: s.versao,
                hookAlvo: s.hookAlvo, codigo: s.codigo, ativo: s.ativo, autor: s.autor,
            })),
        };
        ...
        a.href = url;
        a.download = 'spade-sistemas-' + personagemAtual() + '-' + manifesto.exportadoEm + '.json';
    ...
    // 4.2 — valida schema mínimo, nunca sobrescreve família local (colisão
    // vira sufixo '-importado-'+timestamp), e reusa publicar() pra cada
    // sistema — que já roda o sandbox 1x e só salva se passar (mesma
    // garantia que testar() isolado daria, sem rodar o Worker duas vezes
    // pro mesmo código). Nunca lança — item ruim vira { aceito:false, erro }
    // na lista, o resto do arquivo continua sendo processado.
    async importar(jsonTextoOuObjeto) {
        let dados;
        try {
            dados = typeof jsonTextoOuObjeto === 'string' ? JSON.parse(jsonTextoOuObjeto) : jsonTextoOuObjeto;
        } catch (e) {
            return { ok: false, erro: 'JSON inválido: ' + e.message, resultados: [] };
        }
        const lista = Array.isArray(dados) ? dados : (Array.isArray(dados?.sistemas) ? dados.sistemas : null);
        if (!lista) return { ok: false, erro: 'formato não reconhecido — esperava um manifesto com "sistemas" ou uma lista direta.', resultados: [] };

        const familiasLocais = new Set((await this.listar()).map((s) => s.familia));
        const timestamp = Date.now();
        const resultados = [];

        for (const item of lista) {
            if (!item || !item.familia || !item.codigo || !item.hookAlvo) {
                resultados.push({ familiaOriginal: item?.familia ?? null, aceito: false, renomeado: false, erro: 'schema mínimo ausente (familia/codigo/hookAlvo).' });
                continue;
            }
            let familiaFinal = item.familia;
            const renomeado = familiasLocais.has(familiaFinal);
            if (renomeado) familiaFinal = familiaFinal + '-importado-' + timestamp;
            const entradaExemplo = item.hookAlvo === 'antesDeGerar' ? { sceneText: '' } : {};
            const publicado = await this.publicar({
                familia: familiaFinal, nome: item.nome || familiaFinal, hookAlvo: item.hookAlvo,
                codigo: item.codigo, entradaExemplo, autor: { ia: 'importado', aprovadoPor: null },
            });
            familiasLocais.add(familiaFinal); // evita colisão entre itens do mesmo arquivo
            resultados.push({ familiaOriginal: item.familia, familiaFinal, renomeado, aceito: publicado.salvo, erro: publicado.erro || null });
        }
...
function createPanel() {
    ...
    panel.innerHTML =
        ...
        '<div style="padding:8px 12px;border-bottom:1px solid #1a1a3a;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">' +
        '<button type="button" class="axis-btn axis-btn-sm" id="axis-sistemas-export">exportar sistemas reais</button>' +
        '<label class="axis-btn axis-btn-sm" style="cursor:pointer;margin:0;">importar<input type="file" id="axis-sistemas-import" accept=".json" style="display:none;"></label>' +
        '<span id="axis-sistemas-import-status" style="font-size:11px;color:#6a6a8a;"></span></div>' +
    ...
    document.getElementById('axis-sistemas-export').addEventListener('click', () => {
        SistemasReais.exportar().catch(reportFatalError);
    });
    document.getElementById('axis-sistemas-import').addEventListener('change', (e) => {
        const file = (e.target.files || [])[0];
        e.target.value = '';
        if (!file) return;
        const statusEl = document.getElementById('axis-sistemas-import-status');
        if (statusEl) statusEl.textContent = 'importando ' + file.name + '...';
        file.text().then((texto) => SistemasReais.importar(texto)).then((r) => {
            if (!statusEl) return;
            if (!r.ok) { statusEl.textContent = '⚠️ ' + r.erro; return; }
            const aceitos = r.resultados.filter((x) => x.aceito).length;
            const renomeados = r.resultados.filter((x) => x.renomeado && x.aceito).length;
            const falhas = r.resultados.length - aceitos;
            statusEl.textContent = aceitos + ' importado(s)' +
                (renomeados ? ' (' + renomeados + ' renomeado(s) por colisão)' : '') +
                (falhas ? ', ' + falhas + ' falhou(aram)' : '') + '.';
