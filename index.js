...
//
// Retry: generate/generateWithTools já tinham (comRetentativas) — aqui não
// tinha, e streaming era o único caminho sem essa rede de segurança (era
// literalmente a fala da personagem no RP, a coisa mais visível de todas).
// Só reenvia se NENHUM token chegou ainda nessa tentativa — depois que
// onToken já foi chamado uma vez, reenviar duplicaria texto na tela, então
// nesse caso desiste e deixa "do jeito que parou" cuidar do resto.
async function generateStream(model, messages, onToken, opts = {}) {
    ...
    for (let tentativa = 0; tentativa < tentativasMax; tentativa++) {
        let tokenChegou = false;
        const watchdogController = new AbortController();
        let watchdog = setTimeout(() => watchdogController.abort(), timeoutMs);
        const resetWatchdog = () => { clearTimeout(watchdog); watchdog = setTimeout(() => watchdogController.abort(), timeoutMs); };
        ...
        try {
            ...
            while (true) {
                ...
                for (const line of lines) {
                    ...
                    if (delta) { full += delta; tokenChegou = true; onToken(delta); }
                }
            }
            return full;
        } catch (err) {
            ultimoErro = (watchdogController.signal.aborted && !opts.signal?.aborted)
                ? new Error('Sem resposta da NanoGPT por ' + Math.round(timeoutMs / 1000) + 's — rede travada (não foi você interrompendo).')
                : err;
            if (opts.signal?.aborted) throw ultimoErro; // uma atividade nova assumiu — não insiste, não é soluço de rede
            if (tokenChegou) throw ultimoErro; // já mostrou algo — reenviar duplicaria
            if (tentativa < tentativasMax - 1) await new Promise((r) => setTimeout(r, 1200 * (tentativa + 1)));
        } finally {
...
const TOOLS = [
    ...
    { type: 'function', function: { name: 'redirecionar_biblioteca', description: 'Só existe no Espaço. Abre a aba de Biblioteca — use quando o usuário pedir pra treinar seu tom de voz, personalidade, perfil, ou mandar arquivo/foto de referência pra guardar. Isso muda a aba visível pro usuário, tira a conversa do Espaço normal.', parameters: { type: 'object', properties: {} } } },
...
// ====================================
// FALLBACK DE TOOL CALL EM TEXTO — alguns modelos (via NanoGPT), quando a
// coisa aperta, imitam o formato de tool-use da Anthropic como TEXTO
// solto (<invoke name="..."><parameter name="...">valor</parameter></invoke>)
// em vez de preencher `tool_calls` de verdade. Sem isso, essa tentativa de
// chamar ferramenta simplesmente não executava NADA — só aparecia como
// bolha de texto quebrada no Espaço/Pensamento, e a IA ficava travada
// esperando alguém "confirmar" uma coisa que não existe (era exatamente o
// "digita no site e espera eu responder" das fotos). Agora: se o modelo
// devolveu conteúdo com esse padrão e NÃO populou tool_calls, extrai e
// executa do mesmo jeito que uma tool call de verdade, e tira o bloco cru
// do texto mostrado pro usuário.
// ====================================
function extrairInvokesDoTexto(texto) {
    if (!texto || texto.indexOf('<invoke') === -1) return { limpo: texto, chamadas: [] };
    const chamadas = [];
    const reInvoke = /<invoke\s+name="([^"]+)">([\s\S]*?)<\/invoke>/g;
    const reParam = /<parameter\s+name="([^"]+)">([\s\S]*?)<\/parameter>/g;
    let m;
    while ((m = reInvoke.exec(texto))) {
        const nome = m[1];
        const corpo = m[2];
        const args = {};
        let p;
        reParam.lastIndex = 0;
        while ((p = reParam.exec(corpo))) args[p[1]] = p[2];
        chamadas.push({ id: 'fallback_' + newId(), function: { name: nome, arguments: JSON.stringify(args) } });
    }
    const limpo = texto.replace(reInvoke, '').trim();
...
async function passoConstrutora(atividade, instrucao) {
    ...
    let conteudo = resp.content;
    if (!toolCalls?.length && conteudo) {
        const extraido = extrairInvokesDoTexto(conteudo);
        if (extraido.chamadas.length) { toolCalls = extraido.chamadas; conteudo = extraido.limpo; }
    }

    if (conteudo) await journalAdicionar(conteudo, 'pensamento', 'construtora');
    ...
    const novoHistorico = [...historico,
        { role: 'assistant', content: conteudo || null, ...(toolCalls?.length ? { tool_calls: toolCalls } : {}) },
        ...(toolCalls?.length ? toolCalls.map((call, i) => ({ role: 'tool', tool_call_id: call.id, content: JSON.stringify(resultadosTools[i]?.resultado ?? {}) })) : []),
    ].slice(-60);
    await estadoSet('construtoraHistory', novoHistorico);

    return { narracao: conteudo || null, tools: resultadosTools };
}
...
function garantirTicker() {
    ...
    bar.innerHTML =
        ...
        '<span class="axis-alive-badge axis-alive-directive" id="axis-ticker-texto"></span>';
...
async function passadaComFerramentas(messages, atividade, { onNarracao } = {}, opts = {}) {
    ...
    for (let i = 0; i < teto; i++) {
        ...
        // Modelo tentou chamar ferramenta como texto em vez de tool_calls
        // de verdade — recupera em vez de deixar a bolha quebrada aparecer.
        let toolCalls = resp.toolCalls;
        let conteudo = resp.content;
        if (!toolCalls?.length && conteudo) {
            const extraido = extrairInvokesDoTexto(conteudo);
        ...
        if (conteudo && onNarracao) { try { await onNarracao(conteudo); } catch (_) {} }

        if (!toolCalls?.length) { respostaFinal = conteudo || ''; break; }

        messages.push({ role: 'assistant', content: conteudo || null, tool_calls: toolCalls });
        if (conteudo) await sleep(2000); // pausa de apresentação — só isso, não é limite técnico
    ...
    return { respostaFinal, pausaSegundos, redirecionarBiblioteca, toolsChamadas };
}
...
async function respondEspaco(userMessage) {
    ...
    let respostaFinal = '', erro = null, redirecionarBiblioteca = false;
    try {
    ...
    dotsIdle();
    return { resposta: respostaFinal, erro, abandoned: isStale(atividade.token), redirecionarBiblioteca };
}
...
async function Spade_interceptGeneration(chatArr, contextSize, abort, type) {
    try {
        ...
        terminarAtividade(atividade);
        if (!outcome.ok) await journalAdicionar('⚠️ não consegui responder no RP: ' + (outcome.reason || 'erro desconhecido'), 'pensamento');
    } catch (e) {
        reportFatalError(e);
    }
}
...
function renderConfigForm() {
    ...
    document.getElementById('cfg-salvar').addEventListener('click', (ev) => {
        setConfig({
        ...
        // Confirmação fica só no botão — o ticker do topo é território
        // exclusivo da narração da própria IA, nada de aviso de sistema ali.
...
// ====================================
const TABS = ['espaco', 'pensamento', 'biblioteca', 'sistemas', 'config'];
function switchTab(tab) {
    ...
    // O rodapé de envio principal só serve o Espaço
