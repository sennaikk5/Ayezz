...
async function renderJournalEm(elId, contexto, vazioTexto) {
    ...
    el.innerHTML = entradas.slice(-60).map((e) =>
        '<div class="axis-rambling-entry' + (e.autor === 'construtora' ? ' axis-rambling-construtora' : e.autor === 'ingestao' ? ' axis-rambling-ingestao' : '') + '">' +
        '<div class="axis-rambling-meta">' + new Date(e.ts).toLocaleTimeString('pt-BR') + (e.autor === 'construtora' ? ' · 🔧 Construtora' : e.autor === 'ingestao' ? ' · 📎 Ingestão' : '') + '</div>' +
...
async function respondEspaco(userMessage, chatId) {
    ...
    let respostaFinal = '', erro = null, redirecionarBiblioteca = false, redirecionarTom = false;
    try {
    ...
    dotsIdle();
    return { resposta: respostaFinal, erro, abandoned: isStale(atividade.token), redirecionarBiblioteca, redirecionarTom };
}
...
function renderTomChat() {
    ...
    if (!chatEl) return;
    chatEl.innerHTML = tomLocalLog.map((m) =>
        '<div class="axis-msg axis-msg-' + (m.role === 'user' ? 'user' : 'agent') + '">' + renderMarkers(m.display || m.content) + '</div>'
    ).join('') || '<div class="axis-empty">Manda um exemplo de fala dela, um trecho de lore, uma regra do mundo — o que for material de referência pro RP — colado ou por arquivo.</div>';
    chatEl.scrollTop = chatEl.scrollHeight;
}
...
async function carregarTomHistorico() {
    const hist = await estadoGet('tomHistory', []);
...
async function respondTom(userMessage) {
    ...
    dotsThinking();
    const charName = personagemAtual();
    const history = await estadoGet('tomHistory', []);
    ...
    if (!erro) {
        const historicoAgora = await estadoGet('tomHistory', []);
        await estadoSet('tomHistory', [...historicoAgora, { role: 'user', content: userMessage }, { role: 'assistant', content: respostaFinal }].slice(-80));
    }
    dotsIdle();
...
async function sendTomMessage(text, displayText, tentativa = 1) {
    ...
    try {
        ...
        if (result.erro) {
            ...
            tomLocalLog.push({ role: 'agent', content: result.resposta || '(sem resposta)' });
        }
        renderTomChat();
        renderListaDocumentos();
    } finally {
...
function createPanel() {
    ...
    panel.innerHTML =
        ...
        '<span class="axis-mini-tab" id="axis-mini-tom">Treino</span>' +
        ...
        '<div style="padding:6px 12px;border-bottom:1px solid #1a1a3a;display:flex;align-items:center;gap:8px;">' +
        '<select id="axis-tom-folha-select" style="flex:1;background:#12122c;color:#c8c8e8;border:1px solid #2a2a5a;border-radius:6px;padding:4px 6px;font-size:12px;"><option value="">+ novo documento</option></select>' +
        '</div>' +
        ...
        '<div style="font-size:10px;color:#5a5a7a;margin-top:3px;">texto, PNG de character card ou foto — a IA decide o que vale guardar e pra que serve</div>' +
        '<div id="axis-biblioteca-status" style="font-size:11px;color:#6a6a8a;margin-top:4px;"></div>' +
