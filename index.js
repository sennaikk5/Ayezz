...
async function mesclarEmArtefato(categoria, fatosNovos, contextoDocumento) {
    ...
    let corpo;
    try {
        corpo = (await generate(modeloRapido, [{ role: 'user', content: prompt }], { maxTokens: 900, temperature: 0.4, timeoutMs: 20000 })).trim();
    } catch (e) {
...
async function pensarUmaVez() {
    ...
    try {
        ...
        const systemPrompt =
            ...
            'reage, o que evita, o ritmo dele — e guardar isso num Artefato (categoria "usuario", via ' +
            'artefato_escrever — se já existe um, leia com artefato_ler antes e reescreva completo) se valer a pena ' +
            'lembrar depois. Você pode reparar também em como VOCÊ MESMA anda usando a extensão — que sistema ' +
            'ajudou, o que não funcionou — e ajustar.\n\n' +
