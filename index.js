...
function getApiConfig() {
    ...
    if (!cm) throw new Error('Connection Manager nao encontrado');

    const profileId = cm.selectedProfile;
    if (!profileId) throw new Error('Nenhum perfil de conexao selecionado');

    const profile = cm.profiles?.find(p => p.id === profileId);
    if (!profile) throw new Error('Perfil de conexao nao encontrado');

    const apiMap = ctx.CONNECT_API_MAP?.[profile.api];
    if (!apiMap) throw new Error(`API desconhecida: ${profile.api}`);

    const proxyPreset = (ctx.proxies || []).find(p => p.name === profile.proxy);

    return {
        profile,
        api: profile.api,
        model: profile.model,
        chatCompletionSource: apiMap.source,
        secretId: profile['secret-id'],
        apiUrl: profile['api-url'],
        proxyUrl: proxyPreset?.url,
        proxyPassword: proxyPreset?.password,
        vertexAiRegion: profile['api-url'],
        zaiEndpoint: profile['api-url'],
    };
}
...
async function generateRaw(messages, options = {}) {
    ...
    try {
        ...
        const proxyPreset = (ctx.proxies || []).find(p => p.name === config.profile.proxy);
        ...
        const result = await ctx.ChatCompletionService.processRequest({
            ...
            model: config.model,
            chat_completion_source: config.chatCompletionSource,
        ...
        if (e.message.includes('Nenhum perfil') || e.message.includes('nao encontrado')) throw e;
        const detail = e.cause?.message || e.message || String(e);
        throw new Error(`Falha na geracao: ${detail}`);
    }
}
