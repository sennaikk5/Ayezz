...
function buildMessageHtml(text) {
    ...
    const resolved = scope._resolvedApprovals || {};
    ...
    const finalHtml = html.replace(approvalRegex, (_, id, label) => {
        if (Object.prototype.hasOwnProperty.call(resolved, id)) {
            const state = resolved[id]
                ? '<span class="axis-approval-label">✅ Aprovado — ' + escapeHtml(label) + '</span>'
                : '<span class="axis-approval-label">❌ Recusado — ' + escapeHtml(label) + '</span>';
            return `<div class="axis-approval axis-approval-resolved">${state}</div>`;
        }
        return `<div class="axis-approval" data-approval-id="${id}">
            <span class="axis-approval-label">⚠️ ${escapeHtml(label)}</span>
...
let mainChatGenerating = false;
let mainChatGenerationTimeout = null;

function setMainChatGenerating(val) {
    if (mainChatGenerationTimeout) {
        clearTimeout(mainChatGenerationTimeout);
        mainChatGenerationTimeout = null;
    }
    mainChatGenerating = val;
    if (val) {
        mainChatGenerationTimeout = setTimeout(() => {
            mainChatGenerating = false;
            mainChatGenerationTimeout = null;
        }, 60000);
    }
}
...
function createEspacoPanel() {
    ...
    espacoPanel.innerHTML = `
        ...
        <div id="axis-resize-handle" title="Arraste para redimensionar"></div>
    `;
...
function setupDrag(panel, handle) {
    ...
    function start(clientX, clientY, target) {
        if (target.closest('.axis-espaco-header-actions')) return false;
        isDragging = true;
        startX = clientX;
        startY = clientY;
...
function setupResize(panel, handle) {
    ...
    function move(clientX, clientY) {
        ...
        const dx = clientX - startX;
        const dy = clientY - startY;
        const newW = Math.max(MIN_W, Math.min(window.innerWidth * 0.96, initialWidth + dx));
        const newH = Math.max(MIN_H, Math.min(window.innerHeight * 0.96, initialHeight + dy));
...
function toggleToolsPanel() {
    ...
    document.getElementById('axis-tool-import').addEventListener('click', () => {
        ...
        input.addEventListener('change', (e) => { if (e.target.files[0]) importCharacterRecipe(e.target.files[0]); });
        input.click();
    });
...
async function generateRaw(messages, options = {}) {
    ...
    // Caminho preferido: função nativa do SillyTavern. Reaproveita a API
    // que já está conectada no chat principal — não precisa de Connection
    // Profile separado, e evita a classe de bugs de "perfil não configurado".
    ...
    // Reserva: versões mais antigas do SillyTavern sem generateRaw nativo.
    let profileId = options.profileId;
    if (!profileId) {
        const cm = ctx.extensionSettings?.connectionManager;
        profileId = cm?.selectedProfile;
    }
    if (!profileId) throw new Error('Nenhum perfil de conexão selecionado (e esta versão do SillyTavern não tem generateRaw nativo)');
    try {
...
function listenApprovalClicks() {
    ...
    espacoChatArea.querySelectorAll('.axis-approval-yes').forEach(btn => {
        ...
        btn.addEventListener('click', async (e) => {
            ...
            handleApproval(approveId, true, currentMiniChatId || 'main');
        });
    });
    espacoChatArea.querySelectorAll('.axis-approval-no').forEach(btn => {
        ...
        btn.addEventListener('click', (e) => {
            ...
            handleApproval(approveId, false, currentMiniChatId || 'main');
        });
    });
}

const AXIS_SYSTEMS_PROMPT_KEY = 'axis_systems_injection';

// Compila todos os sistemas aprovados do personagem ATUAL e injeta no
// pipeline real de prompt do SillyTavern via setExtensionPrompt — a mesma
// API que a extensão nativa de Memória usa. Isso faz o sistema realmente
// mudar o comportamento do personagem no chat principal, em vez de só
// ficar guardado numa lista sem efeito nenhum.
function applySystemsToPrompt() {
    const ctx = SillyTavern.getContext();
    if (typeof ctx.setExtensionPrompt !== 'function') {
        console.warn('[Spade] setExtensionPrompt indisponível nesta versão do SillyTavern — sistemas não serão aplicados automaticamente ao personagem.');
        return;
    }
    const scope = getScopeData();
    ...
    if (systems.length === 0) {
        ctx.setExtensionPrompt(AXIS_SYSTEMS_PROMPT_KEY, '', 1, 1, false, 0);
        return;
    }

    const compiled = systems.map(s => {
        const stepsText = Array.isArray(s.steps) && s.steps.length
            ? '\nEtapas obrigatórias, siga nessa ordem:\n' + s.steps.map((st, i) => `${i + 1}. ${st}`).join('\n')
            : '';
        const body = s.promptText || s.description || '';
        return `### Sistema ativo — ${s.name}\n${body}${stepsText}`;
    }).join('\n\n');

    const header = `[Sistemas configurados pelo usuário através do Spade. Siga estas instruções como parte do seu comportamento normal, sem mencionar que existe um "sistema" a menos que perguntado.]\n\n`;

    // position 1 = IN_CHAT (perto do fim do histórico, com mais peso sobre a resposta seguinte)
    // depth 1 = logo antes da última mensagem
    // role 0 = SYSTEM
    ctx.setExtensionPrompt(AXIS_SYSTEMS_PROMPT_KEY, header + compiled, 1, 1, false, 0);
}

function processSystemProposals(text, chatId) {
    const scope = getScopeData();
    // Formato JSON (não mais atributos entre aspas) porque um sistema de verdade
    // precisa de texto longo, com quebras de linha e etapas — não cabe numa
    // única string entre aspas sem quebrar o parser.
    const proposalRegex = /\[AXIS:SYSTEM_PROPOSAL\]([\s\S]*?)\[\/AXIS:SYSTEM_PROPOSAL\]/g;
    if (!scope._pendingSystems) scope._pendingSystems = {};
    const result = text.replace(proposalRegex, (_, jsonRaw) => {
        let parsed;
        try {
            parsed = JSON.parse(jsonRaw.trim());
        } catch (e) {
            return `[AXIS:CARD]\n⚠️ O agente tentou propor um sistema mas o formato veio inválido. Peça pra ele tentar de novo.\n[AXIS:CARD_END]`;
        }
        const name = String(parsed.name || 'Sistema sem nome').trim().replace(/"/g, "'");
        const type = String(parsed.type || 'behavior').trim();
        const description = String(parsed.description || '').trim().replace(/"/g, "'");
        const steps = Array.isArray(parsed.steps) ? parsed.steps.map(s => String(s).trim()).filter(Boolean) : [];
        const promptText = String(parsed.promptText || parsed.description || '').trim();

        const uid = `sys_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
        scope._pendingSystems[uid] = { name, type, description, steps, promptText, enabled: true, createdAt: Date.now() };

        const stepsPreview = steps.length ? '\n' + steps.map((s, i) => `${i + 1}. ${s}`).join('\n') : '';
        return `[AXIS:CARD]\n📐 Sistema proposto: ${name} (${type})\n\n${description}${stepsPreview}\n[AXIS:CARD_END]\n[AXIS:APPROVAL ID:"create_system_${uid}" LABEL:"Criar sistema '${name}'?"]`;
    });
    saveData();
    return result;
}
...
function handleApproval(approveId, approved, chatId) {
    ...
    const isReject = approveId.startsWith('reject_');
    const baseId = isReject ? approveId.replace('reject_', '') : approveId;

    // Persiste o resultado pra sempre, pra não voltar a mostrar Sim/Não
    // depois que o chat re-renderizar (ex: ao chegar outra mensagem).
    ...
    if (baseId.startsWith('create_system_')) {
        ...
        if (pending) {
            ...
            saveData();
            applySystemsToPrompt();
            addEspacoMessage('agent', `✅ Sistema "${pending.name}" criado e já está ativo no personagem — ele vai afetar as respostas no chat principal a partir de agora.`, chatId);
        }
    } else if (baseId.startsWith('connect_rp_')) {
        const cid = baseId.replace('connect_rp_', '');
        connectToCampoRP(cid);
        addEspacoMessage('agent', `✅ "${cid}" conectado ao Campo RP.`, chatId);
...
async function sendEspacoMessage() {
    ...
    if (mainChatGenerating) {
        showAxisFatalError('O chat principal ainda está gerando uma resposta. Espera terminar antes de mandar mensagem aqui — é a mesma IA, uma coisa de cada vez.');
        return;
    }
    ...
    try {
        ...
        const messages = [];
        const systemContent = `Você é um agente de IA auxiliar para configuração de personagens de roleplay no SillyTavern. Você é amigável, direto e conversacional. Você pode criar sistemas (engines de comportamento reais), conectar chats ao Campo RP, treinar tom de voz do personagem e ajudar com configurações.

REGRA CRÍTICA: você NUNCA instrui o usuário a colar texto manualmente em nenhum campo do SillyTavern (Main Prompt, System Prompt, Author's Note, etc). Isso não é mais necessário. Qualquer coisa que precise afetar o comportamento do personagem deve ser proposta como um Sistema (veja abaixo), que é aplicado automaticamente e de verdade no personagem assim que aprovado — sem o usuário precisar copiar/colar nada.

Quando o usuário pedir para criar um "sistema" (uma mecânica ou engine de comportamento para o personagem, ex: sistema de pensamento/raciocínio interno, sistema de humor, sistema de memória de curto prazo, sistema de relacionamento, etc), você deve propor algo REALMENTE desenvolvido — não uma frase única ou um prompt vago. Um sistema de verdade tem:
- um TIPO (thinking = raciocínio interno antes de responder, mechanic = mecânica/regra de jogo, behavior = padrão de comportamento, memory = memória/continuidade, other)
- 3 a 6 ETAPAS concretas e sequenciais que definem como o sistema funciona passo a passo
- um "promptText" que é o texto REAL e completo que vai ser injetado nas instruções do personagem — escreva ele como uma instrução de verdade, bem escrita, específica, referenciando as etapas, do jeito que você escreveria pra um roteirista seguir. Não escreva algo genérico tipo "pense antes de responder" — desenvolva de verdade (ex: para um sistema de pensamento, defina exatamente as fases do raciocínio: percepção da situação, avaliação emocional interna, cálculo de resposta social, filtro de personalidade, e só então a fala).

Proponha usando EXATAMENTE este formato (JSON dentro das tags, sem nada fora dele além do texto normal da sua resposta):
[AXIS:SYSTEM_PROPOSAL]
{"name": "Nome curto do sistema", "type": "thinking", "description": "resumo em 1-2 frases", "steps": ["Etapa 1 detalhada", "Etapa 2 detalhada", "Etapa 3 detalhada"], "promptText": "Texto completo e bem desenvolvido da instrução, pode ter várias frases e parágrafos, referenciando as etapas."}
[/AXIS:SYSTEM_PROPOSAL]

Não crie o sistema sozinho — apenas proponha com esse marcador, o usuário vai aprovar ou recusar. Depois de proposto, continue a conversa normalmente, sem repetir o JSON.
...
function listCrossoverTargets() {
    ...
    scopes.forEach(s => {
        const rawName = axisData[s]?.[ESPACO_CHATS_KEY]?.main?.name || s;
...
eventSource.on(event_types.APP_READY, () => {
    ...
    // Re-renderiza mesmo se o painel já existia (criado antes do APP_READY),
    // porque só agora o personagem/chat atual está garantidamente carregado
    // e o escopo correto pode ser resolvido — sem isso o chat salvo "some"
    // ao reiniciar o SillyTavern.
    renderEspacoChat();
    renderMiniChatBar();
    applySystemsToPrompt();
});
...
// Evita que o Spade mande uma mensagem pra IA enquanto o chat principal
// do SillyTavern já está gerando uma resposta — é a mesma IA por trás,
// então uma coisa de cada vez.
eventSource.on(event_types.GENERATION_STARTED, () => {
    setMainChatGenerating(true);
});
eventSource.on(event_types.GENERATION_STOPPED, () => {
    setMainChatGenerating(false);
});
