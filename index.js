...
function toggleProfilePanel() {
    ...
    p.innerHTML =
        ...
        '</select>' +
        '<div class="axis-inline-row">' +
        '<input type="text" id="axis-profile-preset-name" class="axis-select" placeholder="Nome do preset (ex: Hanna)" style="flex:1;">' +
        '<button class="axis-btn axis-btn-sm" id="axis-profile-preset-new">💾 Salvar</button>' +
        '</div>' +
    ...
    document.getElementById('axis-profile-preset-new').addEventListener('click', () => {
        try {
            const name = (document.getElementById('axis-profile-preset-name').value || '').trim();
            if (!name) { document.getElementById('axis-profile-preset-name').placeholder = '⚠️ Digite um nome primeiro'; return; }
            const s2 = scope();
            ...
            console.error('[Spade] Falha ao salvar preset nomeado:', e);
            const inputEl = document.getElementById('axis-profile-preset-name');
            if (inputEl) inputEl.placeholder = '⚠️ Erro: ' + (e.message || e);
        }
    });
...
// Substitui confirm()/prompt() nativos em ações destrutivas — em alguns
// WebViews/apps de desktop remoto esses diálogos do navegador não abrem
// (o clique parece não fazer nada). Padrão: primeiro clique arma, segundo
// clique dentro de alguns segundos confirma de verdade.
let _confirmArmed = {};
function armConfirm(key, ms) {
    if (_confirmArmed[key]) { delete _confirmArmed[key]; return true; }
    _confirmArmed[key] = true;
    setTimeout(() => { delete _confirmArmed[key]; }, ms || 4000);
    return false;
}
...
function deleteMiniChat(id) {
    ...
    if (!mc) return;
    if (!armConfirm('delMini_' + id)) {
        const btn = document.querySelector('.axis-mini-del[data-del="' + id + '"]');
        if (btn) { btn.textContent = '✕?'; btn.title = 'Clique de novo pra apagar de vez'; }
        return;
    }
...
function clearMainChat(btn) {
    if (!armConfirm('clearMain')) {
        if (btn) { btn.textContent = '🧹 Clique de novo pra confirmar'; }
        return false;
    }
    const s = scope();
...
function buildHtml(text) {
    ...
    return c.innerHTML.replace(
        ...
        (_, id, label) => res.hasOwnProperty(id)
            ...
            : '<div class="axis-approval" data-approval-id="' + id + '"><span class="axis-approval-label">⚠️ ' + esc(label) + '</span><button type="button" class="axis-approval-yes" data-approve="' + id + '">Sim</button><button type="button" class="axis-approval-no" data-approve="reject_' + id + '">Não</button></div>'
    ).replace(
        ...
        (_, id, label) => res.hasOwnProperty(id)
            ...
            : '<div class="axis-voice-approval" data-voice-approval-id="' + id + '">' +
                ...
                '<button type="button" class="axis-approval-yes axis-voice-approve-yes" data-voice-approve="' + id + '">Sim, salvar</button>' +
                '<button type="button" class="axis-approval-no axis-voice-approve-no" data-voice-approve="reject_' + id + '">Não</button>' +
                '</div>' +
                ...
                '<button type="button" class="axis-btn axis-btn-sm axis-voice-adjust-btn" data-voice-id="' + id + '">Ajustar</button>' +
                '</div></div>'
    ).replace(
        ...
        (_, id, label) => res.hasOwnProperty(id)
            ...
            : '<div class="axis-voice-approval axis-profile-approval" data-profile-approval-id="' + id + '">' +
                '<div class="axis-approval-label">⚠️ ' + esc(label) + '</div>' +
                '<div class="axis-voice-approval-actions">' +
                '<button type="button" class="axis-approval-yes axis-profile-approve-yes" data-profile-approve="' + id + '">Sim, salvar</button>' +
                '<button type="button" class="axis-approval-no axis-profile-approve-no" data-profile-approve="reject_' + id + '">Não</button>' +
                '</div>' +
                '<div class="axis-voice-approval-adjust">' +
                ...
                '<button type="button" class="axis-btn axis-btn-sm axis-voice-adjust-btn axis-profile-adjust-btn" data-profile-id="' + id + '">Ajustar</button>' +
                '</div></div>'
    );
}
    ...
    el.querySelectorAll('.axis-profile-approve-yes').forEach(b => {
        ...
        b.addEventListener('click', e => {
            try {
                ...
                if (card) card.innerHTML = '<span class="axis-approval-label">✅ Salvando...</span>';
                handleProfileApproval(id, true, fixedChatId || currentMiniChatId || 'main');
            } catch (err) {
                console.error('[Spade] Erro ao aprovar Perfil:', err);
                e.target.closest('.axis-voice-approval')?.insertAdjacentHTML('beforeend', '<div style="color:#f88;font-size:11px;">⚠️ Erro: ' + esc(err.message || String(err)) + '</div>');
            }
        });
    });
    el.querySelectorAll('.axis-profile-approve-no').forEach(b => {
        ...
        b.addEventListener('click', e => {
            try {
                const id = e.target.dataset.profileApprove;
                const card = e.target.closest('.axis-profile-approval');
                if (card) card.innerHTML = '<span class="axis-approval-label">❌ Rascunho descartado</span>';
                handleProfileApproval(id, false, fixedChatId || currentMiniChatId || 'main');
            } catch (err) {
                console.error('[Spade] Erro ao descartar rascunho de Perfil:', err);
                e.target.closest('.axis-voice-approval')?.insertAdjacentHTML('beforeend', '<div style="color:#f88;font-size:11px;">⚠️ Erro: ' + esc(err.message || String(err)) + '</div>');
            }
        });
    });
    el.querySelectorAll('.axis-profile-adjust-btn').forEach(b => {
        ...
        b.addEventListener('click', e => {
            try {
                ...
                console.error('[Spade] Erro ao ajustar rascunho de Perfil:', err);
            }
        });
    });
}
...
function toggleToolsPanel() {
    ...
    document.getElementById('axis-tool-clear-main').addEventListener('click', (e) => { if (clearMainChat(e.target)) p.remove(); });
