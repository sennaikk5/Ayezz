...
async function generateRaw(messages, options = {}) {
    ...
    try {
        const result = await connectionManager.sendRequest(profileId, messages, maxTokens, {
            stream,
            signal,
            extractData: true,
            includePreset: false,
...
function handleApproval(approveId, approved) {
    ...
    if (approveId.startsWith('create_system_')) {
        const pending = scope._pendingSystem;
        if (pending && approved) {
            scope[SYSTEMS_KEY].push(pending);
            delete scope._pendingSystem;
            saveData();
...
async function sendEspacoMessage() {
    ...
    try {
        ...
        addEspacoMessage('agent', cleaned, chatId);
    } catch (e) {
...
const originalGenerateRaw = connectionManager?.sendRequest;
if (originalGenerateRaw) {
    connectionManager.sendRequest = async function (profileId, prompt, maxTokens, custom, overridePayload) {
        return await originalGenerateRaw.call(connectionManager, profileId, prompt, maxTokens, custom, overridePayload);
    };
}
