...
async function postCharacterMessage(text) {
    try {
        if (isGlobalLocked()) return { ok: false, reason: 'RP está gerando algo agora — espera terminar e pede de novo.' };
        ...
        if (decision.order === 'npc_first') {
            ...
            if (line) {
                ...
                if (decision.charSpeaks === false) {
                    ...
                    // aborta a geração aqui — sem isso o mutex ficava travado
                    // pra sempre, achando que ainda tinha algo gerando.
                    globalUnlock();
                    dotsIdle();
                }
            }
...
function toggleToolsPanel() {
    ...
    document.getElementById('axis-tool-unlock').addEventListener('click', () => {
        p.remove();
        GLOBAL_MUTEX = false; GLOBAL_MUTEX_SINCE = 0; isGenerating = false;
