...
async function getRecentSceneText(n = 6) {
    return chatRealRecente(n * 2).map((m) => (m.ehUsuario ? 'Você' : personagemAtual()) + ': ' + m.texto).join('\n');
}
...
const TOOLS = [
    ...
    { type: 'function', function: { name: 'biblioteca_editar_documento', description: 'Reescreve um documento INTEIRO guardado na Biblioteca (os chunks de um arquivo que foi lido, tratados como uma folha só) — útil pra limpar/organizar um documento bagunçado, ou deixar só o que interessa. Sempre leia o documento (biblioteca_buscar com o nome do arquivo) antes de reescrever, pra não perder conteúdo por engano.', parameters: { type: 'object', properties: { arquivo: { type: 'string', description: 'Nome do arquivo, exatamente como aparece em metadata.arquivo dos resultados de busca.' }, novoTexto: { type: 'string' } }, required: ['arquivo', 'novoTexto'] } } },
    ...
    { type: 'function', function: { name: 'chat_ver', description: 'Mostra as últimas mensagens do chat de VERDADE no SillyTavern agora — com índice real de cada uma. Use antes de editar/apagar algo específico, ou só pra checar o que realmente está na tela (o usuário pode ter editado, apagado ou dado swipe direto no ST sem você saber).', parameters: { type: 'object', properties: { quantas: { type: 'integer', description: 'Quantas mensagens recentes ver (padrão 12).' } } } } },
    ...
    { type: 'function', function: { name: 'rp_editar_idx', description: 'Edita uma mensagem ESPECÍFICA do chat, pelo índice que você viu com chat_ver — use quando não for a última mensagem, ou quando quiser confirmar que é exatamente aquela antes de mexer.', parameters: { type: 'object', properties: { idx: { type: 'integer' }, texto: { type: 'string' } }, required: ['idx', 'texto'] } } },
    { type: 'function', function: { name: 'rp_apagar_idx', description: 'Apaga uma mensagem ESPECÍFICA do chat, pelo índice que você viu com chat_ver.', parameters: { type: 'object', properties: { idx: { type: 'integer' } }, required: ['idx'] } } },
...
async function executeTool(name, args) {
    switch (name) {
        ...
        case 'chat_ver': return chatRealRecente(Math.min(30, Math.max(1, Number(args.quantas) || 12)));
...
const ACTION_LABEL = {
    ...
    rp_editar_idx: '✍️ editando uma mensagem específica',
    rp_apagar_idx: '🗑️ apagando uma mensagem específica',
    chat_ver: '👀 olhando o chat de verdade',
...
async function aplicarAcaoRp(acao, args, atividade) {
    if (acao === 'rp_postar') return postCharacterMessage(args.texto, atividade.token);
    if (acao === 'rp_editar_ultima' || acao === 'rp_reescrever_ultima') return editLastCharacterMessage(args.texto);
    if (acao === 'rp_apagar_ultima') return deleteLastCharacterMessage();
    if (acao === 'rp_editar_idx') return editMessageByIdx(args.idx, args.texto);
    if (acao === 'rp_apagar_idx') return deleteMessageByIdx(args.idx);
    return { ok: false, reason: 'ação desconhecida: ' + acao };
}
// Editar/apagar por índice — pra quando ela viu o chat de verdade (via
// chat_ver) e sabe QUAL mensagem, não só "a última que eu lembro escrever".
function editMessageByIdx(idx, newText) {
    const chatArr = ctx().chat;
    if (!Array.isArray(chatArr) || !chatArr[idx]) return { ok: false, reason: 'Não existe mensagem no índice ' + idx + ' — confere com chat_ver de novo, o chat pode ter mudado.' };
...
async function passadaComFerramentas(messages, atividade, { onNarracao } = {}, opts = {}) {
    ...
    for (let i = 0; i < teto; i++) {
        ...
        for (const call of resp.toolCalls) {
            ...
            let resultado, erroTool = null;
            try {
                resultado = nome.startsWith('rp_') ? await aplicarAcaoRp(nome, args, atividade)
...
async function pensarUmaVez() {
    ...
    try {
        ...
        escreverTicker('⚠️ tropecei em algo, tentando de novo...');
        await journalAdicionar('⚠️ deu erro nessa passada (' + e.message + ') — tento de novo em ' + Math.round(esperaErro) + 's.', 'pensamento');
