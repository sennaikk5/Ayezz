...
// Ajuste pedido pelo usuário (qualquer tipo — fala, comportamento, plot,
// relação, regra de mundo) marcado sempreIncluir: true entra inteiro,
// sempre, direto no prompt do RP — não é buscado por similaridade nem
// compete com outra coisa, e não tem timer/expiração: o Artefato em si
// já é a fonte, lido direto toda vez. É o que faz Espaço e RP serem "um
// só" pro usuário — o que ele pedir em qualquer lugar vale imediatamente
// e continua valendo, pra qualquer tipo de ajuste, não só tom de voz.
async function compiledAjustesFixosBlock(chatId) {
    const personagem = personagemAtual();
    let docs = [];
    try { docs = (await chamarSpadeFs('/documento/listar', { personagem, chatId })).documentos || []; }
    catch { return ''; }
    const fixos = docs.filter((d) => d.sempreIncluir);
    if (!fixos.length) return '';
    const conteudos = await Promise.all(fixos.map((d) =>
        chamarSpadeFs('/documento/ler', { personagem, slug: d.slug, chatId: d.escopo === 'chat' ? chatId : null })
            .then((r) => r.conteudo).catch(() => null)
    ));
    const texto = conteudos.filter(Boolean).join('\n\n');
    return texto ? '[AJUSTE OBRIGATÓRIO — o usuário pediu isso direto, siga à risca em TODA resposta a partir de agora, sem exceção. Isso pesa mais que hábito antigo ou o que pareceria "natural" pra ela]\n' + texto : '';
}
...
async function runCantos(userMessage) {
    ...
    const [bibliotecaResult, repeticaoResult, estadoResult, mundoResult, sistemasResult, sistemasReaisResult, ajustesFixosResult] = await Promise.allSettled([
        ...
        compiledAjustesFixosBlock(stChatIdAtual()),
    ]);
...
async function buildSystemPrompt(cantos) {
    ...
    const parts = [
        ...
        (cantos.ajustesFixos?.value ? ' Releia o AJUSTE OBRIGATÓRIO lá em cima antes de responder — essa é a última checagem antes de falar, não uma sugestão do começo do prompt pra esquecer.' : ''),
    ];
...
async function executeTool(name, args) {
    switch (name) {
        ...
        case 'artefato_escrever': {
            ...
            return Object.assign(r, { sincronizadoNaBiblioteca: r.chunks + ' trecho(s) — já buscável no RP de verdade agora.', ajusteFixo: !!args.sempreIncluir });
        }
...
async function respondEspaco(userMessage, chatId) {
    ...
    const systemPrompt =
        ...
        'AJUSTE NA HORA (qualquer tipo, não só fala) — se o usuário pedir uma mudança direta pra valer JÁ no RP ' +
        '("fala mais desse jeito", "ela devia ficar com ciúme disso", "para de fazer ela concordar com tudo", "ela ' +
        'ainda não sabe disso", seja lá o que for — fala, comportamento, relação, regra de mundo, plot), chame ' +
        'artefato_escrever com sempreIncluir: true IMEDIATAMENTE, sem esperar confirmação. sempreIncluir não é ' +
        'sobre categoria nenhuma — é sobre ser uma instrução direta que precisa valer sempre, não uma referência ' +
        'solta pra buscar quando for relevante: com sempreIncluir, o conteúdo entra inteiro em TODA cena do RP, ' +
        'sem depender de busca/ranking, sem prazo pra expirar — Espaço e RP são uma coisa só, o que o usuário pede ' +
        'aqui vale lá igual, imediatamente e pra sempre até ele mudar de novo. Pode dizer pro usuário que já vale a ' +
        'partir da próxima fala dela, porque vale mesmo. Se já existe um artefato sempreIncluir sobre o mesmo ' +
        'assunto, chame artefato_ler nele antes e reescreva completo (mistura o que já tinha com o ajuste novo) em ' +
        'vez de criar um segundo — dois artefatos fixos sobre a mesma coisa é ruído, e cada um custa token em TODA ' +
        'mensagem do RP dali pra frente, então não marque sempreIncluir em algo que é só informação de fundo (isso ' +
        'aí é artefato normal, sem a flag — ele já é buscável). IMPORTANTE sobre COMO escrever: se o ajuste for ' +
        'sobre ritmo/tamanho de fala ("fala menos", "respostas mais dinâmicas"), escreva como REGRA DIRETA e ' +
        'checável ("respostas de no máximo N frases", "corta narração que não muda a cena") — não como descrição ' +
        'solta tipo "ela é mais dinâmica agora". Regra concreta muda comportamento; adjetivo bonito não.\n\n' +
