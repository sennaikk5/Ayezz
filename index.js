...
async function sincronizarAjusteAutomatico(mensagemUsuario) {
    ...
    const prompt =
        ...
        'relacionamento, preferência, lore, aparência. {"tipo":"fato","categoria":"traço_personalidade | ' +
        'sentimento | relacionamento | preferencia | lore | aparencia | outro","texto":"o fato, reescrito claro e ' +
        'autocontido"}\n' +
...
async function handleUploadEspaco(files, instrucaoUsuario) {
    ...
    await estadoSet(campoHistorico, [...historicoAntes, { role: 'user', content: mensagemSalva }].slice(-80));
    await tocarEspacoChat();
