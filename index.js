...
async function compiledEspacoRecenteBlock(stChatId) {
    ...
    const log = (motivo, extra) => { journalAdicionar('[espaço→RP] ' + motivo + (extra ? ': ' + extra : ''), 'pensamento', 'diagnostico').catch(() => {}); };

    let chats = [];
    try { chats = await obterEspacoChats(); } catch (e) { log('erro ao listar chats do Espaço', e.message); return ''; }
    if (!chats.length) { log('nenhum chat de Espaço existe ainda pra essa personagem'); return ''; }

    let chat = stChatId ? chats.find((c) => c.stChatId === stChatId) : null;
    const viaFallback = !chat;
    if (!chat) chat = chats.slice().sort((a, b) => b.atualizadoEm - a.atualizadoEm)[0];
    if (!chat) { log('lista de chats não vazia mas nenhum candidato achado (não deveria acontecer)'); return ''; }

    let historico = [];
    try { historico = await estadoGet('espacoHistory:' + chat.id, []); } catch (e) { log('erro ao ler histórico do chat ' + chat.id, e.message); return ''; }
    if (!historico.length) { log('chat "' + (chat.titulo || chat.id) + '" achado (' + (viaFallback ? 'fallback, sem stChatId batendo' : 'vínculo direto') + ') mas sem mensagens'); return ''; }

    const recentes = historico.slice(-16);
    const texto = recentes.map((m) => (m.role === 'user' ? 'Usuário' : 'Você mesma') + ': ' + m.content).join('\n');
    log('incluído (' + (viaFallback ? 'FALLBACK, stChatId não bateu' : 'vínculo direto') + '), ' + recentes.length + ' msg do chat "' + (chat.titulo || chat.id) + '"', texto.slice(0, 220).replace(/\n/g, ' / '));
...
async function buildSystemPrompt(cantos) {
    ...
    const parts = [
        ...
        // espacoRecente vai por ÚLTIMO de propósito, logo antes da instrução
        // final — é o que tem mais chance de ter sido dito AGORA, e quanto
        // mais perto do ponto de gerar, mais peso ganha. No topo (como era
        // antes), competia com biblioteca/estado/mundo no meio e perdia.
...
function liberarAtividadeRp() {
    if (atividadeAtual?.onde === 'rp') { atividadeAtual = null; atualizarStatusBar(); }
}
...
async function Spade_interceptGeneration(chatArr, contextSize, abort, type) {
    ...
    try {
        ...
        // ANTES: terminarAtividade só rodava no caminho feliz — erro no meio
        // (rede, API, o que for) deixava atividadeAtual presa em 'rp' PRA
        // SEMPRE, travando Pensamento até "mandar qualquer mensagem" roubar
        // o lock de outro lugar. Isso é o que fazia parecer que precisava
        // de um empurrão manual toda vez que dava erro.
