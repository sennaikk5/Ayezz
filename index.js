...
let _saveFailureWarned = false; // evita empilhar o mesmo aviso a cada save() que falhar em sequência
function save() {
    try {
        localStorage.setItem(DB, JSON.stringify(data));
        _saveFailureWarned = false; // voltou a funcionar — destrava o aviso pra próxima falha real
    } catch (e) {
        console.error('[Spade] save() falhou — nada está sendo persistido agora:', e);
        if (!_saveFailureWarned) {
            _saveFailureWarned = true;
            const isQuota = e && (e.name === 'QuotaExceededError' || /quota/i.test(e.message || ''));
            try { showStatusPill('⚠️ Falha ao salvar' + (isQuota ? ' — armazenamento cheio' : '') + '! Nada tá sendo persistido agora.'); } catch (_) {}
            try {
                alert('⚠️ O Spade não conseguiu salvar.' + (isQuota
                    ? ' O armazenamento local do navegador está cheio — provavelmente precisa apagar mini-chats/conversas antigas do Espaço pra liberar espaço.'
                    : ' Erro: ' + (e.message || e)) +
                    '\n\nEnquanto isso não for resolvido, NENHUMA mudança feita na extensão vai persistir (perfil, presets, aprovações, nada).');
            } catch (_) {}
        }
    }
}
...
async function handleProfileApproval(id, approved, chatId) {
    try {
        ...
        console.error('[Spade] handleProfileApproval falhou:', e);
        addMsg('agent', '⚠️ Deu erro ao salvar o perfil: ' + (e.message || e), chatId);
    }
}
...
function toggleProfilePanel() {
    ...
    p.innerHTML =
        ...
        '</select>' +
        '<div class="axis-inline-row" style="display:flex;gap:6px;margin-top:6px;">' +
        '<input type="text" id="axis-profile-preset-name" class="axis-select" style="flex:1;" placeholder="Nome do preset (ex: Hanna)">' +
        '<button class="axis-btn axis-btn-sm" id="axis-profile-preset-new">💾 Salvar preset</button>' +
        '</div>' +
    ...
    document.getElementById('axis-profile-preset-select')?.addEventListener('change', e => {
        try {
            ...
            console.error('[Spade] Falha ao trocar preset:', e2);
            alert('⚠️ Não consegui trocar de preset: ' + (e2.message || e2));
        }
    });
    document.getElementById('axis-profile-preset-new').addEventListener('click', () => {
        try {
            ...
            if (!name) { alert('Digita um nome pro preset antes de salvar.'); return; }
            const s2 = scope();
            ...
            console.error('[Spade] Falha ao salvar preset:', e);
            alert('⚠️ Não consegui salvar o preset: ' + (e.message || e));
        }
    });
    document.getElementById('axis-profile-save').addEventListener('click', () => {
        try {
            ...
            console.error('[Spade] Falha ao salvar perfil:', e);
            alert('⚠️ Não consegui salvar o perfil: ' + (e.message || e));
        }
    });
}
