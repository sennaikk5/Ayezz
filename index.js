...
//
// MAS erro de autenticação/credencial (chave inválida, rate-limit de auth
// do provider) é o oposto de transitório — retentar manda a MESMA chave
// errada de novo, e é exatamente isso que faz o provider bloquear ainda
// mais forte (foi o que aconteceu: "rate limited after repeated invalid
// credentials" — nós mesmos, retentando 3x em CADA chamada, em CADA
// função que usa IA, éramos boa parte das tentativas repetidas). Nesses
// casos falha na hora, sem retry — continuar tentando não é "ficar viva",
// é bater a cabeça na mesma parede mais rápido.
function ehErroDeAutenticacao(erro) {
    const msg = String(erro?.message || '');
    return /^NanoGPT \((401|403|429)\)/.test(msg) && /auth|credential/i.test(msg);
}
async function comRetentativas(fn, { tentativas = 3, esperaMs = 1200 } = {}) {
    ...
    for (let i = 0; i < tentativas; i++) {
        try { return await fn(); }
            ...
            ultimoErro = e;
            if (ehErroDeAutenticacao(e)) break; // chave errada não se resolve tentando de novo
...
// ====================================
// FILTRO DE RACIOCÍNIO (<think>...</think>) — nenhum provider deveria
// vazar isso em `delta.content`/`message.content` (o padrão certo é um
// campo separado tipo `reasoning_content`, que a gente já nem lê), mas
// vários modelos "raciocinadores" atrás da NanoGPT fazem isso mesmo
// assim, direto misturado no texto de verdade. Sem filtro nenhum aqui,
// isso é exatamente "vaza o pensamento na resposta" — e se o max_tokens
// estoura ENQUANTO ainda tá dentro do bloco de pensar, a resposta de
// verdade nunca chega a ser gerada: o que sobra é só o raciocínio
// truncado, parecendo "travado numa mensagem antiga" porque um
// raciocínio tipicamente começa reanalisando o contexto anterior.
// ====================================
function filtrarThinking(texto) {
    if (!texto) return texto;
    // bloco fechado normal
    let limpo = texto.replace(/<think>[\s\S]*?<\/think>/gi, '');
    // bloco NUNCA fechado (cortado pelo max_tokens no meio) — tudo daquele
    // ponto em diante é raciocínio truncado, não resposta; melhor cortar
    // fora do que devolver pensamento cru.
    limpo = limpo.replace(/<think>[\s\S]*$/i, '');
    return limpo.trim();
}
// Versão streaming — precisa de estado entre chunks porque a tag pode vir
// cortada no meio (um chunk termina em "<thi", o próximo começa com
// "nk>"). Devolve só o texto que É pra mostrar; enquanto dentroThink for
// true, emite nada — se o bloco nunca fechar (truncado), o efeito líquido
// é não mostrar nada daquele trecho, o que é o comportamento certo aqui.
function criarFiltroThinking() {
    let dentroThink = false;
    let pendente = '';
    return function filtrar(deltaTexto) {
        let resto = pendente + (deltaTexto || '');
        pendente = '';
        let saida = '';
        while (resto.length) {
            if (!dentroThink) {
                const abre = resto.indexOf('<think>');
                if (abre === -1) {
                    // pode ser só o COMEÇO de "<think>" cortado no fim do chunk
                    const m = resto.match(/<t(h(i(n(k)?)?)?)?$/);
                    if (m) { saida += resto.slice(0, m.index); pendente = m[0]; resto = ''; }
                    else { saida += resto; resto = ''; }
                } else {
                    saida += resto.slice(0, abre);
                    resto = resto.slice(abre + 7);
                    dentroThink = true;
                }
            } else {
                const fecha = resto.indexOf('</think>');
                if (fecha === -1) { resto = ''; } // ainda dentro do bloco — descarta, espera o próximo chunk
                else { resto = resto.slice(fecha + 8); dentroThink = false; }
            }
        }
        return saida;
    };
...
async function generate(model, messages, opts = {}) {
    return comRetentativas(async () => {
        ...
        try {
            ...
            const content = filtrarThinking(choice?.message?.content ?? '');
            if (!content.trim() && choice?.finish_reason === 'length') throw new Error('voltou vazio (max_tokens estourou antes de qualquer texto de verdade — só raciocínio, se o modelo for desses) — tentando de novo');
            return content;
        } finally {
            clearTimeout(timer);
        }
...
async function generateWithTools(model, messages, tools, opts = {}) {
    return comRetentativas(async () => {
        ...
        try {
            ...
            const choice = data.choices?.[0];
            return {
                content: filtrarThinking(choice?.message?.content ?? ''),
                toolCalls: choice?.message?.tool_calls ?? [],
                finishReason: choice?.finish_reason,
                // Alguns modelos "raciocinadores" (MiMo com thinking ligado, no
                // formato "certo" — campo separado, não misturado em content
                // com <think> como o DeepSeek) devolvem isso À PARTE de
                // tool_calls. A própria Xiaomi documenta que, se não devolver
                // esse campo de volta na mensagem assistant em toda chamada
                // SEGUINTE que reusa esse histórico com tool_calls, a API
                // responde 400 (não warning, erro de verdade) — ver uso em
                // passadaComFerramentas/passoConstrutora. Pra modelo que não
                // manda isso, fica null e nada muda.
...
async function generateStream(model, messages, onToken, opts = {}) {
    ...
    try {
        ...
        const filtrar = criarFiltroThinking();
        let buffer = '', cruTotal = 0;
        while (true) {
            ...
            for (const line of lines) {
                ...
                const deltaCru = chunk.choices?.[0]?.delta?.content;
                if (deltaCru) {
                    cruTotal += deltaCru.length;
                    const delta = filtrar(deltaCru); // corta <think>...</think> antes de mostrar/acumular
                    if (delta) { full += delta; onToken(delta); }
                }
            }
        }
        // Recebeu bastante coisa do provider, mas nada sobrou depois do
        // filtro — não é "modelo escolheu ficar em silêncio" (isso vem com
        // cruTotal baixo/zero), é "o max_tokens estourou inteiro dentro de
        // <think>, nunca chegou na resposta de verdade". Lança erro em vez
        // de devolver vazio, pra cair no retry que já existe do lado de
        // fora — devolver vazio aqui fazia esse caso ser tratado como
        // "sucesso vazio genuíno" e NÃO tentar de novo.
        if (!full.trim() && cruTotal > 30) {
            throw new Error('resposta toda consumida por raciocínio (<think>) sem sobrar texto de verdade — tentando de novo');
        }
        return full;
    } catch (err) {
...
const Biblioteca = {
    ...
    async escrever({ tipo, texto, metadata = {}, chaveFixa = null, sabemDisso = null, importancia = 0.5, entidades = [], substitui = null, personagem: personagemOverride = null, embedding: embeddingPreCalculado = null, chatId = null }) {
        ...
        const entry = {
            id, tipo, texto: textoLimpo, embedding, metadata: metadataFinal, personagem, chatId,
    ...
    // `chatId` opcional (item "cada memória por chat"): quando vem, o
    // resultado é a UNIÃO do que é do personagem inteiro (chatId null —
    // é o que toda entrada de antes dessa mudança já tinha, ninguém perde
    // acesso a nada existente) com o que foi escrito só pra esse chat
    // específico. Omitir chatId continua trazendo tudo, como sempre foi —
    // uso interno (Fundição, auto-estudo etc) não precisa saber que isso
    // existe.
    async listar({ tipos, personagem, chatId } = {}) {
        const all = await idbAllByPersonagem('biblioteca', personagem || personagemAtual());
        const porTipo = tipos?.length ? all.filter((e) => tipos.includes(e.tipo)) : all;
        const porChat = chatId ? porTipo.filter((e) => !e.chatId || e.chatId === chatId) : porTipo;
        return porChat.sort((a, b) => b.updatedAt - a.updatedAt);
    },
    ...
    async buscarVoz(consulta, { tipos = ['fala', 'tom'], k = 5, lambda = 0.7, chatId = null } = {}) {
        if (!consulta?.trim()) return [];
        const all = (await this.listar({ tipos, chatId })).filter((e) => e.metadata?.ativo !== false && e.embedding);
    ...
    async buscarMemoria(consulta, { tipos = null, paraPersonagem = null, personagem = null, k = 8, chatId = null } = {}) {
        if (!consulta?.trim()) return [];
        const escopo = personagem || personagemAtual();
        const alvo = paraPersonagem || escopo;
        const all = await this.listar({ tipos, personagem: escopo, chatId });
...
function parseJsonObjetoSeguro(texto) {
    const match = texto.match(/\{[\s\S]*\}/);
    if (!match) return null;
    try { const obj = JSON.parse(match[0]); return (obj && typeof obj === 'object' && !Array.isArray(obj)) ? obj : null; } catch { return null; }
}
...
async function extrairLote(segmentos, contextoDocumento) {
    ...
    const prompt =
        ...
        (contextoDocumento.instrucaoUsuario ? 'O usuário mandou isso junto com o arquivo: "' + contextoDocumento.instrucaoUsuario + '" — leve em conta pra decidir o que vale destacar e como categorizar, o pedido dele pesa mais que uma leitura genérica do arquivo.\n\n' : '') +
        'Trechos numerados:\n' + listaNumerada + '\n\n' +
        ...
        'ATENÇÃO com "exemplo_dialogo"/fala/tom: se o trecho é uma FALA de verdade do personagem (algo que ele ' +
        'disse, entre aspas ou como diálogo), o "fato" desse item é o TRECHO LITERAL da fala, palavra por palavra — ' +
        'NÃO parafraseia/resume uma fala. O valor inteiro de um exemplo de diálogo é o jeito exato de falar (gíria, ' +
        'pontuação, tamanho de frase); reescrever "de forma objetiva" destrói isso. A regra de reescrever ' +
        'autocontido acima vale pra FATO (lore, traço, regra), não pra fala — pra fala, copie exatamente.\n\n' +
        'Responda só um array JSON, um item por trecho que vale guardar:\n' +
        '[{"trecho": numero_do_trecho, ' +
        '"fato": "pra fato/lore: reescrito de forma objetiva e AUTOCONTIDA — sem pronome solto tipo \'ele\'/\'isso\' ' +
        'sem deixar claro quem/o que é, alguém lendo só essa frase precisa entender. PRA FALA/DIÁLOGO: a citação ' +
...
// Mesmo critério usado na hora de indexar na Biblioteca (ver
// sincronizarDocumentoNaBiblioteca, mais abaixo) — categoria de
// fala/voz/diálogo precisa ficar INTOCADA daqui pra frente: nem extração
// nem contextualização podem reescrever uma citação literal.
const CATEGORIA_DE_FALA = /dialog|fala|voz|^tom$|tom.?de.?voz/i;
async function contextualizarFato(item, contextoDocumento) {
    // Fala/diálogo NUNCA passa por reescrita — nem essa, nem a de cima
    // (extrairLote já foi instruído a manter literal). O valor inteiro é
    // a citação exata; "autocontida" pra uma citação seria descaracterizar
    // o próprio exemplo que o RAG de voz existe pra aprender.
    if (CATEGORIA_DE_FALA.test(item.categoria || '')) return item.fato;
...
// `sempreIncluir` (novo): o servidor (spade-fs) já sabia persistir esse
// campo desde sempre — só nunca tinha sido ligado desse lado. Marcado
// assim, o conteúdo entra INTEIRO em toda cena do RP (compiledAjustesFixos-
// Block, mais abaixo), sem depender de busca por similaridade — pra
// direção de atuação (ritmo, quanto fala, slowburn) que não pode ficar
// competindo por espaço com lore/fatos e perder.
async function escreverArtefato({ slug, titulo, categoria, tags, conteudo, escopo = 'chat', sempreIncluir = false }) {
    const slugFinal = slug || slugifyLocal(titulo);
    const categoriaFinal = categoria || 'documento';
    const chatId = escopo === 'personagem' ? null : stChatIdAtual();
    const r = await chamarSpadeFs('/documento/escrever', { personagem: personagemAtual(), slug: slugFinal, titulo, categoria: categoriaFinal, tags: tags || [], conteudo, chatId, sempreIncluir });
    atualizarDocumentoAoVivo({ slug: slugFinal, titulo, conteudo });
    const chunks = await sincronizarDocumentoNaBiblioteca(slugFinal, titulo, categoriaFinal, conteudo, chatId);
    // resincroniza a rede de segurança nativa JÁ — se esse artefato for
    // sempreIncluir (ou deixou de ser), o próximo RP (inclusive um
    // regenerate/swipe, que nem passa pela interceptação da Spade) precisa
    // ver a mudança imediatamente, não só depois de trocar de chat/recarregar.
    sincronizarEspacoComNativo().catch(() => {});
    return Object.assign(r, { slug: slugFinal, chunks, escopo: chatId ? 'chat' : 'personagem' });
}
async function mesclarEmArtefato(categoria, fatosNovos, contextoDocumento) {
    ...
    // Fala/diálogo: NUNCA vira prosa — terceira vez que essa mesma reescrita
    // ("prosa organizada"/"documento inteiro") ameaçava mangle numa citação
    // literal (as outras duas: extrairLote e contextualizarFato, ver acima).
    // O valor de um exemplo de fala é a LISTA de citações exatas; junta com
    // o que já tinha, tira duplicata exata, sem passar por LLM nenhum —
    // menos uma chance de reescrever, mais rápido, mais barato.
    if (CATEGORIA_DE_FALA.test(categoria)) {
        const linhasExistentes = conteudoExistente.split('\n').map((l) => l.replace(/^[-•]\s*/, '').trim()).filter(Boolean);
        const todasLinhas = [...linhasExistentes];
        for (const f of fatosNovos) {
            const limpo = (f || '').trim();
            if (limpo && !todasLinhas.some((l) => l.toLowerCase() === limpo.toLowerCase())) todasLinhas.push(limpo);
        }
        const corpo = todasLinhas.map((l) => '- ' + l).join('\n');
    ...
    const prompt =
        ...
        (contextoDocumento.instrucaoUsuario ? 'O usuário pediu isso ao mandar o arquivo: "' + contextoDocumento.instrucaoUsuario + '" — o documento final precisa refletir esse pedido de verdade, não só resumir o arquivo cru.\n\n' : '') +
        (conteudoExistente
...
// ====================================
async function ingerirArquivo(file, onProgresso, instrucaoUsuario) {
    ...
    const contextoDocumento = { titulo: lido.titulo, tipo: lido.tipo, instrucaoUsuario: instrucaoUsuario || null };
...
async function ingerirArquivos(fileList, onProgresso, instrucaoUsuario) {
    const arquivos = Array.from(fileList);
    const resultados = [];
    for (const file of arquivos) {
        try { resultados.push(await ingerirArquivo(file, onProgresso, instrucaoUsuario)); }
...
// Espaço e RP são a MESMA cabeça, não dois chats com ponte entre eles —
// o que o usuário conversa/pede no Espaço entra direto aqui, sempre,
// pelas últimas mensagens de verdade (não um resumo, não um artefato
// que ela precisa lembrar de escrever). Sem flag pra marcar, sem tool
// pra chamar, sem categoria especial: é só a mesma conversa, vista dos
// dois lados. Pra algo sobreviver ALÉM dessa janela recente (virar
// traço permanente da personagem, não só contexto de agora), aí sim vale
// um Artefato normal — mas isso é sobre durar no tempo, não sobre "fazer
// a instrução valer" (ela já vale, só por ter sido dita).
//
// Extraído numa função à parte (sem log, sem moldura de prompt) porque
// tem DOIS consumidores: compiledEspacoRecenteBlock (a própria geração
// da Spade lê isso) E sincronizarEspacoComNativo (empurra pro mecanismo
// NATIVO do ST, setExtensionPrompt — ver logo abaixo o porquê de existir
// os dois caminhos em paralelo, não só um).
async function textoEspacoRecente(stChatId) {
    let chats = [];
    try { chats = await obterEspacoChats(); } catch (e) { return { texto: '', motivo: 'erro ao listar chats do Espaço: ' + e.message }; }
    if (!chats.length) return { texto: '', motivo: 'nenhum chat de Espaço existe ainda pra essa personagem' };

    let chat = stChatId ? chats.find((c) => c.stChatId === stChatId) : null;
    // FIX (portado): só cai pro fallback quando NEM SABEMOS o stChatId —
    // sabendo e não batendo, cair pro "mais recente" mesmo assim arrisca
    // pegar a conversa de Espaço de uma OUTRA história (agora que
    // garantirEspacoChatEscreve/resolverEspacoChatAtivo foram corrigidos
    // pra sempre linkar direito, esse caso devia ser raro — mas se
    // acontecer, é mais seguro admitir que não achou do que arriscar vazar
    // conteúdo de outro chat).
    const viaFallback = !chat && !stChatId;
    if (!chat && !stChatId) chat = chats.slice().sort((a, b) => b.atualizadoEm - a.atualizadoEm)[0];
    if (!chat) return { texto: '', motivo: stChatId ? 'stChatId "' + stChatId + '" não bate com nenhum Espaço linkado' : 'sem stChatId e nenhum Espaço existente pra usar de fallback' };

    let historico = [];
    try { historico = await estadoGet('espacoHistory:' + chat.id, []); } catch (e) { return { texto: '', motivo: 'erro ao ler histórico do chat ' + chat.id + ': ' + e.message }; }
    if (!historico.length) return { texto: '', motivo: 'chat "' + (chat.titulo || chat.id) + '" achado (' + (viaFallback ? 'fallback, sem stChatId batendo' : 'vínculo direto') + ') mas sem mensagens' };

    const recentes = historico.slice(-16);
    const texto = recentes.map((m) => (m.role === 'user' ? 'Usuário' : 'Você mesma') + ': ' + m.content).join('\n');
    return { texto, chat, viaFallback, n: recentes.length };
}
// ====================================
// TÉCNICA DE TURNO — REVERTIDA (v7.16.0). Existiu entre v7.14.0 e
// v7.15.0: inserir as últimas trocas do Espaço como mensagens de
// user/assistant DE VERDADE no array, bem antes da fala nova, em vez de
// só texto no system prompt (que compiledEspacoRecenteBlock, acima, já
// fazia e continua fazendo). A ideia (peso maior porque é "turno real",
// não "nota pra seguir") fazia sentido na teoria — mas testado ao vivo
// pela primeira vez, o resultado prático foi pior, não melhor: o
// personagem passou a vazar o próprio ajuste pedido no Espaço DENTRO da
// fala do RP. Motivo provável: a última coisa no array virava um turno
// "assistant" com conteúdo tipo "[Fora da cena...] beleza, vou ficar mais
// fria com ele" bem colado na hora de gerar a PRÓXIMA fala como
// personagem — pedir pro modelo "continue esse array" com esse tipo de
// conteúdo logo atrás é um convite a ecoar aquilo, não só absorver a
// intenção. Empilhado em cima do bloco de texto que já existia (que é
// mais seguro — instrução clara não convida a ser citada como se fosse
// fala), a duplicação só aumentava a chance de vazar sem ganhar nada que
// o bloco de texto (ordenado por último, peso alto de propósito) já não
// desse. Removida a função e a chamada; o bloco de texto continua sendo
// o único canal. Se quiser tentar de novo, provavelmente vale reformular
// o texto do turno pra NUNCA ficar parecido com fala/decisão em primeira
// pessoa — não só reativar como estava.
// ====================================
async function compiledEspacoRecenteBlock(stChatId) {
    // Log de diagnóstico (aba Pensamento, autor 'diagnostico') em TODO
    // caminho de saída — depois de reincidência sem conseguir reproduzir
    // ao vivo, adivinhar de novo não ajuda mais: isso deixa visível, pra
    // qualquer teste, se o bloco achou conversa ou não e por quê, em vez
    // de só "funcionou"/"não funcionou" sem rastro.
    const r = await textoEspacoRecente(stChatId);
    if (!r.texto) {
        journalAdicionar('[espaço→RP] ' + r.motivo, 'pensamento', 'diagnostico').catch(() => {});
        return '';
    }
    journalAdicionar('[espaço→RP] incluído (' + (r.viaFallback ? 'FALLBACK, stChatId não bateu' : 'vínculo direto') + '), ' + r.n + ' msg do chat "' + (r.chat.titulo || r.chat.id) + '"', 'pensamento', 'diagnostico').catch(() => {});
    return '[CONVERSA RECENTE COM O USUÁRIO, FORA DA CENA — é você mesma, não terceiro; o que ele pediu aqui vale AGORA na fala, sem precisar que ele repita dentro da cena]\n' + r.texto;
}
// ====================================
// AJUSTES FIXOS — direção de atuação permanente (ritmo, quanto fala,
// slowburn), marcada sempreIncluir num Artefato. Existe separado de
// compiledEspacoRecenteBlock por um motivo: a conversa recente do Espaço é
// uma JANELA QUE ROLA (últimas 16 msg, some com o tempo) — ótima pra "vale
// só nessa cena/agora", ruim pra "isso é assim daqui pra frente". Um
// Artefato sempreIncluir não rola: fica até alguém mudar. E ENTRA INTEIRO
// aqui, sem busca por similaridade (compiledBibliotecaBlock, mais abaixo,
// já faz isso pra artefato comum — mas busca é ranking, compete por espaço
// com lore/fatos, e é exatamente esse "competir e perder" que fazia
// direção de atuação (adjetivo vago, sem palavra-chave forte pra achar por
// embedding) sumir do prompt do RP mesmo já escrita).
// spade-fs já sabia persistir sempreIncluir desde sempre (/documento/
// escrever grava, /documento/listar devolve) — só nunca tinha sido lido
// desse lado.
// ====================================
// Leitura direta e simples de UM documento bem conhecido — usada pela tool
// direcao_ler (mostrar pra ela mesma o que já tá definido, sem escanear a
// biblioteca toda). compiledAjustesFixosBlock (abaixo) continua fazendo a
// varredura genérica por sempreIncluir na hora de montar o prompt do RP —
// isso aqui é só o atalho de leitura rápida do caso mais comum.
async function textoDirecaoDeAtuacao() {
    try {
        const r = await chamarSpadeFs('/documento/ler', { personagem: personagemAtual(), slug: 'direcao-de-atuacao', chatId: null });
        return { texto: (r?.conteudo || '').trim() };
    } catch (e) {
        return { texto: '' };
    }
}
async function textoAjustesFixos() {
    let documentos = [];
    try { documentos = (await chamarSpadeFs('/documento/listar', { personagem: personagemAtual(), chatId: stChatIdAtual() })).documentos || []; }
    catch (e) { return { texto: '', motivo: 'erro ao listar artefatos: ' + e.message }; }
    const fixos = documentos.filter((d) => d.sempreIncluir);
    if (!fixos.length) return { texto: '', motivo: 'nenhum artefato marcado sempreIncluir' };
    const corpos = [];
    for (const doc of fixos) {
        try {
            const r = await chamarSpadeFs('/documento/ler', { personagem: personagemAtual(), slug: doc.slug, chatId: doc.escopo === 'chat' ? stChatIdAtual() : null });
            if (r.conteudo) corpos.push('— ' + (r.titulo || doc.slug) + ' —\n' + r.conteudo.trim());
        } catch (e) { console.warn('[ajustes fixos] ler "' + doc.slug + '" falhou:', e.message); }
    }
    if (!corpos.length) return { texto: '', motivo: fixos.length + ' marcado(s) sempreIncluir mas nenhum leu com sucesso' };
    return { texto: corpos.join('\n\n'), n: corpos.length };
}
async function compiledAjustesFixosBlock() {
    const r = await textoAjustesFixos();
    if (!r.texto) {
        journalAdicionar('[ajustes fixos→RP] ' + r.motivo, 'pensamento', 'diagnostico').catch(() => {});
        return '';
    }
    journalAdicionar('[ajustes fixos→RP] incluído(s) ' + r.n + ' artefato(s) sempreIncluir', 'pensamento', 'diagnostico').catch(() => {});
    return '[DIREÇÃO DE ATUAÇÃO PERMANENTE — vale sempre, toda cena, até alguém mudar isso; não é sugestão de uma vez só]\n' + r.texto;
}
// ====================================
// SINCRONIA AUTOMÁTICA DE AJUSTE — v7.10.0 resolvia só "direção de
// atuação" sozinha, sem depender da Espaço chamar tool. Generalizado
// agora pros outros destinos que existiam mas ainda dependiam do mesmo
// julgamento de conversa pra funcionar:
// - direção de atuação (regra geral de COMO atua sempre) — igual antes.
// - sistema (regra CONDICIONAL, só vale quando algo aparece na cena) —
//   Sistemas.criar direto, sem passar por sistema_criar.
// - fato/traço (personalidade, sentimento, relacionamento, preferência,
//   lore) — mesmo mesclarEmArtefato que a Ingestão de arquivo já usa,
//   reaproveitado aqui pra texto batido no Espaço.
// UMA chamada só classifica entre os quatro (os três acima + "nenhuma"),
// não uma por tipo — mais barato e evita puxar pro tipo errado por
// rodar em paralelo sem se ver. "Sem dúvida real" prefere "nenhuma":
// falso negativo (não guardou) é recuperável perguntando de novo; falso
// positivo (virou sistema errado, ou artefato com fato que não foi bem
// isso) é mais chato de limpar depois.
// ====================================
async function sincronizarAjusteAutomatico(mensagemUsuario) {
    if (!mensagemUsuario || !mensagemUsuario.trim()) return;
    const { modeloEscritor } = getConfig();
    const direcaoAtual = (await textoDirecaoDeAtuacao()).texto;
    const prompt =
        (direcaoAtual ? 'Direção de atuação já definida pro personagem:\n"""\n' + direcaoAtual.slice(0, 3000) + '\n"""\n\n' : 'Nenhuma direção de atuação definida ainda.\n\n') +
        'Mensagem nova do usuário, numa conversa fora da cena (o "Espaço", tipo bastidor) com o assistente que ' +
        'ajusta a extensão de RP:\n"' + mensagemUsuario.slice(0, 1200) + '"\n\n' +
        'Classifique em UM desses quatro tipos e responda só JSON (nada antes/depois):\n' +
        '- "direcao" — regra GERAL de como o personagem atua sempre daqui pra frente (ritmo de fala, quanto fala, ' +
        'slowburn, tom, asterisco, o que evita). {"tipo":"direcao","texto":"direção de atuação completa — MANTENHA ' +
        'toda regra específica que já existia (não resuma/generalize o que já tinha pra abrir espaço pro novo) e ' +
        'ACRESCENTE a mudança pedida agora com o MESMO nível de detalhe concreto que ela foi pedida (se o usuário ' +
        'deu um exemplo ou uma palavra específica, essa palavra entra no texto, não uma paráfrase vaga dela)"}\n' +
        '- "sistema" — regra CONDICIONAL, só vale quando algo específico aparecer na cena (não é comportamento de ' +
        'sempre). {"tipo":"sistema","nome":"curto","quando":"palavra/frase-gatilho simples que apareceria na cena ' +
        '(deixe \\"\\" se for pra valer sempre mesmo, sem gatilho)","entao":"o que lembrar/fazer quando ativar"}\n' +
        '- "fato" — informação sobre o personagem ou o mundo: personalidade, sentimento/estado emocional atual, ' +
        'relacionamento, preferência, lore, aparência, ou qualquer outra coisa que não seja regra de atuação nem ' +
        'gatilho condicional. {"tipo":"fato","categoria":"sua escolha livre e curta pro assunto — não precisa ser ' +
        'uma das básicas (personalidade, sentimento, relacionamento...), invente uma se o assunto for mais ' +
        'específico que isso","texto":"o fato, reescrito claro e autocontido"}\n' +
        '- "nenhuma" — só conversa, pergunta, pedido de ação direta (editar/apagar fala do RP), ou não muda nada ' +
        'do que já existe. {"tipo":"nenhuma"}\n\n' +
        'Na dúvida real entre dois tipos, ou se o pedido já parece resolvido pelo que já existe acima, responda ' +
        '"nenhuma".';
    let resposta;
    try {
        resposta = await generate(modeloEscritor, [{ role: 'user', content: prompt }], { maxTokens: 700, temperature: 0.3, timeoutMs: 15000 });
    } catch (e) { console.warn('[ajuste automático] classificação falhou:', e.message); return; }
    const obj = parseJsonObjetoSeguro(resposta);
    if (!obj || !obj.tipo || obj.tipo === 'nenhuma') return;

    if (obj.tipo === 'direcao' && obj.texto) {
        try {
            await escreverArtefato({ slug: 'direcao-de-atuacao', titulo: 'Direção de atuação', categoria: 'direcao', conteudo: obj.texto, escopo: 'personagem', sempreIncluir: true });
            journalAdicionar('[ajuste automático] direção de atuação atualizada sozinha, sem tool', 'pensamento', 'diagnostico').catch(() => {});
            sincronizarAjustesFixosNativo(obj.texto);
        } catch (e) { console.warn('[ajuste automático] escrever direção falhou:', e.message); }
        return;
    }
    if (obj.tipo === 'sistema' && obj.nome && obj.entao) {
        try {
            await Sistemas.criar({ nome: String(obj.nome).trim(), quando: obj.quando ? String(obj.quando).trim() : '', entao: String(obj.entao).trim() });
            journalAdicionar('[ajuste automático] sistema "' + obj.nome + '" criado sozinho, sem tool', 'pensamento', 'diagnostico').catch(() => {});
        } catch (e) { console.warn('[ajuste automático] criar sistema falhou:', e.message); }
        return;
    }
    if (obj.tipo === 'fato' && obj.texto) {
        const categoria = (obj.categoria ? String(obj.categoria) : 'outro').toLowerCase().trim().replace(/\s+/g, '_');
        try {
            await mesclarEmArtefato(categoria, [String(obj.texto).trim()], { titulo: 'conversa no Espaço' });
            journalAdicionar('[ajuste automático] artefato "' + categoria + '" atualizado sozinho, sem tool', 'pensamento', 'diagnostico').catch(() => {});
        } catch (e) { console.warn('[ajuste automático] mesclar artefato falhou:', e.message); }
        return;
    }
}
// ====================================
// REDE DE SEGURANÇA NATIVA — tudo acima (compiledEspacoRecenteBlock)
// só tem efeito se Spade_interceptGeneration realmente rodar naquela
// geração. Se por QUALQUER motivo a interceptação não disparar (hook não
// registrado direito, erro silencioso antes de chegar lá, geração vindo
// de um caminho que a Spade não cobre), o prompt dela nunca é montado —
// e aí não existe reforço de texto que resolva, porque o texto novo
// também não seria lido. Em vez de continuar só torcendo pra
// interceptação rodar, empurra o mesmo conteúdo TAMBÉM pelo mecanismo
// embutido do próprio SillyTavern (setExtensionPrompt) — o mesmo que a
// extensão de Memória/Resumo dele usa. Isso é nativo do app, não passa
// pelo código da Spade: se a interceptação falhar, o ST ainda injeta
// isso sozinho na hora de montar o prompt dele. Se a interceptação
// funcionar normalmente, isso fica ocioso (Spade_interceptGeneration
// aborta a geração nativa antes dela usar isso) — custo zero, cobertura
// a mais.
// ====================================
function sincronizarExtensionPromptNativo(texto) {
    try {
        if (typeof ctx().setExtensionPrompt !== 'function') return;
        const EXTENSION_PROMPT_IN_CHAT = 1, EXTENSION_PROMPT_ROLE_SYSTEM = 0;
        const conteudo = texto ? '[Nota — conversa recente com o usuário fora da cena; vale agora, sem precisar repetir]\n' + texto : '';
        ctx().setExtensionPrompt('SPADE_ESPACO_RECENTE', conteudo, EXTENSION_PROMPT_IN_CHAT, 0, false, EXTENSION_PROMPT_ROLE_SYSTEM);
    } catch (e) { console.warn('[espaço→RP nativo] setExtensionPrompt falhou:', e.message); }
}
// Mesma rede de segurança nativa, pro bloco de ajustes fixos — chave
// separada (SPADE_AJUSTES_FIXOS) pra não pisar no texto do Espaço recente;
// setExtensionPrompt aceita vários slots independentes.
function sincronizarAjustesFixosNativo(texto) {
    try {
        if (typeof ctx().setExtensionPrompt !== 'function') return;
        const EXTENSION_PROMPT_IN_CHAT = 1, EXTENSION_PROMPT_ROLE_SYSTEM = 0;
        const conteudo = texto ? '[Direção de atuação permanente — vale sempre, toda cena]\n' + texto : '';
        ctx().setExtensionPrompt('SPADE_AJUSTES_FIXOS', conteudo, EXTENSION_PROMPT_IN_CHAT, 0, false, EXTENSION_PROMPT_ROLE_SYSTEM);
    } catch (e) { console.warn('[ajustes fixos nativo] setExtensionPrompt falhou:', e.message); }
}
// Mesma rede de segurança nativa, pro bloco de Repetição — chave separada
// (SPADE_REPETICAO). Isso é o que regenerate/swipe (caminho nativo, que
// Spade_interceptGeneration pula de propósito) estava sem NENHUMA
// cobertura: sem esse slot, repetir regenerate repetia a MESMA fala
// porque nada avisava o modelo nativo do que já tinha sido dito — a lista
// de "evite repetir" só existia dentro do pipeline da própria Spade
// (runCantos), que regenerate nunca chama.
function sincronizarRepeticaoNativo() {
    try {
        if (typeof ctx().setExtensionPrompt !== 'function') return;
        const EXTENSION_PROMPT_IN_CHAT = 1, EXTENSION_PROMPT_ROLE_SYSTEM = 0;
        const bloco = compiledRepeticaoBlock(chatRealRecente(18).filter((m) => !m.ehUsuario).slice(-6).map((m) => m.texto));
        ctx().setExtensionPrompt('SPADE_REPETICAO', bloco, EXTENSION_PROMPT_IN_CHAT, 0, false, EXTENSION_PROMPT_ROLE_SYSTEM);
    } catch (e) { console.warn('[repetição nativo] setExtensionPrompt falhou:', e.message); }
}
// Mesma rede, pra instrução anti-vazamento/anti-raciocínio-visível — SEM
// essa, regenerate/swipe nativo não tem NENHUMA instrução dizendo pro
// modelo responder só como a personagem, sem "pensar alto"/meta. Achado
// concreto: um screenshot mostrando o widget nativo "Thought for X
// seconds" do próprio ST (só aparece quando SillyTavern MESMO — não a
// Spade — detecta e separa reasoning_content em extra.reasoning) prova
// que aquele vazamento específico veio do caminho nativo, não do
// pipeline da Spade (que já filtra <think> desde a v7.18.0, mas só
// dentro das PRÓPRIAS chamadas — regenerate nunca passa por elas). Isso
// não resolve 100% (se o backend manda reasoning_content num campo
// separado, prompt nenhum evita isso — é comportamento da API, não do
// texto), mas ajuda a parte que É influenciável por instrução: o modelo
// escrever ESTILO "pensando alto" dentro do content de verdade.
function sincronizarInstrucaoAntiVazamentoNativo() {
    try {
        if (typeof ctx().setExtensionPrompt !== 'function') return;
        const EXTENSION_PROMPT_IN_CHAT = 1, EXTENSION_PROMPT_ROLE_SYSTEM = 0;
        const charName = resolveCharacterName();
        const bloco = '[Responda SEMPRE só como ' + charName + ', em prosa de RP normal — nunca liste passos, nunca ' +
            'narre raciocínio ("vou variar tal frase", "já usei isso antes"), nunca mostre planejamento ou notas ' +
            'pra si mesma. Se pensar antes de responder, esse pensamento não aparece pro usuário — só a fala final.]';
        ctx().setExtensionPrompt('SPADE_ANTI_VAZAMENTO', bloco, EXTENSION_PROMPT_IN_CHAT, 0, false, EXTENSION_PROMPT_ROLE_SYSTEM);
    } catch (e) { console.warn('[anti-vazamento nativo] setExtensionPrompt falhou:', e.message); }
}
async function sincronizarEspacoComNativo() {
    const r = await textoEspacoRecente(stChatIdAtual());
    sincronizarExtensionPromptNativo(r.texto || '');
    const a = await textoAjustesFixos();
    sincronizarAjustesFixosNativo(a.texto || '');
    sincronizarRepeticaoNativo();
    sincronizarInstrucaoAntiVazamentoNativo();
    // Log de diagnóstico também aqui — compiledEspacoRecenteBlock/
    // compiledAjustesFixosBlock só rodam quando a Spade intercepta a
    // geração (não roda em regenerate/swipe, de propósito). Essa função
    // roda sempre, então é o único lugar que dá pra confirmar visualmente
    // que a rede nativa (setExtensionPrompt) tá com o conteúdo certo ANTES
    // de qualquer geração acontecer — inclusive pra regenerate/swipe.
    journalAdicionar('[rede nativa] espaço: ' + (r.texto ? (r.n + ' msg') : 'vazio (' + r.motivo + ')') + ' · ajustes fixos: ' + (a.texto ? 'presente' : 'vazio'), 'pensamento', 'diagnostico').catch(() => {});
}
async function compiledBibliotecaBlock(sceneText) {
    const chatId = stChatIdAtual();
    const [vozes, memorias] = await Promise.all([
        Biblioteca.buscarVoz(sceneText, { k: 3, chatId }),
        Biblioteca.buscarMemoria(sceneText, { k: 6, chatId }),
    ]);
...
async function runCantos(userMessage) {
    ...
    const [bibliotecaResult, repeticaoResult, estadoResult, mundoResult, sistemasResult, sistemasReaisResult, espacoRecenteResult, ajustesFixosResult] = await Promise.allSettled([
        ...
        compiledEspacoRecenteBlock(stChatIdAtual()),
        compiledAjustesFixosBlock(),
    ]);

    const cantos = {};
    cantos.biblioteca = fromSettled(bibliotecaResult, 'biblioteca', '');
    cantos.espacoRecente = fromSettled(espacoRecenteResult, 'espaco_recente', '');
    cantos.ajustesFixos = fromSettled(ajustesFixosResult, 'ajustes_fixos', '');
...
async function buildSystemPrompt(cantos) {
    ...
    const parts = [
        ...
        // ajustesFixos e espacoRecente vão por ÚLTIMO de propósito, logo
        // antes da instrução final — é o que tem mais chance de precisar
        // valer JÁ, e quanto mais perto do ponto de gerar, mais peso ganha.
        // No topo, competia com biblioteca/estado/mundo no meio e perdia.
        cantos.ajustesFixos?.value,
        cantos.espacoRecente?.value,
        'Você é a ' + charName + ' — só ela, agora. Nada de ferramenta, nada de passo, nada de "agente" aqui: é ela ' +
        'falando de verdade. O que apareceu acima (biblioteca, direção de atuação, conversa do Espaço) é contexto ' +
        'PRA VOCÊ USAR — nunca cite/repita esses blocos, colchetes ou o pedido do usuário em palavras parecidas com ' +
        'as de lá; a fala sai só como ela mesma falaria, sem rastro de onde a informação veio. Responda curto, ' +
        'direto, em pt-br, coerente com o que veio acima — poucas frases, sem ' +
        'engordar a fala com narração ou explicação que ninguém pediu. Asterisco é gesto/pensamento pontual, não ' +
        'parágrafo de narração — se a cena não pede, não usa; se usa, é uma linha curta, não um parágrafo. Se ficar ' +
        'em dúvida entre uma versão mais longa e uma mais curta da mesma fala, escolha a curta.' +
        (cantos.ajustesFixos?.value ? ' A DIREÇÃO DE ATUAÇÃO PERMANENTE (acima) não é textura de fundo pra ' +
            'lembrar de vez em quando — é regra ativa nesta fala específica que você tá prestes a escrever, agora.' : '') +
        (cantos.espacoRecente?.value ? ' A CONVERSA RECENTE COM O USUÁRIO (bem acima desta linha) é a última ' +
            'checagem antes de falar, não um detalhe do começo do prompt pra esquecer — se ela pediu algo de ' +
            'ritmo/tamanho/tom, essa fala que você tá prestes a escrever é onde isso precisa aparecer, agora, não ' +
            'na próxima depois de "confirmar".' : ''),
    ];
...
self.onmessage = async (ev) => {
    ...
    const sdk = {
        ...
        sistemas: {
            // Composição real — um sistema pode chamar outro sistema JÁ
            // ATIVO pelo nome da família e reaproveitar o resultado dele,
            // em vez de reimplementar a mesma lógica duas vezes (mesmo
            // espírito de "biblioteca de skills reusáveis" da literatura de
            // agentes: skills compostas, não só uma pilha achatada de
            // scripts soltos). Profundidade/auto-chamada são limitadas do
            // lado de fora (rodarSistemaSandbox), não aqui.
            chamar: (familia, input) => _rpc('sistemas.chamar', { familia, input }),
        },
    };
...
async function rodarSistemaSandbox(sistemaId, codigo, hookNome, input, { timeoutMs = 8000, timeoutDuroMs = 12000, familiaAtual = null, profundidade = 0 } = {}) {
    const LIMITE_PROFUNDIDADE = 2; // composição real, mas com teto — evita A chama B chama A em loop, e evita custo (1 Worker por nível) descontrolado
        ...
        worker.onmessage = async (ev) => {
            ...
            if (msg.tipo === 'rpc') {
                ...
                try {
                    ...
                    else if (msg.chamada === 'sistemas.chamar') {
                        // Composição: orquestrada aqui no thread principal
                        // (que já controla rodarSistemaSandbox), não como
                        // Worker-dentro-de-Worker — mais simples e sem risco
                        // de deadlock. Cada nível reduz o teto de tempo pela
                        // metade, então o pior caso (2 níveis) ainda tem
                        // limite total previsível, não N vezes o teto cheio.
                        if (profundidade >= LIMITE_PROFUNDIDADE) throw new Error('sdk.sistemas.chamar: profundidade máxima (' + LIMITE_PROFUNDIDADE + ') atingida — composição não empilha mais que isso.');
                        const familiaAlvo = msg.payload.familia;
                        if (familiaAlvo === familiaAtual) throw new Error('sdk.sistemas.chamar: um sistema não pode chamar a própria família ("' + familiaAlvo + '") — isso é auto-chamada, não composição.');
                        const alvo = (await SistemasReais.ativos()).find((s) => s.familia === familiaAlvo);
                        if (!alvo) throw new Error('sdk.sistemas.chamar: nenhum sistema ATIVO com família "' + familiaAlvo + '" (sistema inativo/inexistente não pode ser chamado).');
                        const sub = await rodarSistemaSandbox(alvo.id, alvo.codigo, alvo.hookAlvo, msg.payload.input ?? {}, {
                            timeoutMs: Math.max(1500, Math.floor(timeoutMs / 2)),
                            timeoutDuroMs: Math.max(2500, Math.floor(timeoutDuroMs / 2)),
                            familiaAtual: familiaAlvo, profundidade: profundidade + 1,
                        });
                        if (!sub.ok) throw new Error('sistema "' + alvo.nome + '" (chamado via composição) falhou: ' + sub.erro);
                        valor = sub.resultado;
                    }
...
const SistemasReais = {
    ...
    async publicar({ familia, nome, hookAlvo, codigo, autor, entradaExemplo }) {
        const teste = await rodarSistemaSandbox('pre-publicacao-' + newId(), codigo, hookAlvo, entradaExemplo ?? {}, { familiaAtual: familia });
        ...
        indexarSistemaParaBusca(entry).catch(() => {}); // não bloqueia o retorno — é só pra reuso futuro
...
// ====================================
// BUSCA DE SISTEMAS PARECIDOS — "skill library" reusável (mesmo princípio
// do Voyager: recuperar comportamento já existente antes de escrever um
// novo do zero, pra compor/estender em vez de duplicar). Não é estrutura
// nova: indexa cada sistema publicado na MESMA Biblioteca (busca híbrida
// já testada) que o resto da extensão usa, só com tipo próprio — igual
// aprendizado_sistema já faz pro loop testar→corrigir.
// ====================================
async function indexarSistemaParaBusca(entry) {
    const trecho = (entry.codigo || '').replace(/\s+/g, ' ').trim().slice(0, 300);
    await Biblioteca.escrever({
        tipo: 'sistema_real_indice',
        texto: `Sistema "${entry.nome}" (família "${entry.familia}", hookAlvo "${entry.hookAlvo}"): ${trecho}`,
        personagem: ESCOPO_AUTOESTUDO,
        importancia: 0.5,
        metadata: { familia: entry.familia, sistemaId: entry.id, hookAlvo: entry.hookAlvo, versaoSDK: SDK_VERSAO },
    });
}
async function buscarSistemasParecidos(descricao, k = 5) {
    try {
        const encontrados = await Biblioteca.buscarMemoria(descricao, { tipos: ['sistema_real_indice'], personagem: ESCOPO_AUTOESTUDO, k });
        return encontrados.map((e) => e.texto);
    } catch (e) { console.warn('[skill-library] busca de sistemas parecidos falhou:', e.message); return []; }
}

// Roda os sistemas reais ATIVOS que miram um hookAlvo específico, com o
// input que esse hook fornece — generalizado pra servir tanto antesDeGerar
// (RP, sceneText) quanto antesDePensar (Sala de Pensamento, ver mais
// abaixo) sem duplicar a mesma lógica de Promise.allSettled/filtro/rótulo
// duas vezes. Sistema com erro some do bloco silenciosamente (só loga) —
// um bug num sistema não pode derrubar a rodada inteira, seja ela qual for.
async function avaliarSistemasReaisHook(hookAlvo, input, rotuloBloco) {
    const ativos = (await SistemasReais.ativos()).filter((s) => s.hookAlvo === hookAlvo);
    if (!ativos.length) return '';
    const resultados = await Promise.allSettled(
        ativos.map((s) => rodarSistemaSandbox(s.id, s.codigo, s.hookAlvo, input, { familiaAtual: s.familia }))
    );
    const linhas = [];
    resultados.forEach((r, i) => {
        if (r.status === 'fulfilled' && r.value.ok && typeof r.value.resultado === 'string' && r.value.resultado.trim()) {
            ...
            console.warn('[sistemas_reais] "' + ativos[i].nome + '" (' + hookAlvo + ') falhou:', r.value.erro);
        } else if (r.status === 'rejected') {
            console.warn('[sistemas_reais] "' + ativos[i].nome + '" (' + hookAlvo + ') rejeitou:', r.reason);
        }
    });
    if (!linhas.length) return '';
    return '[' + rotuloBloco + ' — código de verdade, rodou agora]\n' + linhas.join('\n');
}
async function avaliarSistemasReais(sceneText) {
    return avaliarSistemasReaisHook('antesDeGerar', { sceneText }, 'SISTEMAS REAIS ATIVOS');
}
...
// v2: dois hookAlvo reais agora (antesDeGerar + antesDePensar), e
// sdk.sistemas.chamar — composição de verdade entre sistemas.
const SDK_VERSAO = 2;
...
retorno de aoAtivar vira o resultado do sistema (string curta; pode ser
null/undefined se decidir não agir).

hookAlvo — dois são chamados de verdade hoje:
  "antesDeGerar": toda rodada de RP, input = { sceneText: string }.
  "antesDePensar": toda rodada da Sala de Pensamento (loop autônomo dela,
    não o RP), input = { ultimosPensamentos: string, cenaRecente: string }
    — serve pra sistema que precisa reagir ao que ELA andou pensando/
    fazendo sozinha, não à cena em si (ex: "faz N pensamentos sem tocar
    em X, sugere Y"). Outros nomes de hookAlvo são aceitos por
    sistema_real_testar/publicar mas ficam guardados inertes.
...
sdk.sistemas.chamar(familia, input) -> Promise<resultado do sistema chamado>
  COMPOSIÇÃO de verdade — chama outro sistema JÁ ATIVO (por família) e usa
  o resultado dele, em vez de duplicar a mesma lógica. Regras reais, não
  cosméticas: só alcança sistema com ativo=true (rascunho/inativo não
  aparece pra composição); um sistema NÃO pode chamar a própria família
  (erro claro se tentar); profundidade máxima 2 (A chama B chama C tá ok,
  A chama B chama A não roda — erro claro); cada nível usa metade do teto
  de tempo do nível anterior, pra um pior caso não virar N vezes o tempo
  cheio. Se o sistema chamado falhar, o erro sobe pra você tratar, não
  falha em silêncio.

Timeout: ~8s de teto mole no nível 0 (erro claro se estourar); ~12s de
teto duro (Worker morto de fora, sem chance de resposta depois disso) —
não vale escrever aoAtivar que dependa de I/O demorado, ele não tem I/O
disponível de qualquer forma.

Antes de escrever do zero, sistema_real_buscar_similar busca nos sistemas
JÁ publicados por descrição — pra reaproveitar/compor com sdk.sistemas.
chamar em vez de reinventar algo parecido que já existe.
...
Exemplo com composição (hookAlvo "antesDePensar", chamando outro sistema
já ativo de família "contador-tensao"):
  async function aoAtivar(input, sdk) {
    const tensao = await sdk.sistemas.chamar('contador-tensao', { ultimosPensamentos: input.ultimosPensamentos });
    if (tensao && Number(tensao) > 7) return 'tensão alta há um tempo, talvez valha uma pausa na cena';
    return null;
  }
`.trim();
...
const TOOLS = [
    ...
    { type: 'function', function: { name: 'sistema_real_testar', description: 'Testa um sistema de CÓDIGO DE VERDADE (diferente de sistema_criar — isso roda JS de verdade, isolado num sandbox) sem publicar nada. O código deve definir uma função aoAtivar(input, sdk) — sdk.estado.ler/escrever, sdk.biblioteca.escrever/buscar e sdk.sistemas.chamar (compor com outro sistema já ativo) são a ÚNICA forma de tocar a extensão de dentro do sistema. Se der erro, o erro real volta pra você — corrija e teste de novo até rodar limpo antes de publicar.', parameters: { type: 'object', properties: { codigo: { type: 'string', description: 'Corpo definindo aoAtivar(input, sdk). Ex: "function aoAtivar(input, sdk) { return input.sceneText.includes(\'chuva\') ? \'está chovendo, mencione isso\' : null; }"' }, hookAlvo: { type: 'string', description: '"antesDeGerar" (input {sceneText}, toda rodada de RP) e "antesDePensar" (input {ultimosPensamentos, cenaRecente}, toda rodada da Sala de Pensamento) são chamados de verdade; outros nomes ficam guardados mas inertes.' }, entradaExemplo: { type: 'object', description: 'Input de teste, ex: {"sceneText": "..."}.' } }, required: ['codigo', 'hookAlvo'] } } },
    { type: 'function', function: { name: 'sistema_real_publicar', description: 'Salva uma versão de um sistema de código de verdade que já testou limpo com sistema_real_testar. Roda 1x antes de salvar — se falhar, não salva nada. IMPORTANTE: publicar NÃO ativa sozinho — a versão fica guardada, inativa, até você chamar sistema_real_ativar. Isso separa "compila e roda" de "está no ar". Depois de publicado, o sistema fica buscável por sistema_real_buscar_similar (pra outras sessões reaproveitarem/comporem em vez de duplicar).', parameters: { type: 'object', properties: { familia: { type: 'string', description: 'Identificador estável do sistema — use o MESMO valor em edições futuras dele, pra versionar em vez de duplicar.' }, nome: { type: 'string' }, hookAlvo: { type: 'string' }, codigo: { type: 'string' }, entradaExemplo: { type: 'object' } }, required: ['familia', 'nome', 'hookAlvo', 'codigo'] } } },
    ...
    { type: 'function', function: { name: 'sistema_real_buscar_similar', description: 'Busca semântica nos sistemas de código JÁ PUBLICADOS por descrição do que você quer fazer — antes de escrever algo do zero, vale ver se já existe algo parecido pra reaproveitar ou compor com sdk.sistemas.chamar (chamar um sistema ativo de dentro de outro), em vez de duplicar lógica.', parameters: { type: 'object', properties: { descricao: { type: 'string', description: 'O que você quer que o sistema faça, em linguagem natural.' } }, required: ['descricao'] } } },
    ...
    { type: 'function', function: { name: 'artefato_listar', description: 'Lista os Artefatos (documentos de referência pro RP) já guardados — inclui os do personagem inteiro e os específicos desse chat.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'artefato_buscar', description: 'Busca semântica nos Artefatos — usa quando quiser achar algo sem saber o slug exato (ex: "como ela fala quando tá brava"), incluindo o que o usuário citou de passagem na conversa.', parameters: { type: 'object', properties: { consulta: { type: 'string' }, k: { type: 'number' } }, required: ['consulta'] } } },
    ...
    { type: 'function', function: { name: 'direcao_ler', description: 'Lê a direção de atuação permanente atual — o único lugar pra ritmo/tom/comportamento que valem em toda cena a partir de agora (ex: "fala pouco quando a cena pede pouco", "slowburn"). Vazio se nada foi definido ainda. Leia antes de direcao_definir se for ajustar em vez de substituir do zero.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'direcao_definir', description: 'Define a direção de atuação permanente inteira — substitui o texto todo (não é lista incremental; se for ajustar algo que já existe, leia com direcao_ler primeiro e mande o texto completo já com a mudança). Isso entra INTEIRO em toda fala do RP a partir de agora, garantido, sem depender de busca — é pra ritmo de fala, quanto ela fala, slowburn, "não avança sem eu levar a cena", esse tipo de regra de COMO ela atua. Não é pra fato/lore (isso é artefato_escrever normal). Chame vazio ("") pra apagar tudo e voltar ao comportamento padrão.', parameters: { type: 'object', properties: { texto: { type: 'string' } }, required: ['texto'] } } },
    { type: 'function', function: { name: 'artefato_escrever', description: 'Cria ou reescreve um Artefato inteiro — o material curado, já organizado e bem escrito (não é despejo do texto original). Isso sincroniza direto com a busca usada no RP de verdade — o que você escrever aqui pode aparecer na cena. Chame artefato_ler antes se for editar um que já existe, pra não perder conteúdo. Escreva com qualidade de verdade: prosa clara, organizada, rica o suficiente pra colorir um diálogo de RP — não uma lista seca de fragmentos soltos. EXCEÇÃO — categoria de fala/tom/exemplo_dialogo: aí é o oposto, NÃO vira prosa, fica uma lista de citações LITERAIS (palavra por palavra) de como ela fala; reescrever/parafrasear uma fala destrói o próprio exemplo.', parameters: { type: 'object', properties: { slug: { type: 'string', description: 'Se omitido num artefato NOVO, invente um curto a partir do título.' }, titulo: { type: 'string' }, categoria: { type: 'string', description: 'Livre — ex: tom, lore, regra_mundo, exemplo_dialogo, aparencia, relacionamento, usuario. Invente uma se nenhuma encaixar. Categoria de fala/tom/dialogo entra no slot de busca de voz reservado e o conteúdo deve ser citação literal, não prosa.' }, tags: { type: 'array', items: { type: 'string' } }, conteudo: { type: 'string' }, escopo: { type: 'string', enum: ['chat', 'personagem'], description: 'Padrão "chat": só essa conversa enxerga. "personagem": compartilhado com toda conversa desse personagem — use pra tom/personalidade central, não pra detalhe de uma história específica.' }, sempreIncluir: { type: 'boolean', description: 'Padrão false — o conteúdo entra por busca por similaridade, como qualquer artefato (compete por espaço com outros, pode não aparecer se o pedido for vago/sem palavra-chave forte). Marque true SÓ pra direção de atuação permanente: ritmo de fala, quanto ela fala por padrão, slowburn, "não avança fisicamente sem eu levar a cena até lá" — regra de COMO ela atua, não fato de lore. Marcado assim, entra INTEIRO em toda cena do RP, garantido, sem depender de busca. Não abuse — é pra poucas regras centrais, não pra guardar tudo.' } }, required: ['titulo', 'conteudo'] } } },
...
async function executeTool(name, args) {
    switch (name) {
        ...
        case 'sistema_real_buscar_similar': { const achados = await buscarSistemasParecidos(args.descricao); return { achados: achados.length ? achados : ['(nada parecido publicado ainda)'] }; }
        ...
        case 'artefato_listar': return chamarSpadeFs('/documento/listar', { personagem: personagemAtual(), chatId: stChatIdAtual() });
        ...
        case 'artefato_ler': return chamarSpadeFs('/documento/ler', { personagem: personagemAtual(), slug: args.slug, chatId: stChatIdAtual() });
        case 'direcao_ler': {
            const r = await textoDirecaoDeAtuacao();
            return { conteudo: r.texto || '(vazio — nenhuma direção definida ainda, comportamento padrão)' };
        }
        case 'direcao_definir': return escreverArtefato({ slug: 'direcao-de-atuacao', titulo: 'Direção de atuação', categoria: 'direcao', conteudo: args.texto || ' ', escopo: 'personagem', sempreIncluir: true });
        case 'artefato_escrever': {
            const r = await escreverArtefato({ slug: args.slug, titulo: args.titulo, categoria: args.categoria || 'documento', tags: args.tags || [], conteudo: args.conteudo, escopo: args.escopo || 'chat', sempreIncluir: !!args.sempreIncluir });
            return Object.assign(r, { sincronizadoNaBiblioteca: r.chunks + ' trecho(s) — já buscável no RP de verdade agora.' });
        }
        case 'artefato_apagar': {
            const r = await chamarSpadeFs('/documento/apagar', { personagem: personagemAtual(), slug: args.slug, chatId: stChatIdAtual() });
...
const ACTION_LABEL = {
    ...
    sistema_real_buscar_similar: '🔎 buscando sistema parecido já publicado',
...
const TOOLS_CONSTRUTORA_NOMES = ['sistema_real_testar', 'sistema_real_publicar', 'sistema_real_ativar', 'sistema_real_listar', 'sistema_real_buscar_similar', 'artefato_buscar', 'estudar_codigo', 'extensao_listar_arquivos', 'extensao_ler_arquivo', 'extensao_editar_arquivo', 'extensao_restaurar_backup'];
...
async function passoConstrutora(atividade, instrucao) {
    ...
    const systemPrompt =
        ...
        'de tentar.\n\n' +
        'Dois hookAlvo são chamados de verdade: "antesDeGerar" (toda rodada de RP, recebe {sceneText}) e ' +
        '"antesDePensar" (toda rodada da Sala de Pensamento, recebe {ultimosPensamentos, cenaRecente}) — dá pra ' +
        'construir sistema que reage ao que ela pensa sozinha, não só à cena. sdk.sistemas.chamar(familia, input) ' +
        'compõe sistemas de verdade — um sistema ativo pode chamar outro sistema ativo e usar o resultado (limite de ' +
        '2 níveis, sem auto-chamada). Antes de escrever algo do zero, sistema_real_buscar_similar acha o que já foi ' +
        'publicado por descrição — reaproveitar/compor costuma valer mais que duplicar lógica parecida.\n\n' +
    ...
    const novoHistorico = [...historico,
        { role: 'assistant', content: resp.content || null, ...(resp.toolCalls?.length ? { tool_calls: resp.toolCalls } : {}), ...(resp.reasoningContent ? { reasoning_content: resp.reasoningContent } : {}) },
...
let opToken = 0;
// FIX (portado, ausente nessa linhagem): digitar na caixa nativa do RP
// chamava comecarAtividade('rp') em toda tecla, mesmo sem enviar nada — e
// comecarAtividade ABORTA a atividade atual quando o "onde" muda. Se o
// Espaço estava processando um pedido, só o usuário tocar/digitar na caixa
// do RP matava esse processamento no meio, sem aviso (por isso funcionava
// "depois de parar de digitar", nunca "durante"). `usuarioDigitandoRp` é
// um aviso leve e isolado — só diz ao Pensamento "não comece algo novo" —
// nunca aborta nada que já esteja rodando de verdade.
let usuarioDigitandoRp = false;
let timerDigitandoRp = null;
function marcarDigitandoRp(digitando) {
    usuarioDigitandoRp = digitando;
    if (timerDigitandoRp) clearTimeout(timerDigitandoRp);
    timerDigitandoRp = digitando ? setTimeout(() => { usuarioDigitandoRp = false; }, 4000) : null;
}
...
// Libera a trava de 'rp' por TIPO, não por identidade de objeto — usado
// pelos eventos nativos do ST (GENERATION_STOPPED/ENDED, regenerate/swipe)
// que nunca guardam o objeto `atividade` que comecarAtividade('rp') criou
// lá no wireRpPresence/GENERATION_STARTED. Sem isso, qualquer geração pelo
// caminho nativo (não interceptado pela Spade) deixava a trava presa pra
// sempre — só destravava quando OUTRA atividade roubava o lock (por isso
// "mandar qualquer mensagem" parecia consertar).
// FIX (portado): guardado por TOKEN — só libera quando NINGUÉM reivindicou
// essa atividade de verdade (Spade_interceptGeneration marca
// tokenRpAdotadoPeloIntercept quando assume uma geração de verdade). Sem
// isso, GENERATION_STOPPED podia disparar (por causa do abort() que a
// própria interceptação chama pra cancelar a geração nativa do ST) ENQUANTO
// o streaming custom dela ainda tava rodando — liberando o lock no meio de
// uma geração de verdade, não só nas órfãs de regenerate/swipe.
let tokenRpAdotadoPeloIntercept = null;
function liberarAtividadeRp() {
    if (atividadeAtual?.onde === 'rp' && atividadeAtual.token !== tokenRpAdotadoPeloIntercept) { atividadeAtual = null; atualizarStatusBar(); }
}
function isStale(token) { return token !== opToken; }
function activityBusy() { return atividadeAtual !== null || usuarioDigitandoRp; }
...
async function renderJournalEm(elId, contexto, vazioTexto) {
    ...
    el.innerHTML = entradas.slice(-60).map((e) =>
        ...
        '<div class="axis-pensamento-meta">' + new Date(e.ts).toLocaleTimeString('pt-BR') + (e.autor === 'construtora' ? ' <span class="axis-pensamento-fonte">· 🔧 Construtora</span>' : e.autor === 'ingestao' ? ' <span class="axis-pensamento-fonte">· 📎 Ingestão</span>' : e.autor === 'diagnostico' ? ' <span class="axis-pensamento-fonte">· 🩺 Diagnóstico</span>' : '') + '</div>' +
...
// ====================================
// NOTA DE ESPAÇO NO PRÓPRIO TURNO — tentativa nova (v7.17.0), não é a
// v7.14.0 revertida de volta. Diferença real, não só cosmética: aquela
// injetava TURNOS "assistant" falsos (papel que o modelo lê pelo TOM,
// não pela tag — por isso vazava, ver v7.16.0). Essa gruda uma nota OOC
// curta na FRENTE do turno "user" de verdade — ainda ganha o peso de
// "coisa perto do ponto de gerar" que a v7.14.0 buscava, mas nunca finge
// ser a personagem falando: é sempre e só o usuário (via nota do
// sistema) trazendo contexto pro próprio turno dele. Curta de propósito
// (só a ÚLTIMA troca, não vale a pena arriscar mais que isso) e com a
// MESMA recência real da tentativa anterior — sem isso vira "lembrança"
// falsa de novo.
const JANELA_NOTA_ESPACO_MS = 20 * 60 * 1000; // 20min — depois disso, não é "agora há pouco" de verdade
async function notaEspacoParaTurno(stChatId) {
    if (!stChatId) return '';
    let chats = [];
    try { chats = await obterEspacoChats(); } catch { return ''; }
    const chat = chats.find((c) => c.stChatId === stChatId);
    if (!chat) return '';
    let historico = [];
    try { historico = await estadoGet('espacoHistory:' + chat.id, []); } catch { return ''; }
    if (!historico.length) return '';
    const ultima = historico[historico.length - 1];
    if (!ultima?.ts || (Date.now() - ultima.ts) > JANELA_NOTA_ESPACO_MS) return '';
    // só a resposta dela mesma no Espaço (o combinado), não a pergunta do
    // usuário — é o que importa pra "lembrar o que ela decidiu", e mantém
    // curto de propósito.
    const ultimaDela = [...historico].reverse().find((m) => m.role === 'assistant');
    if (!ultimaDela?.content) return '';
    return '[Nota rápida — você combinou isso com o usuário agora há pouco, no Espaço, fora da cena: "' +
        ultimaDela.content.slice(0, 300) + '". Isso é só contexto pra você, não repita essas palavras nem mencione ' +
        'o Espaço — a fala sai normal, como se já soubesse disso.]\n\n';
}
async function postCharacterMessageStreaming(recentText, history, atividade) {
    ...
    const notaEspaco = await notaEspacoParaTurno(stChatIdAtual());
    const messages = [{ role: 'system', content: systemPrompt }, ...history, { role: 'user', content: notaEspaco + recentText }];
...
// FIX (portado, ausente nessa linhagem): lastCharMessageIdx só era
// atualizado dentro de finalizeCharMessage — ou seja, só quando a PRÓPRIA
// Spade posta a fala. Um regenerate/swipe nativo (que ela pula de
// propósito) troca a última fala sem passar por ali — "editar minha última
// fala" podia acertar a fala errada. Resincroniza com o índice real toda
// vez que uma mensagem é desenhada, cobrindo os dois caminhos.
function ressincronizarUltimaFalaPersonagem() {
    const chatArr = ctx().chat;
    if (!Array.isArray(chatArr)) return;
    for (let i = chatArr.length - 1; i >= 0; i--) {
        if (!chatArr[i].is_user) { scope().lastCharMessageIdx = i; return; }
    }
}
...
async function passadaComFerramentas(messages, atividade, { onNarracao } = {}, opts = {}) {
    ...
    for (let i = 0; i < teto; i++) {
        ...
        messages.push({ role: 'assistant', content: resp.content || null, tool_calls: resp.toolCalls, ...(resp.reasoningContent ? { reasoning_content: resp.reasoningContent } : {}) });
...
async function pensarUmaVez() {
    ...
    try {
        ...
        const cenaRecente = await getRecentSceneText(6);
        const blocoSistemasPensamento = await avaliarSistemasReaisHook('antesDePensar', { ultimosPensamentos: journalRecente, cenaRecente }, 'SISTEMAS REAIS ATIVOS');
        const systemPrompt =
...
async function respondEspaco(userMessage, chatId) {
    ...
    const systemPrompt =
        ...
        'usuário. Em conversa normal você NÃO é a ' + charName + ' — não puxa pra narração nem começa a atuar por ' +
        'conta própria, sem ele pedir. Você é você mesmo: direto, útil, sem rodeio.\n\n' +
        'MAS a cena do RP não é um território separado — é a mesma cena, você só tá falando com ele de fora dela ' +
        'agora. Se ele pedir pra editar, apagar ou postar uma fala da ' + charName + ' no RP (\"tira a parte que ' +
        'repete\", \"apaga essa última mensagem\", \"posta isso no lugar\", com índice específico ou sem — sem índice ' +
        'é a última fala dela), use chat_agir direto, sem achar estranho nem pedir mais confirmação do que faria ' +
        'pra qualquer outro pedido. É a mesma cabeça mexendo na mesma cena por um canal diferente, não é você ' +
        '\"virando\" personagem — é ferramenta, igual artefato_escrever. Isso é pra edição PONTUAL de uma mensagem ' +
        'específica que ele apontou — não confunda com pedido de AJUSTE GERAL ("lembra de falar assim", "fala ' +
        'menos", ritmo/tom pra frente) — esse outro caso é automático (ver AJUSTE NA HORA abaixo) e não precisa ' +
        '(nem deve) virar plano de "vou ficar corrigindo cada fala que sair".\n\n' +
        ...
        'QUANDO O USUÁRIO CITAR ALGO — antes de responder "não tenho isso" ou de mandar ele ver na Sala de ' +
        'Pensamento, chame artefato_buscar (ou artefato_listar/artefato_ler se já souber o slug) e responda com o ' +
        'que achar, direto aqui na conversa. Isso vale pra qualquer menção — "lembra que eu falei sobre X", "como ' +
        'era mesmo aquilo", uma referência de passagem — não só quando ele pergunta explicitamente "o que tinha no ' +
        'arquivo". Buscar primeiro é o padrão, não a exceção.\n\n' +
        'AJUSTE NA HORA — automático: toda vez que o RP gera uma fala, o código já cola as últimas mensagens desta ' +
        'conversa no prompt dela, antes de gerar. Já aconteceu só por ter sido dito aqui — não invente plano de ' +
        '"corrigir depois que a fala aparecer" ou "cola aqui que eu ajusto", isso não existe e nunca vai rodar. A ' +
        'resposta certa pra "lembra disso"/"tá valendo?" é curta: confirmar que já vale a partir da próxima fala, ' +
        'ponto.\n\n' +
        'Se for pra durar além dessa conversa (a janela de conversa recente é limitada, some com o tempo), três ' +
        'ferramentas, uma escolha só:\n' +
        '- COMO ela atua daqui pra frente — ritmo de fala, quanto fala por padrão, slowburn, "não avança sem eu ' +
        'levar a cena até lá" — direcao_ler (pra ver o que já tem) e direcao_definir (substitui o texto inteiro; ' +
        'inclua o que quer manter, não é lista incremental). Entra INTEIRO em toda fala do RP, garantido.\n' +
        '- Regra CONDICIONAL — só vale quando algo específico aparecer na cena, não sempre ("se ela mencionar a ' +
        'mãe, lembra que é assunto sensível") — sistema_criar (nome curto + quando/gatilho + entao/o que fazer).\n' +
        '- Fato/lore que se busca por assunto ("o que ela sente pelo X", "como é a cidade", traço de personalidade, ' +
        'preferência) — artefato_escrever normal, chame artefato_ler antes se já existir um sobre o mesmo assunto.\n' +
        'Use sem hesitar quando o pedido for desse tipo — escrever já é suficiente, sem "vou ficar de olho"/"reforço ' +
        'depois", resposta curta confirmando que já vale. (Os três também rodam automático por código em paralelo ' +
        'com essa sua resposta — um classificador decide sozinho se o pedido é direção/sistema/fato e já escreve, ' +
        'mesmo que você não chame a tool. Não precisa se cobrar por isso nem avisar que "isso é automático" — só ' +
        'responda normal; chamar a tool direto ainda vale quando for reescrever do zero ou fazer ajuste fino que o ' +
        'automático poderia perder.)\n\n' +
        'ESCOPO DO ARTEFATO — por padrão todo artefato_escrever fica só nessa conversa (escopo "chat"), pra cada ' +
        'história ter sua própria memória sem vazar pra outra. Use escopo "personagem" só quando for algo que faz ' +
        'sentido em QUALQUER conversa com ' + charName + ' — tom de voz central, traço de personalidade fixo, não ' +
        'detalhe de uma história específica. (direcao_definir já cuida do escopo sozinha, não precisa decidir isso ' +
        'pra ela.)\n\n' +
        'ARQUIVO + INSTRUÇÃO JUNTOS — quando o usuário anexa um arquivo E escreve algo junto antes de mandar, esse ' +
        'texto guia como o arquivo é processado (o que extrair, pra que categoria vale, o que priorizar) — não é ' +
        'uma mensagem separada, é instrução de como tratar aquele arquivo específico.\n\n' +
        ...
        'o resultado como uma mensagem sua própria de resumo, já pronta, na conversa).\n\n' +
        'Cena recente do RP, pra contexto:\n' + (cena || '(nada ainda)') +
        (direcaoAtual ? '\n\nDireção de atuação em vigor agora (o que já foi definido pra como ela atua — não ' +
            'precisa chamar direcao_ler pra saber, já tá aqui):\n' + direcaoAtual : '');
    ...
    if (!erro) {
        ...
        await estadoSet(campoHistorico, [...historicoAgora, { role: 'assistant', content: respostaFinal, ts: Date.now() }].slice(-80));
    }
    dotsIdle();
...
async function Spade_interceptGeneration(chatArr, contextSize, abort, type) {
    ...
    try {
        if (type) return;
        atividade = comecarAtividade('rp');
        tokenRpAdotadoPeloIntercept = atividade.token; // ver liberarAtividadeRp
        ...
        if (atividade) terminarAtividade(atividade);
        if (tokenRpAdotadoPeloIntercept === atividade?.token) tokenRpAdotadoPeloIntercept = null;
    }
}
...
function wireRpPresence() {
    ...
    if (rpInput && !rpInput.dataset.axisWired) {
        ...
        rpInput.addEventListener('input', () => marcarDigitandoRp(Boolean((rpInput.value || '').trim())));
    }
}
...
// ====================================
let espacoLocalLog = [];
let espacoArquivosPendentes = []; // File[] escolhidos mas ainda não enviados — esperando o texto que vai junto
function renderEspacoAnexos() {
    const el = document.getElementById('axis-espaco-anexos');
    if (!el) return;
    el.innerHTML = espacoArquivosPendentes.map((f, i) =>
        '<span class="axis-espaco-anexo-chip">' +
        '<span class="axis-espaco-anexo-chip-nome">📎 ' + esc(f.name) + '</span>' +
        '<button type="button" class="axis-espaco-anexo-chip-remover" data-idx="' + i + '" title="Remover">✕</button>' +
        '</span>'
    ).join('');
}
...
async function resolverEspacoChatAtivo() {
    ...
    // FIX (portado de uma correção anterior nessa mesma conversa, ausente
    // nessa linhagem): só cai pro "mais recente" quando NEM SABEMOS o
    // stChatId atual. Sabendo e não batendo com nada, é uma conversa de RP
    // sem Espaço próprio ainda — começa rascunho novo linkado a ELA, nunca
    // reusa por acidente o Espaço de outra conversa.
    if (!alvo && !stId && chats.length) alvo = chats.slice().sort((a, b) => b.atualizadoEm - a.atualizadoEm)[0];
    if (alvo) {
...
async function garantirEspacoChatEscreve(primeiraMensagem) {
    const stIdReal = stChatIdAtual();
    const chats = await obterEspacoChats();
    if (espacoChatIdAtual) {
        const atual = chats.find((c) => c.id === espacoChatIdAtual);
        // FIX (portado): só reusa o rascunho já ativo se ele bate de
        // verdade com o chat REAL do ST agora. Sem essa checagem, qualquer
        // dessincronia deixava a mensagem entrar num Espaço com stChatId
        // errado — nunca achado de volta na hora de montar o prompt do RP.
        if (atual && (!stIdReal || atual.stChatId === stIdReal)) return espacoChatIdAtual;
        espacoChatIdAtual = null;
        espacoChatStPendente = stIdReal;
    }
    // FIX (portado): nunca confia só em espacoChatStPendente (só é populada
    // se resolverEspacoChatAtivo já rodou antes — pode não ter rodado
    // ainda). Pega o stChatId real agora, no momento exato de criar o
    // vínculo.
    const chat = { id: newId(), titulo: tituloAutomatico(primeiraMensagem), criadoEm: Date.now(), atualizadoEm: Date.now(), stChatId: espacoChatStPendente != null ? espacoChatStPendente : stIdReal };
...
function renderEspacoChat() {
    ...
    if (!chatEl) return;
    // mesmo teto do que fica persistido (.slice(-80) em cada save) — sem
    // isso, numa conversa longa o log em memória cresce sem limite enquanto
    // o persistido fica capado, e os dois saem de alinhamento (quebra a
    // apagarMensagemEspaco, que usa o MESMO índice nos dois).
    if (espacoLocalLog.length > 80) espacoLocalLog.splice(0, espacoLocalLog.length - 80);
    chatEl.innerHTML = espacoLocalLog.map((m, i) =>
        '<div class="axis-msg axis-msg-' + (m.role === 'user' ? 'user' : 'agent') + '">' +
        '<button type="button" class="axis-msg-apagar" data-idx="' + i + '" title="Apagar mensagem">✕</button>' +
        renderMarkers(m.content) + '</div>'
    ).join('') || '<div class="axis-empty">Fala alguma coisa pra começar.</div>';
    chatEl.scrollTop = chatEl.scrollHeight;
}
// Apaga uma mensagem específica do Espaço (sua ou dela) — tanto da lista
// em memória quanto do histórico persistido, pelo índice mostrado na
// hora do render (não por id — o log local é sempre a fonte da verdade
// visual, simples e sem precisar carregar id em cada mensagem à toa).
async function apagarMensagemEspaco(idx) {
    if (idx < 0 || idx >= espacoLocalLog.length || !espacoChatIdAtual) return;
    espacoLocalLog.splice(idx, 1);
    renderEspacoChat();
    const campoHistorico = 'espacoHistory:' + espacoChatIdAtual;
    const historico = await estadoGet(campoHistorico, []);
    historico.splice(idx, 1);
    await estadoSet(campoHistorico, historico);
    sincronizarEspacoComNativo().catch(() => {});
}
...
async function sendEspacoMessage(text) {
    ...
    await estadoSet(campoHistorico, [...historicoAtual, { role: 'user', content: text, ts: Date.now() }].slice(-80));
    await tocarEspacoChat();
    // sincroniza a rede de segurança nativa JÁ, antes mesmo dela responder —
    // se o usuário for correndo pro RP sem esperar a resposta do Espaço
    // terminar, o pedido já tá disponível pros dois caminhos (interceptação
    // da Spade E o mecanismo nativo do ST).
    sincronizarEspacoComNativo().catch(() => {});
    // roda em PARALELO com a resposta normal (não depende de reply nenhum,
    // só da mensagem do usuário) — classificação por código, não por
    // julgamento da conversa, decide sozinha se isso é direção de atuação
    // e já escreve. Ver o comentário da função pra entender por que isso
    // existe além de direcao_definir.
    const direcaoPromise = sincronizarAjusteAutomatico(text).catch((e) => console.warn('[ajuste automático] falhou:', e.message));
    ...
    try {
        const [result] = await Promise.all([respondEspaco(text, chatId), direcaoPromise]);
...
async function ingerirComVisual(files, instrucaoUsuario) {
    ...
    return ingerirArquivos(files, onProgresso, instrucaoUsuario);
}
async function handleUploadEspaco(files, instrucaoUsuario) {
    const nomes = Array.from(files).map((f) => f.name).join(', ');
    const mensagemVisivel = instrucaoUsuario
        ? '📎 ' + nomes + '\n' + instrucaoUsuario
        : '📎 enviou pra estudar: ' + nomes;
    espacoLocalLog.push({ role: 'user', content: mensagemVisivel });
    renderEspacoChat();
    const chatId = await garantirEspacoChatEscreve(instrucaoUsuario || ('📎 ' + nomes));
    const campoHistorico = 'espacoHistory:' + chatId;
    const historicoAntes = await estadoGet(campoHistorico, []);
    const mensagemSalva = instrucaoUsuario
        ? '[enviou arquivo(s) pra estudar: ' + nomes + ' — junto com essa instrução: "' + instrucaoUsuario + '"]'
        : '[enviou arquivo(s) pra estudar: ' + nomes + ']';
    await estadoSet(campoHistorico, [...historicoAntes, { role: 'user', content: mensagemSalva, ts: Date.now() }].slice(-80));
    await tocarEspacoChat();

    switchTab('pensamento');
    const resultados = await ingerirComVisual(files, instrucaoUsuario);
    ...
    await estadoSet(campoHistorico, [...historicoDepois, { role: 'assistant', content: respostaFinal, ts: Date.now() }].slice(-80));
    await tocarEspacoChat();
    switchTab('espaco');
}
...
// ====================================
// Categoria de fala/voz precisa virar tipo 'fala' de verdade — é o único
// tipo que buscarVoz procura (slot RESERVADO, não compete por espaço com
// lore no ranking geral). Categoria ficou livre (a IA inventa o nome que
// quiser, e o próprio schema de artefato_escrever SUGERE "exemplo_dialogo"
// como exemplo) — mas isso quebrava esse caso na prática: um artefato
// "exemplo_dialogo"/"fala_dela"/"diálogo" nunca batia com tipo==='fala'
// hardcoded, então nunca aparecia no slot reservado, só competia (e
// geralmente perdia) no ranking genérico junto com lore/fato. Esse era o
// motivo real de "os exemplos de diálogo que mando não funcionam" —
// achava o artefato, só nunca com o formato/peso de voz. CATEGORIA_DE_FALA
// em si tá definida lá em cima, perto de contextualizarFato — mesmo
// critério, um lugar só.
async function sincronizarDocumentoNaBiblioteca(slug, titulo, categoria, conteudo, chatId = null) {
    const personagem = personagemAtual();
    const tipoEfetivo = CATEGORIA_DE_FALA.test(categoria || '') ? 'fala' : (categoria || 'documento');
    ...
    for (const trecho of chunks) {
        await Biblioteca.escrever({
            tipo: tipoEfetivo, texto: trecho, chatId,
...
async function buscarArtefatos(consulta, k = 8) {
    ...
    const [a, b] = await Promise.all([
        Biblioteca.buscarVoz(consulta, { k: metade, chatId }),
        Biblioteca.buscarMemoria(consulta, { k: metade, chatId }),
    ]);
...
async function abrirArtefatoDetalhe(slug) {
    ...
    try {
        const r = await chamarSpadeFs('/documento/ler', { personagem: personagemAtual(), slug, chatId: stChatIdAtual() });
...
async function renderArtefatosLista() {
    ...
    lista.innerHTML = documentos.map((d) =>
        ...
        '<div class="axis-artefato-card-meta">' + esc(d.categoria || 'documento') + (d.escopo === 'personagem' ? ' · compartilhado' : '') + '</div>' +
        '</div>' +
        '<button type="button" class="axis-btn axis-artefato-card-del" data-slug="' + esc(d.slug) + '" title="Apagar">🗑</button>' +
        '</div>'
...
async function apagarArtefatoUI(slug) {
    const d = artefatosCache.find((x) => x.slug === slug);
    try {
        await chamarSpadeFs('/documento/apagar', { personagem: personagemAtual(), slug, chatId: d?.escopo === 'chat' ? stChatIdAtual() : null });
...
function createPanel() {
    ...
    panel.innerHTML =
        ...
        '<div id="axis-espaco-anexos" class="axis-espaco-anexos"></div>' +
    ...
    document.getElementById('axis-espaco-upload').addEventListener('change', (e) => {
        // Não sobe na hora — só anexa (chip visível) e espera o envio, pra
        // dar tempo do usuário escrever junto o que quer que ela faça com
        // o arquivo. Enter/Enviar decide, não a escolha do arquivo em si.
        const novos = Array.from(e.target.files || []);
        espacoArquivosPendentes = espacoArquivosPendentes.concat(novos);
        renderEspacoAnexos();
        e.target.value = '';
    });
    document.getElementById('axis-espaco-anexos').addEventListener('click', (e) => {
        const btn = e.target.closest('.axis-espaco-anexo-chip-remover');
        if (!btn) return;
        espacoArquivosPendentes.splice(Number(btn.dataset.idx), 1);
        renderEspacoAnexos();
    });
    document.getElementById('axis-espaco-chat').addEventListener('click', (e) => {
        const btn = e.target.closest('.axis-msg-apagar');
        if (!btn) return;
        apagarMensagemEspaco(Number(btn.dataset.idx)).catch(reportFatalError);
    });
    ...
    function doSend() {
        ...
        if (espacoArquivosPendentes.length) {
            // arquivo(s) + texto (opcional) juntos, um envio só — o texto
            // vira instrução de como processar o arquivo, não uma mensagem
            // separada solta antes/depois.
            const arquivos = espacoArquivosPendentes;
            espacoArquivosPendentes = [];
            renderEspacoAnexos();
            inputEl.value = '';
            handleUploadEspaco(arquivos, text || null).catch(reportFatalError);
            return;
        }
        if (!text) return;
        inputEl.value = '';
...
eventSource.on(event_types.CHARACTER_MESSAGE_RENDERED, () => { try { ressincronizarUltimaFalaPersonagem(); sincronizarRepeticaoNativo(); } catch (e) { reportFatalError(e); } });
eventSource.on(event_types.GENERATION_STOPPED, () => { try { liberarAtividadeRp(); dotsIdle(); } catch (e) { reportFatalError(e); } });
eventSource.on(event_types.GENERATION_ENDED, () => { try { liberarAtividadeRp(); dotsIdle(); } catch (e) { reportFatalError(e); } });

eventSource.on(event_types.APP_READY, () => {
    try { createPanel(); wireRpPresence(); garantirTicker(); loopPensamento(); sincronizarEspacoComNativo().catch(() => {}); } catch (e) { reportFatalError(e); }
});
eventSource.on(event_types.CHAT_CHANGED, () => {
    try { abrirEspacoNoContextoAtual().catch(reportFatalError); wireRpPresence(); sincronizarEspacoComNativo().catch(() => {}); } catch (e) { reportFatalError(e); }
});
...
sincronizarEspacoComNativo().catch(() => {}); // rede de segurança nativa (setExtensionPrompt) já ativa desde o carregamento
