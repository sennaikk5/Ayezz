...
// Tom é tratado como referência de voz sempre presente, não uma busca
// rankeada como o resto da Biblioteca — não compete por espaço nem
// depende de vencer similaridade pra aparecer, e não tem timer nem
// estado guardado à parte: o Artefato em si já é a fonte, lido direto
// toda vez. É o que faz Espaço e RP serem "um só" — o que for escrito
// como tom em qualquer lugar entra inteiro na cena seguinte, sempre.
async function compiledTomFixoBlock(chatId) {
    const personagem = personagemAtual();
    let docs = [];
    try { docs = (await chamarSpadeFs('/documento/listar', { personagem, chatId })).documentos || []; }
    catch { return ''; }
    const tons = docs.filter((d) => d.categoria === 'tom');
    if (!tons.length) return '';
    const conteudos = await Promise.all(tons.map((d) =>
...
async function runCantos(userMessage) {
    ...
    const [bibliotecaResult, repeticaoResult, estadoResult, mundoResult, sistemasResult, sistemasReaisResult, tomFixoResult] = await Promise.allSettled([
        ...
        compiledTomFixoBlock(stChatIdAtual()),
    ]);

    const cantos = {};
    cantos.biblioteca = fromSettled(bibliotecaResult, 'biblioteca', '');
    cantos.tomFixo = fromSettled(tomFixoResult, 'tom_fixo', '');
...
async function respondEspaco(userMessage, chatId) {
    ...
    const systemPrompt =
        ...
        'AJUSTE DE TOM NA HORA — se o usuário disser algo tipo "ela tá falando desse jeito, fala mais assim" (com ' +
        'exemplo, ou apontando pra algo que já tá em algum arquivo/artefato), chame artefato_escrever com categoria ' +
        '"tom" IMEDIATAMENTE, sem esperar confirmação. Categoria "tom" não é um artefato comum: ela entra sempre, ' +
        'inteira, direto no prompt do RP — não é buscada por similaridade nem compete com outra coisa, então pode ' +
        'dizer pro usuário que já vale a partir da próxima fala dela, porque vale mesmo, sempre, sem prazo pra ' +
        'vencer. Se já existe um artefato de tom, chame artefato_ler nele antes e reescreva completo (mistura o que ' +
        'já tinha com o ajuste novo) em vez de criar um segundo — mais de um artefato de tom ao mesmo tempo é ruído.\n\n' +
