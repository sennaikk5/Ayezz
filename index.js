...
// ====================================
// TÉCNICA DIFERENTE — Espaço como TURNOS de verdade, não só bloco de texto.
// Tudo acima (textoEspacoRecente/compiledEspacoRecenteBlock) entrega o
// conteúdo dentro do system prompt — funciona, mas ainda é "instrução pra
// seguir", concorrendo com o resto do prompt por atenção. Um modelo de
// chat é treinado principalmente pra dar peso forte ao que foi dito nos
// TURNOS reais da conversa (user/assistant alternando) — é o canal mais
// forte que existe, mais forte que qualquer bloco dentro de um system
// prompt grande. Isso pega as últimas trocas do Espaço e insere ELAS
// MESMAS como mensagens de verdade no array, logo antes da fala nova —
// pro modelo, fica literalmente "isso acabou de ser dito nessa mesma
// conversa", não "uma nota pra lembrar de aplicar".
// Aditivo, não substitui o bloco no prompt — os dois juntos (reforço, não
// redundância inútil): se um canal falhar/for ignorado, o outro ainda
// pode segurar. Cuidado principal: SEMPRE termina garantindo alternância
// user/assistant limpa — um buraco aqui bagunçaria bem a fala que mais
// importa (a próxima).
// ====================================
const JANELA_TURNO_ESPACO_MS = 20 * 60 * 1000; // 20min — depois disso, "agora há pouco" vira mentira
async function turnosEspacoRecentes(stChatId, limite = 2) {
    if (!stChatId) return [];
    let chats = [];
    try { chats = await obterEspacoChats(); } catch { return []; }
    const chat = chats.find((c) => c.stChatId === stChatId);
    if (!chat) return []; // sem fallback aqui de propósito — isso vira TURNO DE CONVERSA de verdade, pegar do chat errado é bem mais grave que só não incluir
    let historico = [];
    try { historico = await estadoGet('espacoHistory:' + chat.id, []); } catch { return []; }
    if (!historico.length) return [];
    // Recência de verdade — sem isso, uma troca de horas/dias atrás vira
    // "[...agora há pouco]" pro modelo, o que é literalmente falso e pode
    // confundir mais do que ajudar (ela "lembraria" de algo que não é
    // recente). Mensagem sem ts (histórico de antes dessa versão) é
    // tratada como velha de propósito — melhor excluir do que injetar
    // errado por falta de dado.
    const ultima = historico[historico.length - 1];
    if (!ultima?.ts || (Date.now() - ultima.ts) > JANELA_TURNO_ESPACO_MS) return [];
    let recentes = historico.slice(-limite * 2);
    while (recentes.length && recentes[0].role !== 'user') recentes = recentes.slice(1);
    while (recentes.length && recentes[recentes.length - 1].role === 'user') recentes = recentes.slice(0, -1);
    if (!recentes.length) return [];
    return recentes.map((m) => ({
        role: m.role === 'user' ? 'user' : 'assistant',
        content: '[Fora da cena, com você mesma agora há pouco — não é o RP] ' + m.content,
    }));
}
...
async function sincronizarAjusteAutomatico(mensagemUsuario) {
    ...
    const prompt =
        ...
        'slowburn, tom, asterisco, o que evita). {"tipo":"direcao","texto":"direção de atuação completa, ' +
        'incorporando o que já existia acima + a mudança pedida agora, reescrita por inteiro"}\n' +
...
async function buildSystemPrompt(cantos) {
    ...
    const parts = [
        ...
        'falando de verdade. Responda curto, direto, em pt-br, coerente com o que veio acima — poucas frases, sem ' +
...
async function postCharacterMessageStreaming(recentText, history, atividade) {
    ...
    const turnosEspaco = await turnosEspacoRecentes(stChatIdAtual());
    const messages = [{ role: 'system', content: systemPrompt }, ...history, ...turnosEspaco, { role: 'user', content: recentText }];
...
eventSource.on(event_types.CHARACTER_MESSAGE_RENDERED, () => { try { ressincronizarUltimaFalaPersonagem(); } catch (e) { reportFatalError(e); } });
