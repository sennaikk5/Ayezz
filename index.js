...
A maioria das vezes a resposta certa é SILENCIO. Você não fala só por falar.` }];
        ...
        const withCreateCompat = processSystemCreateCompat(withDirective); // Ticket 20
...
const AXIS_TAGS_SAFE_TO_RENDER = new Set(['CARD', 'CARD_END', 'APPROVAL', 'VOICE_APPROVAL', 'THINK', 'ACTION']);
...
// ====================================
// BIBLIOTECA DE VOZ (Ticket 15)
// Só visualização por enquanto — mostra a personality consolidada e cada
// fala guardada, com indicador de se já tem embedding calculado ou não
// (⚠️ sem vetor = ainda entra só pelo fallback das últimas 6, não por
// relevância de verdade).
// ====================================
function toggleVoiceLibraryPanel() {
    ...
    const linesHtml = voice.lines.length
        ? voice.lines.slice().reverse().map(l =>
            '<div class="axis-system-item">' +
            '<div class="axis-system-desc">' + (l.embedding ? '✅ com vetor' : '⚠️ sem vetor') + ' · <small>' + new Date(l.ts || 0).toLocaleString() + '</small></div>' +
