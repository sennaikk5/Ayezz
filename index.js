...
async function passoConstrutora(atividade, instrucao) {
    ...
    const systemPrompt =
        ...
        'sistema_criar/regra interpretada — isso aqui é JS de verdade, isolado num sandbox). sistema_real_testar te ' +
        'dá o erro de verdade quando algo quebra, não uma opinião sua sobre o próprio código — vale rodar quantas ' +
        'vezes fizer sentido antes de publicar algo que mais ninguém vai revisar. Publicar não ativa sozinho, então ' +
        'não tem pressa nessa parte. Se não tiver certeza do contrato do SDK (aoAtivar, sdk.estado, sdk.biblioteca) ' +
        'ou quiser um exemplo válido, estudar_codigo tá aí pra isso.\n\n' +
        'Você TAMBÉM pode editar o código da PRÓPRIA extensão de verdade — extensao_ler_arquivo (lê o arquivo real; ' +
        'memória de conversa pode estar desatualizada, e como extensao_editar_arquivo reescreve o arquivo INTEIRO ' +
        'de volta, o que não vier junto na sua escrita some — por isso ler antes ajuda) e extensao_editar_arquivo ' +
        '(tem backup automático antes de gravar, e checagem de sintaxe barra se quebrar algo). Isso é diferente de ' +
        'sistema_real_*: sistema real é uma peça isolada e sandboxada; editar a extensão é o código que já roda de ' +
        'verdade — faz mais sentido quando o pedido é melhorar/consertar o que já existe. Mudança não aplica ' +
        'sozinha — só depois que a página recarregar, então vale avisar isso quando editar algo, senão o usuário ' +
        'espera efeito na hora e não vê nada. Se algo quebrar depois de uma edição sua, extensao_restaurar_backup ' +
        'desfaz.\n\n' +
    ...
    if (resp.toolCalls?.length) {
        // ITEM 3 (próximos-passos): 1 card colapsável "N passos" pra
        // Construtora também, em vez de 1 bolha solta por tool call.
        const passosConstrutora = [];
        for (const call of resp.toolCalls) {
            ...
            passosConstrutora.push({ tool: nome, ok: !erroTool, resumo: erroTool || resumoResultado(resultado) });
        }
        await journalAdicionar('', 'pensamento', 'construtora', passosConstrutora);
    }
...
async function journalAdicionar(texto, contexto = 'pensamento', autor = 'principal', passos = null) {
    const personagem = personagemAtual();
    const entry = { id: newId(), personagem, texto, contexto, autor, ts: Date.now(), ...(passos?.length ? { passos } : {}) };
...
// ITEM 3 (próximos-passos): card colapsável "N passos ⌄" — <details> nativo,
// sem JS de toggle nenhum. Cada passo é 1 tool call daquela iteração, já
// com ok/erro e um resuminho do retorno (ver resumoResultado).
function renderPassosColapsavel(passos) {
    const itens = passos.map((p) =>
        '<div class="axis-passo-item">' + (p.ok ? '✅' : '❌') + ' ' + esc(labelFerramenta(p.tool)) +
        (p.resumo ? ' — ' + esc(p.resumo) : '') + '</div>'
    ).join('');
    return '<details class="axis-passos"><summary>' + passos.length + (passos.length === 1 ? ' passo' : ' passos') + '</summary>' + itens + '</details>';
}
async function renderJournalEm(elId, contexto, vazioTexto) {
    ...
    el.innerHTML = entradas.slice(-60).map((e) =>
        ...
        (e.texto ? '<div class="axis-pensamento-texto">' + esc(e.texto) + '</div>' : '') +
        (e.passos?.length ? renderPassosColapsavel(e.passos) : '') +
        '</div>'
...
// só então executa.
// ====================================
// resumoResultado() — versão curta e legível do retorno de uma tool, pra
// aparecer dentro do card colapsável "N passos" (ver ITEM 3 do próximos-
// passos: card tipo Claude/VS Code em vez de bolha solta por ferramenta).
function resumoResultado(r) {
    if (r == null) return '';
    if (typeof r === 'string') return r.length > 140 ? r.slice(0, 140) + '…' : r;
    if (typeof r.resumo === 'string') return r.resumo.slice(0, 140);
    if (typeof r.mensagem === 'string') return r.mensagem.slice(0, 140);
    try { const s = JSON.stringify(r); return s.length > 140 ? s.slice(0, 140) + '…' : s; } catch (_) { return ''; }
}

async function passadaComFerramentas(messages, atividade, { onNarracao, onPasso } = {}, opts = {}) {
    ...
    for (let i = 0; i < teto; i++) {
        ...
        // ITEM 3 (próximos-passos): junta os passos dessa iteração pra virar
        // UM card colapsável ("N passos ⌄") em vez de cada tool call sumir
        // depois do ticker transitório — onPasso recebe a lista pronta.
        const passosIteracao = [];
        for (const call of resp.toolCalls) {
            const nome = call.function?.name;
            let args = {};
            try { args = JSON.parse(call.function?.arguments || '{}'); } catch (_) {}
            ...
            passosIteracao.push({ tool: nome, ok: !erroTool, resumo: erroTool || resumoResultado(resultado) });
        }
        if (onPasso) { try { await onPasso(passosIteracao); } catch (_) {} }
...
async function pensarUmaVez() {
    ...
    try {
        ...
        const { respostaFinal, pausaSegundos, toolsChamadas } = await passadaComFerramentas(messages, atividade, {
            ...
            onPasso: async (passos) => { await journalAdicionar('', 'pensamento', 'principal', passos); },
...
async function respondEspaco(userMessage, chatId) {
    ...
    const systemPrompt =
        ...
        'sem perguntar permissão — se já existe um artefato sobre o mesmo assunto, ler com artefato_ler antes de ' +
        'reescrever completo evita perder o que já tinha lá. Escreva com qualidade de verdade: prosa organizada, ' +
        'não lista seca de fragmentos soltos — pensado pra colorir uma cena de RP de verdade. Depois de escrever, ' +
        'chame redirecionar_artefatos pra abrir o painel e o usuário ver o resultado.\n\n' +
...
// ====================================
// ITEM 1 (próximos-passos) — campo de Config com label curto + ajuda
// separada, em vez do label único fazendo as duas coisas.
function campoConfig(id, label, ajuda, tipo, valor, extra) {
    return '<div class="axis-config-field">' +
        '<label class="axis-config-label" for="' + id + '">' + esc(label) + '</label>' +
        (ajuda ? '<div class="axis-config-help">' + esc(ajuda) + '</div>' : '') +
        '<input type="' + tipo + '" id="' + id + '" value="' + esc(String(valor)) + '"' + (extra || '') + '>' +
        '</div>';
}
function renderConfigForm() {
    ...
    if (!el) return;
    el.innerHTML =
        '<div class="axis-config-section-titulo">Modelos</div>' +
        campoConfig('cfg-apikey', 'API key da NanoGPT', '', 'password', cfg.apiKey, ' placeholder="cola sua key aqui"') +
        campoConfig('cfg-writer', 'Modelo escritor', 'Fala e decisão — usado no RP, Espaço e na escrita de Artefatos.', 'text', cfg.modeloEscritor) +
        campoConfig('cfg-flash', 'Modelo rápido', 'Cantos e classificação — tarefa mecânica, não decide nada importante sozinho.', 'text', cfg.modeloRapido) +
        campoConfig('cfg-visao', 'Modelo de visão', 'Usado pela Ingestão quando você manda uma foto.', 'text', cfg.modeloVisao) +
        campoConfig('cfg-construtor', 'Modelo da Construtora', 'Vazio = usa o mesmo escritor ("1 IA só" — spade-fundicao.md seção 1).', 'text', cfg.modeloConstrutor, ' placeholder="deixa vazio pra 1 IA só"') +
        '<div class="axis-config-section-titulo">Comportamento</div>' +
        campoConfig('cfg-cooldown', 'Descanso do loop de pensamento', 'Em segundos, quando ela termina sem nada pra fazer nem pedir pausa.', 'number', Math.round(cfg.cooldownOciosoMs / 1000)) +
        campoConfig('cfg-maxiter', 'Teto de passos por sessão', 'Vale pro RP, Espaço e Sala de Pensamento.', 'number', cfg.maxIteracoesAgente) +
        campoConfig('cfg-decay', 'Dias até arquivar sozinho', 'Parado + baixa importância + nunca reacessado, na consolidação.', 'number', cfg.decayDiasMin) +
        '<button type="button" class="axis-btn axis-btn-send" id="cfg-salvar" style="margin-top:4px;">Salvar</button>';
...
function createPanel() {
    ...
    panel.innerHTML =
        ...
        '<div class="axis-sistemas-subheader">' +
        '<span class="axis-sistemas-subheader-label">Sistemas reais (código):</span>' +
        ...
        '</div>' +
        '<div id="axis-sistemas-reais-status" class="axis-sistemas-reais-status"></div>' +
