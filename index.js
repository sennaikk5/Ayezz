...
const TOOLS = [
    ...
    { type: 'function', function: { name: 'redirecionar_tom', description: 'Só existe no Espaço. Abre a aba de Treino de Tom (mini-chat dedicado) — use quando o usuário mandar exemplos de diálogo/fala dela, ou pedir explicitamente pra treinar como ela fala. Se não tiver certeza se é treino de tom ou referência geral, PERGUNTE antes de chamar qualquer uma das duas.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'tom_listar', description: 'Lista as folhas de treino de tom já guardadas pra personagem atual.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'tom_ler', description: 'Lê o conteúdo inteiro de uma folha de tom específica.', parameters: { type: 'object', properties: { slug: { type: 'string', description: 'Identificador da folha, como aparece em tom_listar.' } }, required: ['slug'] } } },
    { type: 'function', function: { name: 'tom_escrever', description: 'Cria ou reescreve uma folha de tom inteira — os diálogos/exemplos curados, já limpos (só o que vale, sem o resto do arquivo original). Isso sincroniza direto com a busca usada no RP de verdade — não é só um documento de referência solto, o que você escrever aqui pode aparecer na cena. Chame tom_ler antes se for editar uma que já existe, pra não perder conteúdo.', parameters: { type: 'object', properties: { slug: { type: 'string', description: 'Se omitido numa folha NOVA, invente um curto a partir do título.' }, titulo: { type: 'string' }, tags: { type: 'array', items: { type: 'string' } }, conteudo: { type: 'string' } }, required: ['titulo', 'conteudo'] } } },
    { type: 'function', function: { name: 'tom_apagar', description: 'Apaga uma folha de tom inteira (vira backup, recuperável pelo lado do servidor se precisar).', parameters: { type: 'object', properties: { slug: { type: 'string' } }, required: ['slug'] } } },
...
async function executeTool(name, args) {
    switch (name) {
        ...
        case 'redirecionar_tom': return { redirecionar: 'tom' };
        case 'tom_listar': return chamarSpadeFs('/tom/listar', { personagem: personagemAtual() });
        case 'tom_ler': return chamarSpadeFs('/tom/ler', { personagem: personagemAtual(), slug: args.slug });
        case 'tom_escrever': {
            const slug = args.slug || slugifyLocal(args.titulo);
            const r = await chamarSpadeFs('/tom/escrever', { personagem: personagemAtual(), slug, titulo: args.titulo, tags: args.tags || [], conteudo: args.conteudo });
            atualizarFolhaTomAoVivo({ slug, titulo: args.titulo, conteudo: args.conteudo });
            const chunks = await sincronizarFolhaNaBiblioteca(slug, args.titulo, args.conteudo);
            return Object.assign(r, { sincronizadoNaBiblioteca: chunks + ' trecho(s) — já buscável no RP de verdade agora.' });
        }
        case 'tom_apagar': {
            const r = await chamarSpadeFs('/tom/apagar', { personagem: personagemAtual(), slug: args.slug });
            await removerFolhaDaBiblioteca(args.slug);
            return r;
        }
...
const ACTION_LABEL = {
    ...
    redirecionar_tom: '🎭 abrindo o treino de tom',
    tom_listar: '🎭 vendo as folhas de tom',
    tom_ler: '🎭 lendo uma folha de tom',
    tom_escrever: '🎭 escrevendo na folha de tom',
    tom_apagar: '🎭 apagando uma folha de tom',
...
async function passadaComFerramentas(messages, atividade, { onNarracao } = {}, opts = {}) {
    ...
    for (let i = 0; i < teto; i++) {
        ...
        for (const call of resp.toolCalls) {
            ...
            if (nome === 'redirecionar_tom') redirecionarTom = true;
...
async function respondEspaco(userMessage) {
    ...
    if (!erro) {
        ...
        const historicoAgora = await estadoGet('espacoHistory', []);
        await estadoSet('espacoHistory', [...historicoAgora, { role: 'assistant', content: respostaFinal }].slice(-80));
    }
    dotsIdle();
...
async function sendEspacoMessage(text) {
    ...
    const historicoAtual = await estadoGet('espacoHistory', []);
    await estadoSet('espacoHistory', [...historicoAtual, { role: 'user', content: text }].slice(-80));
...
async function handleUploadEspaco(files) {
    ...
    renderEspacoChat();
    const historicoAntes = await estadoGet('espacoHistory', []);
    await estadoSet('espacoHistory', [...historicoAntes, { role: 'user', content: '[enviou arquivo(s) pra estudar: ' + nomes + ']' }].slice(-80));
    ...
    renderEspacoChat();
    const historicoDepois = await estadoGet('espacoHistory', []);
    await estadoSet('espacoHistory', [...historicoDepois, { role: 'assistant', content: respostaFinal }].slice(-80));
    switchTab('espaco');
}

// ====================================
// TREINO DE TOM — mini-chat dedicado, separado do Espaço e da Sala de
// Pensamento de propósito (era exatamente essa mistura que fazia a
// animação/sala errada aparecer quando o assunto era treinar voz). Folha
// é sempre reescrita INTEIRA (tom_escrever) — nunca por chunk solto, é
// "a folha", não pedaços.
// ====================================
// Ponte real com o RP — sem isso a folha só existe dentro do mini-chat.
// A folha (arquivo .md, fonte editável de verdade) sincroniza pra dentro
...
async function sincronizarFolhaNaBiblioteca(slug, titulo, conteudo) {
    const personagem = personagemAtual();
    const antigos = (await Biblioteca.listar({ tipos: ['tom'] }))
        .filter((e) => e.personagem === personagem && e.metadata?.folhaSlug === slug);
    for (const e of antigos) await Biblioteca.apagar({ id: e.id });
    const chunks = chunkText(conteudo, 500, 60);
    for (const trecho of chunks) {
        await Biblioteca.escrever({
            tipo: 'tom', texto: trecho,
            metadata: { folhaSlug: slug, folhaTitulo: titulo, ativo: true },
        });
    }
    return chunks.length;
}
async function removerFolhaDaBiblioteca(slug) {
    const personagem = personagemAtual();
    const antigos = (await Biblioteca.listar({ tipos: ['tom'] }))
        .filter((e) => e.personagem === personagem && e.metadata?.folhaSlug === slug);
    for (const e of antigos) await Biblioteca.apagar({ id: e.id });
...
let tomLocalLog = [];
let tomFolhaAtual = null; // { slug, titulo, conteudo } | null — o que o painel ao vivo mostra agora
function slugifyLocal(texto) {
    return String(texto || 'folha').trim().toLowerCase()
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'folha';
}
function renderTomChat() {
    ...
    chatEl.innerHTML = tomLocalLog.map((m) =>
        '<div class="axis-msg axis-msg-' + (m.role === 'user' ? 'user' : 'agent') + '">' + renderMarkers(m.content) + '</div>'
    ).join('') || '<div class="axis-empty">Manda um exemplo de fala dela, ou um arquivo com diálogos, pra começar a treinar.</div>';
    chatEl.scrollTop = chatEl.scrollHeight;
}
// Painel de baixo — a folha "ao vivo". Comparação simples (prefixo comum)
// em vez de diff de verdade: sem lib externa, mas cobre o caso real (ela
// escreve, o texto cresce) com o mesmo efeito visual de "+ apareceu algo
// novo" que um diff de VS Code dá, sem precisar de biblioteca nenhuma.
function atualizarFolhaTomAoVivo({ slug, titulo, conteudo }) {
    const anterior = tomFolhaAtual?.slug === slug ? (tomFolhaAtual.conteudo || '') : '';
    tomFolhaAtual = { slug, titulo, conteudo };
    ...
    el.innerHTML = partes.map((p) => {
        ...
        if (p.tipo === 'novo') return '<span class="axis-tom-diff-novo">' + esc(p.texto) + '</span>';
        return '<span class="axis-tom-diff-removido">' + esc(p.texto) + '</span>';
    }).join('');
    el.scrollTop = el.scrollHeight;
    document.getElementById('axis-tom-folha-vazio')?.classList.add('axis-hidden');
}
async function renderTomListaFolhas() {
    const selectEl = document.getElementById('axis-tom-folha-select');
    if (!selectEl) return;
    try {
        const r = await chamarSpadeFs('/tom/listar', { personagem: personagemAtual() });
        const folhas = r.folhas || [];
        selectEl.innerHTML = '<option value="">+ nova folha</option>' + folhas.map((f) =>
            '<option value="' + esc(f.slug) + '">' + esc(f.titulo) + '</option>').join('');
    } catch (e) { console.warn('[tom] listar folhas falhou:', e.message); }
}
...
async function respondTom(userMessage) {
    ...
    const folhasExistentes = await chamarSpadeFs('/tom/listar', { personagem: charName }).catch(() => ({ folhas: [] }));
    const systemPrompt =
        'Você é o Spade, num mini-chat dedicado SÓ a treinar como ' + charName + ' fala — não é o Espaço geral nem ' +
        'a Sala de Pensamento, aqui o assunto é só tom/voz. O usuário vai mandar exemplos de diálogo dela (colados ' +
        'ou por arquivo — se mandar arquivo, o texto bruto inteiro já vem na mensagem dele, você lê direto, não ' +
        'precisa de ferramenta pra "abrir" nada).\n\n' +
        'Seu trabalho: separar o que É voz dela (fala, cadência, vocabulário, jeito de reagir) do que não é ' +
        '(narração de terceiros, metadado, lixo do arquivo original) — e guardar SÓ o que vale, limpo, com ' +
        'tom_escrever. Uma folha é o documento INTEIRO, sempre — se já existe uma sobre o mesmo assunto, chame ' +
        'tom_ler nela primeiro e reescreva completa (tom_escrever de novo com o mesmo slug), nunca deixe pedaço ' +
        'perdido. Se genuinamente não tiver nada de diálogo aproveitável no que o usuário mandou, diga isso direto ' +
        'em vez de forçar uma folha vazia.\n\n' +
        'Folhas já existentes pra ' + charName + ': ' + (folhasExistentes.folhas?.length ? folhasExistentes.folhas.map((f) => f.titulo).join(', ') : '(nenhuma ainda)') + '.';
...
async function sendTomMessage(text) {
    tomLocalLog.push({ role: 'user', content: text });
    renderTomChat();
    ...
    try {
        ...
        tomLocalLog.push({ role: 'agent', content: result.erro ? '⚠️ ' + result.erro : (result.resposta || '(sem resposta)') });
        renderTomChat();
        renderTomListaFolhas();
    } finally {
        if (inputEl) inputEl.disabled = false;
...
async function handleUploadTom(files) {
    ...
    // Um upload de N arquivos vira UMA mensagem — deixa ela cruzar
    // referência entre eles (mesmo personagem, contextos diferentes) em vez
    // de tratar cada um isolado numa rodada separada.
    await sendTomMessage(lidos.join('\n\n---\n\n'));
}
async function carregarFolhaTomSelecionada(slug) {
    if (!slug) {
        tomFolhaAtual = null;
        const el = document.getElementById('axis-tom-folha-corpo');
        if (el) el.innerHTML = '';
        document.getElementById('axis-tom-folha-titulo').textContent = 'nova folha';
        document.getElementById('axis-tom-folha-vazio')?.classList.remove('axis-hidden');
        return;
    }
    try {
        const r = await chamarSpadeFs('/tom/ler', { personagem: personagemAtual(), slug });
        atualizarFolhaTomAoVivo({ slug, titulo: r.titulo, conteudo: r.conteudo });
    } catch (e) { console.warn('[tom] ler folha falhou:', e.message); }
}
...
function createPanel() {
    ...
    panel.innerHTML =
        ...
        '<span class="axis-mini-tab" id="axis-mini-tom">Tom</span>' +
        ...
        '<select id="axis-tom-folha-select" style="flex:1;background:#12122c;color:#c8c8e8;border:1px solid #2a2a5a;border-radius:6px;padding:4px 6px;font-size:12px;"><option value="">+ nova folha</option></select>' +
        '</div>' +
        '<div class="axis-tom-split">' +
        '<div class="axis-tom-chat" id="axis-tom-chat"></div>' +
        '<div class="axis-tom-folha-panel">' +
        '<div class="axis-tom-folha-header" id="axis-tom-folha-titulo">nova folha</div>' +
        '<div class="axis-tom-folha-corpo" id="axis-tom-folha-corpo"></div>' +
        '<div class="axis-tom-folha-vazio" id="axis-tom-folha-vazio">a folha aparece aqui, ao vivo, enquanto ela escreve</div>' +
        '</div></div>' +
        ...
        '<button type="button" class="axis-btn axis-btn-attach" id="axis-tom-attach-btn" title="Anexar arquivo com diálogos">📎</button>' +
        '<div class="axis-espaco-input-wrap">' +
        '<textarea class="axis-espaco-input" id="axis-tom-input" placeholder="Cola um exemplo de fala dela..." rows="1"></textarea>' +
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        if (panel.classList.contains('axis-visible')) { carregarEspacoHistorico(); switchTab('espaco'); }
    });
    ...
    document.getElementById('axis-tom-folha-select').addEventListener('change', (e) => {
        carregarFolhaTomSelecionada(e.target.value).catch(reportFatalError);
    });
