...
function scope() {
    ...
    if (!data[key]) {
        ...
        data[key] = {
            ...
            [KEYS.ALIVE]: { lastRambling: 0, ramblingLog: [], status: 'idle' },
            [KEYS.USER_BEHAVIOR]: { interactions: 0, lastMsg: 0, avgGap: 0, gaps: [], level: 'normal' },
            [KEYS.VOICE]: { lines: [], personality: '' }, // RAG de voz: falas com embedding + descrição única (Ticket 1)
            _resolved: {},
        };
        save();
    }
...
Se você já notou esse mesmo tipo de padrão mais de uma vez (olhe suas últimas observações acima), considere seriamente propor um sistema pra ajustar isso agora, em vez de só observar de novo.
...
// Compila UM sistema leve (tipo 'voice') cujo promptText é a descrição de
// personalidade — curta e estática. As falas (lines) NÃO entram aqui: elas
// são recuperadas dinamicamente por similaridade a cada geração do RP, ver
// applyVoiceRetrieval() (Ticket 1 — RAG de verdade, não mais um shelf que só
// cresce).
function endVoiceTraining(chatId) {
    ...
    const voice = s[KEYS.VOICE] || (s[KEYS.VOICE] = { lines: [], personality: '' });
    if (!voice.lines.length && !voice.personality) {
...
async function processVoiceLines(text, chatId) {
    ...
    for (const m of matches) {
        ...
        result = result.replace(m[0], '🎙️ _Fala guardada: "' + preview + '"_');
    }
...
// Retrieval em tempo real — o pulo do gato do Ticket 1. Roda a cada geração
// real do RP (chamado de dentro de Spade_interceptGeneration), não só uma
// vez no fim do treino. queryEmb pode vir já calculado (reaproveitado com o
// ranking de sistemas do Ticket 2) ou null, caso em que cai pro fallback
// das últimas 6 falas sem ranking por similaridade.
async function applyVoiceRetrieval(recentText, queryEmb) {
    ...
    if (queryEmb) {
        top = voice.lines
            ...
            .slice(0, 6);
        if (!top.length) top = voice.lines.slice(-6); // nenhuma linha com embedding ainda
    } else {
        top = voice.lines.slice(-6); // sem embedding de consulta — fallback determinístico
    }
...
async function Spade_interceptGeneration(chatArr, contextSize, abort, type) {
    try {
        ...
        await applyVoiceRetrieval(recentText, queryEmb);
        await applySystemsRanked(queryEmb);
...
// ====================================
// UPLOAD DE ARQUIVO NO TREINO/CHAT (Ticket 3)
...
function handleFileAttach(e) {
    ...
    files.forEach(file => {
        ...
        if (isTextType || isTextExt) {
            ...
            reader.onerror = () => {
                input.value = (input.value ? input.value + '\n\n' : '') + '[Não consegui ler "' + file.name + '".]';
            };
            reader.readAsText(file);
        } else {
            input.value = (input.value ? input.value + '\n\n' : '') + '[Não consigo ler esse tipo de arquivo ainda ("' + file.name + '") — manda como texto colado, ou um .txt/.md/.csv/.json.]';
            input.focus();
        }
    });
}
...
O que fazer:
1. Pra cada fala/exemplo REAL de diálogo que o usuário mandar (uma linha isolada, só o diálogo em si, sem comentário seu), extraia com: [AXIS:VOICE_LINE]a fala em si, sem aspas nem comentário[/AXIS:VOICE_LINE] — pode ter várias na mesma resposta, uma tag por exemplo.
